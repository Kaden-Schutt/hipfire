...
def silu_mul_ref(gate: np.ndarray, up: np.ndarray) -> np.ndarray:
    return (gate.astype(np.float32) * sigmoid_ref(gate).astype(np.float32) * up.astype(np.float32)).astype(bfloat16)


def run_test(
    xclbin_path: Path, instr_path: Path, hidden_size: int, seed: int = 42, atol: float = 0.02, rtol: float = 0.02
) -> bool:
    ...
    if pass_:
        ...
        for i in failing[:8]:
            print(
                f"    [{i}] gate={float(gate[i]):.4f} up={float(up[i]):.4f} "
...
def main() -> None:
    ...
    parser.add_argument(
        "--atol", type=float, default=0.02, help="Absolute tolerance (default 0.02; bfloat16 unit ulp ~0.008 at 1.0)"
    )
