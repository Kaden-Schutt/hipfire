...
def headnorm_rope_ref(x: np.ndarray, weight: np.ndarray, cs: np.ndarray,
...
def run_test(label: str, xclbin_path: Path, instr_path: Path,
             n_heads: int, head_dim: int,
             weight: np.ndarray, cs: np.ndarray,
    ...
    print(f"  [{label}] correctness: max_abs={max_abs:.5f} mean_abs={mean_abs:.5f} "
    ...
    if not pass_:
        ...
        for idx in failing[:8]:
            ...
            print(f"    [h={h}, i={i}] in={flat_in[idx]:.4f} "
    ...
    return dict(label=label, n_heads=n_heads, head_dim=head_dim,
...
def make_cs(n_rot: int, freq_base: float = 500000.0, pos: int = 1) -> np.ndarray:
    ...
    n_rot2 = n_rot // 2
    freqs = np.array([
        1.0 / (freq_base ** (2 * i / n_rot)) for i in range(n_rot2)
    ], dtype=np.float32)
    angles = pos * freqs
...
def main() -> None:
    ...
    run_test("q", q_xclbin, q_instr, args.n_heads, head_dim, weight, cs,
             args.warmup, args.timed, args.atol, args.rtol)
    ...
    run_test("k", k_xclbin, k_instr, args.n_kv_heads, head_dim, weight, cs,
             args.warmup, args.timed, args.atol, args.rtol)
