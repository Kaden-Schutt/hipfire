...
def run_test(xclbin_path: Path, instr_path: Path, hidden_size: int,
             n_warmup: int = 20, n_timed: int = 200,
    ...
    if pass_:
        ...
        for i in failing[:8]:
            print(f"    [{i}] x={float(x[i]):.4f} w={float(weight[i]):.4f} "
...
def main() -> None:
    ...
    parser.add_argument("--hidden-size", type=int, default=1536,
                        help="Model hidden size (1536 for 1.5B, 3584 for 7B)")
    ...
    ok = run_test(xclbin, instr, args.hidden_size,
