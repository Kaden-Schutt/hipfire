...
def run_test(xclbin_path: Path, instr_path: Path, n_heads: int, ctx_len: int,
             warmup: int = 20, timed: int = 200,
             atol: float = 0.01, rtol: float = 0.01) -> dict:
    ...
    if not pass_:
        ...
        for idx in failing[:8]:
            h, i = idx
            print(f"    [h={h}, i={i}] in={float(scores[h,i]):.4f} "
    ...
    return dict(n_heads=n_heads, ctx_len=ctx_len,
...
def main() -> None:
    ...
    parser.add_argument("--ctx-len",    type=int, required=True,
                        help="Context length (must match a built xclbin)")
    ...
    run_test(xclbin, instr, args.n_heads, ctx_len,
