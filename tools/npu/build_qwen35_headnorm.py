...
"""Compile the NPU per-head RMSNorm kernel for Qwen3.5 Q and K tensors.

Produces four files (two per tensor, Q and K):
  - <out_dir>/qwen35-headnorm-q-{n_heads}h{head_dim}d.xclbin  / -instr.bin
  - <out_dir>/qwen35-headnorm-k-{n_kv_heads}h{head_dim}d.xclbin / -instr.bin

The kernel applies RMSNorm per head using a shared weight vector [head_dim]
that is the same for all head iterations (tensor param, acquired once).
This mirrors gpu.rmsnorm_batched() in the Qwen3.5 forward pass.

Design: _transform_gen (single-core), tile_size=head_dim. The kernel is called
once per head; N_div_n = n_heads (Q) or n_kv_heads (K). The weight buffer is a
tensor param acquired once and reused for all head iterations.

Usage:
    python tools/npu/build_qwen35_headnorm.py \\
...
def build_one(
    ...
    weight_ty: Any = np.ndarray[(head_dim,), np.dtype[bfloat16]]

    include_dirs = []
    if AIE_INCLUDE and AIE_INCLUDE.is_dir():
        include_dirs.append(str(AIE_INCLUDE))

    # arg_types: (input=elem_in, output=elem_out, weight=param0, head_dim=auto-n)
    kernel = ExternalFunction(
        name="rms_norm_head_bf16",
