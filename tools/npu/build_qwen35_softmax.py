...
def build_one(n_heads: int, ctx_len: int, out_dir: Path, npu: str) -> None:
    ...
    print(f"[build_qwen35_softmax] n_heads={n_heads} ctx_len={ctx_len} "
          f"npu={npu} arch={target_arch}")
...
def build(n_heads: int, ctx_lens: list[int], out_dir: Path, npu: str) -> None:
    ...
    for ctx_len in ctx_lens:
        tag = f"CTX{ctx_len}"
        print(f"  export HIPFIRE_QWEN35_XDNA1_SOFTMAX_{tag}_XCLBIN="
              f"{out_dir}/qwen35-softmax-{n_heads}h{ctx_len}ctx.xclbin")
        print(f"  export HIPFIRE_QWEN35_XDNA1_SOFTMAX_{tag}_INSTR="
...
def main() -> None:
    ...
    parser.add_argument("--ctx-lens",  default="64,128,256,512",
