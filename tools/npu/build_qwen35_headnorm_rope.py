...
def build_one(
    ...
    print(
        f"[build_qwen35_headnorm_rope] {label}: n_total={n_total} head_dim={head_dim} "
    ...
    if n_rot % 32 != 0:
        raise ValueError(f"n_rot={n_rot} must be a multiple of 32 (n_rot/2 must be divisible by VEC=16)")
    if (head_dim - n_rot) % 16 != 0:
        raise ValueError(
            f"head_dim-n_rot={head_dim - n_rot} must be a multiple of 16 (passthrough region VEC alignment)"
        )
    ...
    mlir_module = _transform_gen(kernel, [qk_buf], out_buf, packed_buf, tile_size=head_dim)
...
def build(n_heads: int, n_kv_heads: int, head_dim: int, n_rot: int, out_dir: Path, npu: str = "auto") -> None:
    ...
    print(
        f"  export HIPFIRE_QWEN35_XDNA1_HEADNORM_ROPE_Q_XCLBIN="
        f"{out_dir}/qwen35-headnorm-rope-q-{n_heads}h{head_dim}d.xclbin"
    )
    print(
        f"  export HIPFIRE_QWEN35_XDNA1_HEADNORM_ROPE_K_XCLBIN="
