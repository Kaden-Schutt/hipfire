...
| `crates/hipfire-daemon/src/main.rs` | Parse `image_base64` in generate dispatch; refactor `generate_vl` to take `GenerateVLParams` struct with `ImageSource` enum; decode base64 + from-bytes path; move capacity guard below preprocess; fix log line for bytes path; error on `image` + non-VL model (was silent-ignore) |
