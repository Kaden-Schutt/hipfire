...
| Backend module contracts | IN PROGRESS FOR QWEN35 V1 | The first in-place module contract covers Qwen35 dense FFN/SwiGLU/down. It names `qwen35_dense_ffn_swiglu_down`, records CPU oracle, GPU production, and NPU opt-in backend preferences, and builds a module invocation/output object for both the normal GPU production path and the BF16 compare/cpu probe path. The adjacent Qwen35 attention `wo` residual projection now has the same in-place invocation shape on its production helper. Probe evidence includes selected backend, module id, CPU oracle backend, drift stats, and fallback reason. Decode done telemetry and `/health.decode_batch` expose selected backend, batch size, compatible state kinds, real cached-prefix token totals, and fallback reason. XDNA execution remains reserved until a real NPU backend lands. | `crates/hipfire-arch-qwen35/src/ffn_bf16.rs`, `crates/hipfire-arch-qwen35/src/qwen35.rs`, `crates/hipfire-daemon/src/main.rs`, `cli/index.ts` | Promote the contract vocabulary out of the BF16 helper module when broader extraction begins. |
...
| Decode batching | IN PROGRESS FOR QWEN35 V1 | Non-streaming text-only greedy Qwen35/Qwen35-MoE requests that already passed server prefill can enter a worker-local decode active set. The daemon exposes `generate_batch_decode_step`; `serial_reference` remains the fallback oracle. `HIPFIRE_QWEN35_DECODE_BATCH=auto` now selects dense Qwen35 FP32-state `fused_dense_layer_chunked` when resident session capability checks pass, and otherwise remains serial. Dense native multi-row chunks are enabled with `HIPFIRE_QWEN35_DECODE_NATIVE_MULTIROW=1`; batch-size 2/4/8, token-count 4, chunk-cap 1/2/8 parity smokes and 128-request stress pass. Qwen35-MoE `fused_grouped_moe_layer_chunked` remains explicit and now advances multi-session chunks through the grouped-MoE native row worker, with `serial_reference` available as the oracle and internal parity gate. Real A3B MQ4 server decode parity/latency smokes pass for B=2/4/8 with forced chunk cap 2; auto grouped-MoE promotion stays disabled until broader latency evidence is strong enough. Quantized dense KV/state, compacted dense KV, streaming routing, tools/images, sampling, DFlash/MTP, and non-Qwen35 backends remain deferred. | `cli/worker_scheduler.ts`, `cli/index.ts`, `crates/hipfire-daemon/src/main.rs`, `tests/smoke-server-decode-batch.sh`, `tests/stress-server-concurrency.sh` | Collect broader grouped-MoE decode latency evidence before considering conservative auto promotion. |
| MTP/DFlash verify batching | DEFERRED | Planned but intentionally disabled for multi-request batching. Qwen35 speculative decode now has a rollback-admission guard for accept/reject commit shape and AR replay boundary alignment, and the daemon checks it before advancing DFlash state. | `crates/hipfire-arch-qwen35/src/speculative.rs`, `crates/hipfire-daemon/src/main.rs` | Requires full KV/DeltaNet/logits parity evidence before any multi-request verify batching. |
...
| Decode active set | IN PROGRESS | Worker-local active decode scheduler exists for non-streaming greedy Qwen35 V1; serial fallback, auto dense FP32-state native chunked, and explicit Qwen35-MoE chunked daemon step backends are wired. Dense native multi-row chunks are parity/stress-smoked; explicit native grouped-MoE chunks have serial-oracle parity coverage on real A3B MQ4 B=2/4/8 server smokes with forced multi-chunk telemetry. Auto grouped-MoE promotion and streaming-safe routing remain future work. |
...
| Priority | Slice | Why |
|---|---|---|
| 1 | Collect broader grouped-MoE decode latency evidence and decide whether auto promotion is safe. | Explicit grouped-MoE decode now uses the native grouped row worker, B=2/4/8 real A3B parity/latency smokes pass with forced chunk cap 2, and auto remains serial until evidence is strong. |
...
| 3 | Replace Qwen35-owned backing state with allocator-owned generic pages. | Model-worker descriptors now expose typed page kinds, shape metadata, and sequence-state handle identity for Qwen35 active sessions and checkpoints. The backing storage is still the Qwen35-owned session map rather than allocator-owned generic pages. |
