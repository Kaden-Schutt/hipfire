...
| Sampling / argmax | **L39** Device-side argmax | On-GPU argmax on logits, host gets one int | `argmax_stage1_kernel` + `argmax_stage2_kernel` (`linear/lm_head.hip:67,105`) | Host argmax (`bench_qwen35_speed.rs:417`) | XS | +0% prefill / +0.5-1% decode | Same as L37, separately costed for clarity. |
...
- `crates/hipfire-runtime/examples/bench_qwen35_speed.rs:1-200` — bench config + argmax (L39)
