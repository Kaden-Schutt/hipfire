...
echo auto | sudo -n tee "$PL" >/dev/null   # NEVER force high (under-clocks RDNA3/4); auto = representative wall%
...
echo profile_standard | sudo -n tee "$PL" >/dev/null
...
echo auto | sudo -n tee "$PL" >/dev/null
