...
# DRY_RUN=1 (default) reports the fold candidate + composed A/B + gate; does NOT advance.
set -u
CUR_ROUND="${1:-0}"
DRY_RUN="${DRY_RUN:-1}"
MAIN="$HOME/hipfire"
...
python3 - "$MAIN" "$BASELINE_REF" <<'PY' > /tmp/rollv2_pending.tsv 2>/dev/null
...
if HIPFIRE_REPO="$WT" timeout 1400 bash /tmp/oracle_profile.sh gfx1201 0 0 "$MODEL" 40 > /tmp/bod_new.json 2>/dev/null \
  ...
  cp /tmp/bod_gfx1201.json "/tmp/bod_gfx1201.v$((NEWN-1)).json" 2>/dev/null; cp /tmp/bod_new.json /tmp/bod_gfx1201.json
  python3 - /tmp/bod_gfx1201.json "$VMED" <<'PY'
import json,sys
