#!/usr/bin/env bash
# ab_certify_v2b.sh <arch> <dev> <card> <model> <kernel> <label> <candidate.patch>
# v2b = v2 (adaptive sampling, clean worktree ownership, clock-gating, coherence) + the EXPANDED launch-config
# surface: candidate is an arch-gated git-diff patch (.hip + gemm.rs/gemv.rs), guarded by the ARCH-BLEED check.
...
set -u
ARCH=$1 DEV=$2 CARD=$3 MODEL=$4 KERNEL=$5 LABEL=$6 PATCH=$7   # PATCH = git-diff: .hip + ARCH-GATED gemm.rs/gemv.rs
...
git checkout "$BASELINE_REF" -- kernels/src crates/rdna-compute/src 2>/dev/null || git checkout -- kernels/src crates/rdna-compute/src 2>/dev/null
...
measure(){ local bin=$1 out=/tmp/v2m_${ID}.jsonl clkf=/tmp/v2m_${ID}.clk
  rm -f .hipfire_kernels/*/*.hsaco "$clkf" 2>/dev/null   # v2b: candidate may touch multiple kernels
...
# ---- apply candidate PATCH (.hip + arch-gated launch config) + ARCH-BLEED GUARD ----
git apply --whitespace=nowarn "$PATCH" 2>/tmp/v2b_apply_$ID || { git checkout "$BASELINE_REF" -- kernels/src crates/rdna-compute/src 2>/dev/null; echo "{\"arch\":\"$ARCH\",\"label\":\"$LABEL\",\"verdict\":\"PATCH_APPLY_FAIL\"}"; exit 0; }
BLEED=$(python3 - "$BASELINE_REF" <<'PYB'
...
def L(get):
  ...
  for f in FS:
    ...
    for m in re.findall(r"\[\s*[0-9][0-9*u\s]*u32\s*,\s*1\s*,\s*1\s*\]",x):s.add(re.sub(r"\s","",m))
  return s
bl=L(lambda f:subprocess.check_output(["git","show",f"{base}:{f}"],text=True,stderr=subprocess.DEVNULL))
tl=L(lambda f:open(f).read())
print("BLEED" if (bl-tl) else "OK")
PYB
)
[ "$BLEED" = BLEED ] && { git checkout "$BASELINE_REF" -- kernels/src crates/rdna-compute/src 2>/dev/null; echo "{\"arch\":\"$ARCH\",\"label\":\"$LABEL\",\"verdict\":\"ARCH_BLEED_FAIL\"}"; exit 0; }
if ! build; then git checkout "$BASELINE_REF" -- kernels/src crates/rdna-compute/src 2>/dev/null; echo "{\"arch\":\"$ARCH\",\"label\":\"$LABEL\",\"verdict\":\"VARIANT_BUILD_FAIL\"}"; exit 0; fi
cp "$DB" /tmp/v2_var_$ID
git checkout "$BASELINE_REF" -- kernels/src crates/rdna-compute/src 2>/dev/null; git clean -fdq kernels/src 2>/dev/null
...
if [ "$VERDICT" = WIN ]; then
  git checkout "$BASELINE_REF" -- kernels/src crates/rdna-compute/src 2>/dev/null; git clean -fdq kernels/src 2>/dev/null
  git apply --whitespace=nowarn "$PATCH" 2>/dev/null; git add -A 2>/dev/null
...
python3 - "$ARCH" "$KERNEL" "$LABEL" "$(basename "$PATCH")" "$VERDICT" "$ROUNDS" "$F" "$DELTA" "$BM" "$VM" "$BC" "$VC" "$COMMITTED" "$LEDGER" "${BASE[*]}" "${VAR[*]}" "$BP" "$VP" <<'PY'
import sys,json
