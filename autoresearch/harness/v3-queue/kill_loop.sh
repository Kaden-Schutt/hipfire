...
for pat in "driver_v3" "codex exec" "ab_certify_v2" "release/examples/daemon" "loop_round" "gen_digest" "rollover_v2"; do
  pkill -9 -f "$pat" 2>/dev/null
done
sleep 3
echo "remaining: driver=$(pgrep -cf driver_v3) codex=$(pgrep -cf 'codex exec') certify=$(pgrep -cf ab_certify_v2) daemon=$(pgrep -cf 'release/examples/daemon')"
