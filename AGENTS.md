...
- **Resource Lock Protocol:** `hipfire-daemon` acquires `flock(2)` leases (one `<resource>.lock` file per GPU/NPU/CPU resource) under `~/.hipfire/locks/` before HIP init — the same `hipfire-lock` `FlockGuard` primitive as the `hipfire gpu-lock` CLI. Override the root with `HIPFIRE_RESOURCE_LOCK_DIR`; use `HIPFIRE_RESOURCE_LOCK_WAIT_MS` to wait for busy leases. `flock` auto-releases when the holder process exits (no stale-lock reclamation needed). Non-daemon GPU binaries (cargo `--example` benches, `hipfire eval`, `hipfire-quantize`) do not self-lock — coordinate them with the native CLI mutex `hipfire gpu-lock {acquire,release,status}` (the legacy `scripts/gpu-lock.sh` shell adapter has been removed).
