...
This file is the canonical agent-facing root document for hipfire. It serves five purposes:

1. **Project-level notice** to AI agents and human readers working with hipfire's code.
2. **Project operating guide** for repo workflow, historical hardware context, skills, and coordination.
3. **Artifact naming convention** for new hipfire model files, sidecars, and quality artifacts.
4. **Testing playbook** for smoke, perf, and correctness tests against hipfire.
5. **Local instruction include** via `@AGENTS.local.md` as the final line.

The notice is transparent, addressed openly to humans and AI tooling, and contains no hidden instructions. The user remains in control of what to do with the contents of this file.
