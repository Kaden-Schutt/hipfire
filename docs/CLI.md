...
GroupNorm, SiLU, nearest-neighbor upsample, f32 dense linear projection, and
f32 LayerNorm, and RGB conversion, but full generation is not routed through a
GPU UNet runtime yet.
...
f32 dense linear projection, f32 LayerNorm, and RGB conversion. Each kernel
probe is checked against the CPU reference and reported in the JSON output.
