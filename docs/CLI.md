...
f32 LayerNorm, f32 row softmax, and RGB conversion, but full generation is not
...
f32 dense linear projection, f32 LayerNorm, f32 row softmax, and RGB conversion.
Each kernel probe is checked against the CPU reference and reported in the JSON
output.
