...
P6 prompt (committed at `benchmarks/prompts/trains-meet.txt` once the PR adding it lands; for now its bytes are):
...
Built `hf_hidden_dump.py` (CPU since gfx906 isn't in ROCm 6.4's rocBLAS) — runs HF forward and dumps per-layer post-block residual streams. Diffed against hipfire's `forward_scratch_with_hidden` output per-position per-layer.
...
- `hf_hidden_dump.py` — HF transformers reference dump (per-layer post-block residual stream in a simple binary format)

The Phase 6.2 / 7.5 / 7.6 diff scripts (per-channel cos, layer×position grid, 3-channel decomposition, MLP sub-stage bisect, etc.) were ~20-50 line numpy programs that became misleading once Phase 7.7 retracted their underlying comparison. They are not committed here. The HFHS binary format they consumed is documented below so they can be recreated.
...
`hf_hidden_dump.py` in this directory writes that format. A diff script template:
...
For the FA-stages dump format (used in Phase 7.6 MLP-bisect, also retracted), each record is `32B header (8× u32: layer_idx, pos, stage_id, n_elems, 0, 0, 0, 0)` then `n_elems` f32 values. The hipfire-side writer is `dump_fa_stage` in `crates/hipfire-arch-qwen35/src/qwen35.rs` (env-gated via `HIPFIRE_DUMP_FA_STAGES` + `HIPFIRE_DUMP_DN_LAYER`).
...
PROMPT_JSON=$(python3 -c "import json; print(json.dumps(open('/path/to/p6.txt').read()))")
(
...
docker run --rm --device /dev/dri --device /dev/kfd \
    ...
    -n 1500 -p "$(cat /path/to/p6.txt)" --no-display-prompt
```
...
user = open('/path/to/p6.txt').read().rstrip('\n')
