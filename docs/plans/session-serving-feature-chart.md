...
| Priority | Slice | Why |
|---|---|---|
| 1 | Migrate Qwen35 session ownership into the generic state-arena scaffold. | Boundary checkpointing is still implemented in the wrapped Qwen35 serial path; the next step is moving checkpoint/fork/release operations behind the generic arena contract. |
| 2 | Expose equivalent boundary snapshots from fused prefill backends. | Serial prefill can checkpoint exact intermediate positions today; fused dense/grouped workers need a generic arena snapshot hook instead of serial segmentation. |
