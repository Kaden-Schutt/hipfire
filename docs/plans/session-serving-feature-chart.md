...
| Request-path exclusion for streaming and `/v1/responses` | COMPLETE | Streaming chat and `/v1/responses` cannot queue for `generate_batch_prefill`. | `cli/server_prefill_request_path.ts`, `cli/server_prefill_request_path.test.ts`, `cli/index.ts` | None for V1. |
...
| `/v1/responses` prefill batching | DEFERRED | `/v1/responses` requests are explicitly excluded from `generate_batch_prefill`. | `cli/server_prefill_request_path.ts` | Revisit when responses path has equivalent non-streaming session semantics. |
