...
| Decode batching | IN PROGRESS FOR QWEN35 V1 | Non-streaming text-only greedy Qwen35/Qwen35-MoE requests that already passed server prefill can enter a worker-local decode active set. The daemon exposes `generate_batch_decode_step`; `serial_reference` remains the default oracle, and explicit `HIPFIRE_QWEN35_DECODE_BATCH=fused` enables dense Qwen35 FP32-state final-token `fused_dense_layer_chunked` execution through the existing session-batched worker with one token per session. Multi-step fused state advance, quantized KV/state, compacted KV, streaming routing, tools/images, sampling, DFlash/MTP, grouped-MoE fused decode, true capped per-layer chunks, and non-Qwen35 backends remain deferred. | `cli/worker_scheduler.ts`, `cli/index.ts`, `crates/hipfire-runtime/examples/daemon.rs`, `scripts/smoke-server-decode-batch.sh` | Fix fused multi-step state-advance parity, then promote dense fused decode from explicit opt-in to auto after broader parity/perf gates, grouped-MoE routed layer chunks, and true capped dense chunks. |
...
| Decode active set | IN PROGRESS | Worker-local active decode scheduler exists for non-streaming greedy Qwen35 V1; serial and explicit dense FP32-state final-token daemon step backends are covered by smoke. Multi-step fused state advance, grouped-MoE fused decode, true capped dense chunks, and streaming-safe routing remain future work. |
...
| `./scripts/smoke-server-decode-batch.sh` | NEW | Two concurrent non-streaming greedy no-think HTTP requests coalesce through server prefill, then run through the Qwen35 `generate_batch_decode_step` backend with decode-batch telemetry and no leaked resident decode sessions. Default mode checks two-token `serial_reference`; `HIPFIRE_QWEN35_DECODE_BATCH=fused` checks one-token dense FP32-state `fused_dense_layer_chunked`; add `HIPFIRE_DECODE_BATCH_PARITY=1` to compare fused response text against a serial baseline. |

## Recommended Next Slices

| Priority | Slice | Why |
|---|---|---|
| 1 | Fix dense fused multi-step state-advance parity, then promote from explicit opt-in to auto after perf gates. | `fused_dense_layer_chunked` exists for FP32-state final-token dense batches, but default `auto` still routes to serial reference. |
| 2 | Add true capped dense chunks and grouped-MoE routed layer chunks for Qwen35 decode. | Current dense fused decode processes the selected final-token batch as a whole; MoE remains explicitly gated. |
