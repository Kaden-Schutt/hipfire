...
| Decode batching | IN PROGRESS FOR QWEN35 V1 | Non-streaming text-only greedy Qwen35/Qwen35-MoE requests that already passed server prefill can enter a worker-local decode active set. The daemon exposes `generate_batch_decode_step`; `serial_reference` remains the default oracle. Explicit `HIPFIRE_QWEN35_DECODE_BATCH=fused` enables dense Qwen35 FP32-state `fused_dense_layer_chunked` native state advance with chunk telemetry in daemon responses and `/health.decode_batch`; `HIPFIRE_QWEN35_DECODE_NATIVE_MULTIROW=1` enables multi-row dense native chunks and now passes the two-row parity/stress smoke. Qwen35-MoE `fused_grouped_moe_layer_chunked` remains explicit and serial-oracle advanced. Quantized dense KV/state, compacted dense KV, streaming routing, tools/images, sampling, DFlash/MTP, native grouped-MoE routed kernels, broader dense parity/perf gates, and non-Qwen35 backends remain deferred. | `cli/worker_scheduler.ts`, `cli/index.ts`, `crates/hipfire-runtime/examples/daemon.rs`, `scripts/smoke-server-decode-batch.sh`, `scripts/stress-server-concurrency.sh` | Broaden dense fused parity/perf gates, then promote dense fused decode from explicit opt-in to auto. |
...
| Decode active set | IN PROGRESS | Worker-local active decode scheduler exists for non-streaming greedy Qwen35 V1; serial, explicit dense FP32-state native chunked, and explicit Qwen35-MoE chunked daemon step backends are wired. Dense native multi-row chunks are explicit and parity-smoked; native grouped-MoE routed kernels, broader auto-promotion gates, and streaming-safe routing remain future work. |
...
| Priority | Slice | Why |
|---|---|---|
| 1 | Broaden dense native decode parity/perf gates, then promote dense from explicit opt-in to auto. | `fused_dense_layer_chunked` accepts multi-step dense batches and reports chunk telemetry with native state advance; default `auto` still routes to serial reference until broader batch-size/token-count and latency gates pass. |
