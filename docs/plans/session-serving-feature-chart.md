...
| Backend module contracts | IN PROGRESS FOR QWEN35 V1 | The first in-place module contract covers Qwen35 dense FFN/SwiGLU/down through the BF16 oracle/probe path. It names `qwen35_dense_ffn_swiglu_down`, records CPU oracle, GPU production, and NPU opt-in backend preferences, and emits evidence with selected backend, module id, CPU oracle backend, drift stats, and fallback reason. XDNA execution remains reserved until a real NPU backend lands. | `crates/hipfire-arch-qwen35/src/ffn_bf16.rs`, `crates/hipfire-arch-qwen35/src/qwen35.rs` | Extend the contract from probe evidence into the production invocation API before broad module extraction. |
