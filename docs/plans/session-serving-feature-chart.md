...
| Prefix checkpoint attach/fork | COMPLETE FOR QWEN35 V1 | Qwen35 single-GPU prefill returns attachable checkpoint handles with daemon-authoritative `xxh128` prefix hashes computed over the rendered/tokenized runtime stream. Cache lookup can preflight candidate prompt hashes through the daemon before dispatch, exact rendered-input hits attach with full-prompt retokenization, and serial Qwen35 prefill now stores intermediate semantic-boundary checkpoints for partial-prefix reuse at completed chat-template boundaries. CLI invalidates resident manifests on reset/unload/reload/drain and on stale attach/hash mismatch failure. | `cli/state_cache.ts`, `cli/index.ts`, `crates/hipfire-runtime/examples/daemon.rs`, `scripts/smoke-server-prefix-checkpoint-reuse.sh`, `scripts/smoke-server-prefix-hash-preflight.sh`, `scripts/smoke-server-prefix-boundary-reuse.sh` | Genericize beyond Qwen35 resident checkpoints and expose equivalent boundary snapshots from fused backends through the generic state arena. |
| Prefix-cache health/telemetry shell | COMPLETE FOR QWEN35 V1 | `/health` exposes batching/cache metadata, metadata-only hits, runtime hits, resident decode sessions, resident checkpoints, resident cap, eviction/recompute counters, disk-vs-resident enablement, daemon-prefix-hash authority, semantic-boundary checkpoint presence/count, and prefix-hash preflight boundary matches. | `cli/index.ts`, `cli/prefill_batch_health.ts`, `cli/prefill_batch_health.test.ts` | Add cached-token, rehydrate, and disk-spill counters when serialization exists. |
...
| Decode batching | IN PROGRESS FOR QWEN35 V1 | Non-streaming text-only greedy Qwen35/Qwen35-MoE requests that already passed server prefill can enter a worker-local decode active set. The daemon exposes `generate_batch_decode_step`; `serial_reference` remains the default oracle, explicit `HIPFIRE_QWEN35_DECODE_BATCH=fused` enables dense Qwen35 FP32-state `fused_dense_layer_chunked` dispatch, and explicit `HIPFIRE_QWEN35_DECODE_BATCH=fused_grouped_moe` enables Qwen35-MoE `fused_grouped_moe_layer_chunked` dispatch. Both explicit chunked paths report `HIPFIRE_QWEN35_DECODE_BATCH_MAX` chunk telemetry and currently advance each chunk through the serial state oracle. Native fused multi-step state advance, quantized dense KV/state, compacted dense KV, streaming routing, tools/images, sampling, DFlash/MTP, native grouped-MoE routed kernels, native per-layer chunk kernels, and non-Qwen35 backends remain deferred. | `cli/worker_scheduler.ts`, `cli/index.ts`, `crates/hipfire-runtime/examples/daemon.rs`, `scripts/smoke-server-decode-batch.sh` | Replace the serial-oracle dense/grouped chunk paths with native fused state advance, then promote dense fused decode from explicit opt-in to auto after broader parity/perf gates. |
...
| Decode active set | IN PROGRESS | Worker-local active decode scheduler exists for non-streaming greedy Qwen35 V1; serial, explicit dense FP32-state chunked, and explicit Qwen35-MoE chunked daemon step backends are wired. Native fused state advance, native grouped-MoE routed kernels, native per-layer chunk kernels, and streaming-safe routing remain future work. |
...
| `./scripts/smoke-server-decode-batch.sh` | NEW | Two concurrent non-streaming greedy no-think HTTP requests coalesce through server prefill, then run through the Qwen35 `generate_batch_decode_step` backend with decode-batch telemetry and no leaked resident decode sessions. Default mode checks two-token `serial_reference`; `HIPFIRE_QWEN35_DECODE_BATCH=fused` checks dense FP32-state `fused_dense_layer_chunked`; add `HIPFIRE_DECODE_BATCH_PARITY=1` to compare fused response text against a serial baseline, and set `HIPFIRE_QWEN35_DECODE_BATCH_MAX=1` to require multiple reported chunks. |

## Recommended Next Slices

| Priority | Slice | Why |
|---|---|---|
| 1 | Replace dense/grouped decode's serial-oracle chunk advance with native fused state advance, then promote dense from explicit opt-in to auto after parity and perf gates. | `fused_dense_layer_chunked` and `fused_grouped_moe_layer_chunked` accept multi-step batches and report chunk telemetry, but default `auto` still routes to serial reference because the native fused state-advance kernel paths are not the oracle yet. |
| 2 | Add native grouped-MoE routed layer chunks for Qwen35 decode. | Grouped-MoE has an explicit parity-safe chunked control path; it does not yet use a routed grouped kernel. |
| 3 | Add arena snapshot/checkpoint hooks for fused prefill interior boundaries. | Top-level checkpoint/fork/release operations route through the arena wrapper, but serial boundary checkpointing still snapshots by segmenting the Qwen35 path directly. |
| 4 | Split Qwen35 wrapped state into generic state-page descriptors. | The arena wrapper is still backed by Qwen35-owned KV/DeltaNet/logits structs rather than backend-neutral pages. |
