...
| Backend module contracts | IN PROGRESS FOR QWEN35 V1 | The first in-place module contract covers Qwen35 dense FFN/SwiGLU/down. It names `qwen35_dense_ffn_swiglu_down`, records CPU oracle, GPU production, and NPU opt-in backend preferences, and builds a module invocation/output object for both the normal GPU production path and the BF16 compare/cpu probe path. Probe evidence includes selected backend, module id, CPU oracle backend, drift stats, and fallback reason. Decode done telemetry and `/health.decode_batch` expose selected backend, batch size, compatible state kinds, real cached-prefix token totals, and fallback reason. XDNA execution remains reserved until a real NPU backend lands. | `crates/hipfire-arch-qwen35/src/ffn_bf16.rs`, `crates/hipfire-arch-qwen35/src/qwen35.rs`, `crates/hipfire-daemon/src/main.rs`, `cli/index.ts` | Extend this contract to an adjacent Qwen35 segment before broad module extraction. |
