...
- `hipfire-model` owns `ModelWorkerKey` identity, feature-flag normalization, worker-key id construction, worker-key compatibility comparison, accelerator inventory/device contracts and JSON rendering, model-worker runtime id construction, daemon `worker_id` / `worker_key_id` alias parsing, and generate worker/model identity requirement policy used by scheduler/state/generate/control-plane adapters.
...
- `hipfire-model` owns tokenizer parsing, encode/decode, prompt normalization helpers, tokenizer signature/fingerprint policy, HFQ tokenizer metadata payload selection (`tokenizer` vs `gguf_meta`), optional safetensors `tokenizer.json` sidecar read policy, and HFQ chat-template extraction used by runtime tokenizer/prompt compatibility checks. Non-example Rust tokenizer callers import these contracts directly from `hipfire-model`; `hipfire-runtime::tokenizer` remains a compatibility re-export for external/example paths.
...
- `hipfire-scheduler` can consume model-owned accelerator inventory to reject prefill sessions targeting missing or unavailable worker devices while preserving unprobed inventory compatibility for existing control-plane paths.
- Rust server `/health` consumes scheduler-owned health JSON builders and reports daemon-backed accelerator inventory when a daemon is already running; the server also has a daemon-inventory-backed `PriorityPrefillScheduler` construction seam for request admission, while live Rust request queuing remains daemon-serial.
- Live Rust request handling remains daemon-serial until server request paths consume the shared scheduler queue/session contracts.
...
- `hipfire-daemon-protocol` owns the typed `inventory` request/response envelope over the model-owned accelerator inventory contract.
...
- `hipfire-daemon-adapter` owns the typed daemon inventory client call over the shared accelerator inventory contract.
...
- `hipfire-evidence` owns generic runtime oneshot evidence writers plus sparse router-histogram rendering contracts; model-specific gatherers, such as Qwen3.5 MoE router counters, adapt their native data into these shared artifact shapes.
- The daemon AR text path accepts an additive `GenerateTextRequest.evidence_dir` control and writes standard runtime oneshot artifacts through `hipfire-evidence`; DFlash/MTP/VL evidence emission remains path-specific follow-up work.
- Daemon-backed speed eval rows pass per-case `evidence_dir` paths through `hipfire-generate` so existing runtime-evidence artifact ingestion can collect the daemon-written performance, phase-timing, memory, launch-count, and run records.
- The profile battery has a daemon-backed model anchor that joins the shared daemon-load path and requests runtime evidence artifacts.
- Daemon Qwen3.5 MoE AR requests adapt native router counters into the generic `hipfire-evidence` histogram contract when `evidence_dir` is set; DFlash/MTP/VL evidence emission remains path-specific follow-up work.
...
- `hipfire-eval` exposes an explicit `--executor daemon` mode for protocol-backed eval rows. The daemon executor uses `hipfire-daemon-adapter`, `hipfire-model`, and `hipfire-generate` to run smoke model-load, finite greedy decode, repeated greedy reset/recall, and daemon speed-timing anchors through resident daemon model loads; when smoke and speed run together, they share one daemon process/model load where semantics allow. The default `--executor auto` path prefers daemon-backed smoke/speed rows, while examples-backed pp32/pp128 speed-gate rows remain the benchmark-grade default.
- `hipfire-eval` profile rows declare the runtime evidence artifacts they expect (`performance`, `memory`, `launch_counts`, and `moe_router_histogram`) so missing model-backed artifact collection remains visible in row metadata.
