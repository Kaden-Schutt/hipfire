...
def hfq_index(path):
    b = path.read_bytes()
    assert b[:4] == b"HFQM", "not an HFQ file"
    n_tensors, = struct.unpack_from("<I", b, 12)
    meta_off, data_off = struct.unpack_from("<QQ", b, 16)
    meta_len = struct.unpack_from("<Q", b, 16)[0]
    pos = meta_off + len(json.dumps(json.loads(
        b[meta_off:data_off].split(b"\x00")[0].decode(errors="ignore") or "{}")))
    # index immediately follows the metadata blob
    pos = meta_off
    while b[pos:pos+1] != b"" and pos < data_off:
        pos += 1
        break
    return b, n_tensors, data_off
...
if __name__ == "__main__":
    src, hfq = sys.argv[1], Path(sys.argv[2])
    ...
    for name, data in codes.items():
        if data not in blob:
            (missing if name.encode() not in blob else mismatch).append(name)
    print(f"escha_code tensors in source: {len(codes)}")
    print(f"  absent from .hfq entirely : {len(missing)}")
    print(f"  present but not byte-equal: {len(mismatch)}")
    if missing or mismatch:
        for n in (missing + mismatch)[:10]:
            print("   ", n)
        sys.exit(1)
