...
**Pending GPU-box validation (code-complete, not yet hardware/runtime-exercised).** One landed change is still verified only statically (compile + clippy + no-gpu-ci Rust steps) and needs a run on an appropriate box before it is considered fully landed:
...
- **3.7 daemon typed dispatch — ✅ runtime-validated 2026-07-06** on gfx1103/nix2 via a live daemon JSONL smoke (see the 3.7 row above); no wire-shape regression, strict deserialization accepts real load/generate traffic, unknown types rejected cleanly.
