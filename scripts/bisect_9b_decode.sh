...
set -u
MODEL="${HIPFIRE_9B_MODEL:-$HOME/.hipfire/models/qwen3.5-9b-mq4.hfq}"
...
# can't hand us a stale target/release/examples/bench_qwen35_speed.
rm -f target/release/examples/bench_qwen35_speed
build_log=$(cargo build --release --features deltanet -p hipfire-runtime --example bench_qwen35_speed 2>&1)
build_rc=$?
echo "$build_log" | tail -3
if [ $build_rc -ne 0 ] || [ ! -x target/release/examples/bench_qwen35_speed ]; then
...
out=$(HIPFIRE_KV_MODE=asym3 HIPFIRE_GRAPH=1 \
    target/release/examples/bench_qwen35_speed "$MODEL" \
