...
for cfg in "${CONFIGS[@]}"; do
    ...
    HIPFIRE_RQ_BULK_BITS=$BITS HIPFIRE_RQ_PROTECT_FRAC=$FRAC HIPFIRE_RQ_GROUP=256 \
        ...
        echo "QUANTIZE FAILED bits=$BITS frac=$FRAC; tail:"
        tail -20 /tmp/rq_quant.log
        exit 1
    }
    "$HIPFIRE_GPULOCK_BIN" gpu-lock acquire "roughquant-sweep-b${BITS}-f${FRAC}" --watch-pid "$$" 2>/dev/null || true
