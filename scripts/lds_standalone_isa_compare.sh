...
OUT="${1:-/tmp/hipfire-lds-standalone-isa-compare}"
ARCH="${ARCH:-gfx1103}"
HIPCC="${HIPCC:-/opt/rocm/bin/hipcc}"
READOBJ="${READOBJ:-/opt/rocm/llvm/bin/llvm-readobj}"
OBJDUMP="${OBJDUMP:-/opt/rocm/llvm/bin/llvm-objdump}"
SRC="${SRC:-$ROOT/lds_standalone_probe.hip}"
SINGLE_INSTANTIATION="${SINGLE_INSTANTIATION:-1}"

DEFAULT_SYMBOLS=(
    "_Z9lds_probeILi6ELi6ELi512EEvPfi"
    "_Z16lds_probe_nomaskILi6ELi512EEvPfi"
)
...
cp "$SRC" "$OUT/lds_standalone_probe.hip"
...
instantiation_for_symbol() {
    case "$1" in
        "_Z9lds_probeILi6ELi6ELi512EEvPfi")
            echo "template __global__ void lds_probe<6, 6, 512>(float*, int);"
            ;;
        "_Z16lds_probe_nomaskILi6ELi512EEvPfi")
            echo "template __global__ void lds_probe_nomask<6, 512>(float*, int);"
            ;;
        *)
            return 1
            ;;
...
if [[ "$SINGLE_INSTANTIATION" == "1" ]]; then
    ...
    for symbol in "${symbols[@]}"; do
        instantiation="$(instantiation_for_symbol "$symbol")" || {
        ...
        {
            echo "#define HIPFIRE_LDS_STANDALONE_NO_MAIN"
            echo "#include \"$SRC\""
            echo "$instantiation"
        } >"$single_src"
        (
...
printf 'symbol\tsize\tinstructions\ts_nop\tds_store\tds_load\ts_barrier\ts_waitcnt\ts_cbranch\ts_and_saveexec\tglobal_load\tglobal_store\tgroup_segment\tprivate_segment\tsgpr\tvgpr\twavefront\tsection\n' >"$summary"
...
while IFS=$'\t' read -r symbol obj; do
    ...
    rg -n "s_nop|ds_store|ds_load|s_waitcnt|s_barrier|s_and_saveexec|s_cbranch|global_load|global_store" \
        "$section" >"$keyops" || true
    ...
    s_waitcnt="$(count_op '^\s+s_waitcnt' "$section")"
    s_cbranch="$(count_op '^\s+s_cbranch' "$section")"
    saveexec="$(count_op '^\s+s_and_saveexec' "$section")"
    global_load="$(count_op '^\s+global_load' "$section")"
    global_store="$(count_op '^\s+global_store' "$section")"
    ...
    printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
        ...
        "$s_barrier" "$s_waitcnt" "$s_cbranch" "$saveexec" "$global_load" \
        "$global_store" "$group_segment" "$private_segment" "$sgpr" "$vgpr" \
        "$wavefront" "$section" >>"$summary"
done <"$OUT/object-list.txt"
