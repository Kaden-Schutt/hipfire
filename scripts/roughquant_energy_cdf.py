...
HESS = (
    sys.argv[1]
    if len(sys.argv) > 1
    else f"{__import__('os').path.expanduser('~')}/.hipfire/hessians/qwen3.5-0.8b.hessian.bin"
)
SNAP = sys.argv[2] if len(sys.argv) > 2 else glob.glob("/srv/huggingface/models--Qwen--Qwen3.5-0.8B/snapshots/*/")[0]
DMODEL = 1024
...
def load_hessian(path, want_full=None):
    ...
    for _ in range(nt):
        ...
        sz = 4 if dt == 1 else 8
        a = (
            np.frombuffer(f.read(k * k * sz), dtype=np.float32 if dt == 1 else np.float64)
            .reshape(k, k)
            .astype(np.float64)
        )
...
def st_loader(snap):
    ...
    g = open(st, "rb")
    (hlen,) = struct.unpack("<Q", g.read(8))
    hdr = json.loads(g.read(hlen))
    base = 8 + hlen
...
def cdf_report(label, energy, fracs=(0.01, 0.02, 0.05, 0.10, 0.20, 0.50)):
    ...
    for fr in fracs:
        i = max(1, int(fr * n))
        print(f"  top {fr * 100:4.0f}% ({i:5d}) hold {100 * cum[i - 1] / tot:5.1f}%")
    for thr in (0.90, 0.99):
        i = int(np.searchsorted(cum, thr * tot)) + 1
        print(f"  {thr * 100:.0f}% energy in top {i} ({100 * i / n:.1f}%)")
...
def main():
    ...
    resid_prod = np.zeros(DMODEL)
    resid_act = np.zeros(DMODEL)
    nr = 0
