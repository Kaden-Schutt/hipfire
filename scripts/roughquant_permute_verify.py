...
W1 = rng.standard_normal((d_hid, d_in)); W2 = rng.standard_normal((d_out, d_hid))
...
print("\n=== #2 MLP neurons (SwiGLU: gate/up rows + down cols) ===")
d, m = 1024, 3584
gate = rng.standard_normal((m, d)); up = rng.standard_normal((m, d)); down = rng.standard_normal((d, m))
...
T = 5
Q = rng.standard_normal((T, n_q, hd)); K = rng.standard_normal((T, n_kv, hd)); V = rng.standard_normal((T, n_kv, hd))
...
hd = 128
q = rng.standard_normal(hd); k = rng.standard_normal(hd)
...
ok("Q·K invariant under per-head dim perm (WITH RoPE, pos_q≠pos_k)",
   np.array(qk_rope), np.array(qk_rope_perm))
