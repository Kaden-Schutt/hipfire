...
def parse_hfq(path: str) -> list[dict]:
    ...
    n = struct.unpack_from("<I", meta, pos)[0]
    pos += 4
    assert n == n_tensors, f"index count mismatch: {n} vs header {n_tensors}"

    tensors = []
    for _ in range(n):
        name_len = struct.unpack_from("<H", meta, pos)[0]
        pos += 2
        ...
        n_dims = meta[pos]
        pos += 1
        shape = list(struct.unpack_from(f"<{n_dims}I", meta, pos))
        pos += 4 * n_dims
        _gs = struct.unpack_from("<I", meta, pos)[0]
        pos += 4
        size = struct.unpack_from("<Q", meta, pos)[0]
        pos += 8
