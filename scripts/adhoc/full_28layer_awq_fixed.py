...
for i in range(n_layers):
    ...
    lw = {
        ...
        "qr": f"{p}.self_attn.q_proj",
        "kr": f"{p}.self_attn.k_proj",
        "vr": f"{p}.self_attn.v_proj",
        "or": f"{p}.self_attn.o_proj",
        "gr": f"{p}.mlp.gate_proj",
        "ur": f"{p}.mlp.up_proj",
        "dr": f"{p}.mlp.down_proj",
    }
    layers_w.append(lw)
...
kv_k = [[] for _ in range(n_layers)]
kv_v = [[] for _ in range(n_layers)]
...
def forward_token(tok_id, pos):
    ...
    for i in range(n_layers):
        ...
        kv_k[i].append(k.clone())
        kv_v[i].append(v.clone())
        ...
        o = givens_rotate(attn_out, lw["or"]) @ lw["wo"].T
        x = x + o
