...
command -v ffmpeg >/dev/null || {
    echo "extract-video-frames: ffmpeg not found" >&2
    exit 1
}
command -v ffprobe >/dev/null || {
    echo "extract-video-frames: ffprobe not found" >&2
    exit 1
}
...
[ ${#INPUTS[@]} -gt 0 ] || {
    echo "usage: extract-video-frames.sh <dir-or-file>... [--out <root>]" >&2
    exit 1
}
...
scale_filter() {
    local r
    r="$(ffprobe -v error -select_streams v:0 -show_entries stream=color_range \
...
extract_one() {
    ...
    local count
    count="$(find "$outdir" -maxdepth 1 -name 'f_*.png' | wc -l)"
...
for inp in "${INPUTS[@]}"; do
    if [ -d "$inp" ]; then
        ...
        done < <(find "$inp" -type f \( "${find_args[@]}" \) -print0)
