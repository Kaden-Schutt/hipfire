...
def load_calib_chunks(path, n_chunks=200, chunk_chars=1500):
    ...
    return [
        (
            f"calib_{i:04d}",
            data[
                (i * chunk_chars) % max(1, len(data) - chunk_chars) : (i * chunk_chars)
                % max(1, len(data) - chunk_chars)
                + chunk_chars
            ],
        )
...
def eval_recursive_acceptance(model, mtp, embed, lm_head, tok, eval_prompts, max_len=MAX_LEN_EVAL):
    ...
    for name, txt in eval_prompts:
        ...
        totals["agree0"] += agree0
        totals["total0"] += T - 1
        totals["agree1"] += agree1
        totals["total1"] += T - 2
        rows.append(
            {
                "prompt": name,
...
def main():
    ...
    for step in range(N_TRAIN_STEPS):
        ...
        if step % EVAL_EVERY == 0 or step == N_TRAIN_STEPS - 1:
            ...
            print(
                f"{step:<5} {sched.get_last_lr()[0]:<10.2e} {loss_0.item():<7.3f} {loss_1.item():<7.3f} {loss.item():<7.3f} {T:<5} {dt:<6.1f} {100 * ae_0:<6.2f} {100 * ae_1:<6.2f}"
            )
    ...
    torch.save(
        {
            ...
            "model_id": MODEL_ID,
            "baseline_agree0": a0_0,
            "baseline_agree1": a0_1,
            "final_agree0": af_0,
            "final_agree1": af_1,
