...
def main():
    ...
    print(
        f"{'step':<5} {'lr':<10} {'L0':<6} {'L1':<6} {'L2':<6} {'total':<7} {'tok':<5} {'ms':<6} {'s0':<6} {'s1':<6} {'s2':<6}"
    )
    losses_total = []
    ...
    for step in range(N_TRAIN_STEPS):
        ...
        if step > 0 and step % 200 == 0:
            torch.save(
                {"mtp_state_dict": mtp.state_dict(), "step": step, "model_id": MODEL_ID}, CHECKPOINT + f".step{step}"
            )
        ...
        if step % EVAL_EVERY == 0 or step == N_TRAIN_STEPS - 1:
            ...
            print(
                f"{step:<5} {sched.get_last_lr()[0]:<10.2e} {l0:<6.3f} {l1:<6.3f} {l2:<6.3f} {total_loss.item():<7.3f} {T:<5} {dt:<6.1f} {100 * ae[0]:5.1f} {100 * ae[1]:5.1f} {100 * ae[2]:5.1f}"
            )
    ...
    print(
        f"  delta:     step0={100 * (af[0] - a0[0]):+.2f}pp  step1={100 * (af[1] - a0[1]):+.2f}pp  step2={100 * (af[2] - a0[2]):+.2f}pp"
    )
    ...
    torch.save(
        {
            ...
            "loss_type": "recursive_K3_ce_decays",
            "decays": DECAYS,
        },
        CHECKPOINT,
    )
