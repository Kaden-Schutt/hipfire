...
def main():
    ...
    for step in range(N_TRAIN_STEPS):
        ...
        if step % EVAL_EVERY == 0 or step == N_TRAIN_STEPS - 1:
            ...
            print(
                f"{step:<5} {lr_now:<10.2e} {loss.item():<8.4f} {T:<7} {dt:<8.1f}  {100 * ae:<5.2f} ({100 * (ae - a0):+.2f})"
            )
        else:
            lr_now = sched.get_last_lr()[0]
    ...
    print(
        f"Training wall: {time.time() - train_t0:.1f}s  ({(time.time() - train_t0) / N_TRAIN_STEPS * 1000:.1f} ms/step avg)"
    )
    ...
    torch.save(
        {
            ...
            "config": {
                k: v
                for k, v in text_cfg.__dict__.items()
    ...
    json.dump(
        ...
        open("/tmp/mtp_train_result.json", "w"),
        indent=2,
    )
