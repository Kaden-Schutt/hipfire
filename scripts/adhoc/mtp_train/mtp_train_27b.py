...
def eval_acceptance(model, mtp, embed, lm_head, tok, eval_prompts, max_len=MAX_LEN_EVAL):
    ...
    for name, txt in eval_prompts:
        ...
        totals["agree"] += a
        totals["mtp_correct"] += m
        totals["total"] += int(labels.numel())
        totals["tokens"] += T
        rows.append({"prompt": name, "T": T, "agree": a / labels.numel(), "mtp": m / labels.numel()})
    if totals["total"] == 0:
        return 0.0, 0.0, rows
...
def main():
    ...
    for step in range(N_TRAIN_STEPS):
        ...
        if step % EVAL_EVERY == 0 or step == N_TRAIN_STEPS - 1:
            ...
            print(
                f"{step:<5} {sched.get_last_lr()[0]:<10.2e} {loss.item():<8.4f} {T:<5} {dt:<7.1f} {100 * ae:.2f} ({100 * (ae - a0):+.2f})"
            )
        elif step % 25 == 0:
    ...
    json.dump(
        ...
        open("/tmp/mtp_3p6_27b_result.json", "w"),
        indent=2,
    )
