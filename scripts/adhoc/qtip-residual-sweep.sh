...
CALIB="$W/qwen3.5-0.8b-bf16.calib.hfq"; IM="$HOME/.hipfire/imatrix/qwen3.5-0.8b-bf16.imatrix.gguf"
CORPUS=benchmarks/quality-baselines/slice/wikitext2-1024s-2048ctx.txt
OUT="$W/qtip-residual-sweep.md"; Q=./target/release/hipfire-quantize; PPL=./target/release/examples/perplexity
export LD_LIBRARY_PATH="${LD_LIBRARY_PATH:-/opt/rocm/lib}"
./target/release/hipfire lock acquire "qtip-residual-sweep" --watch-pid "$$" || { echo "lock busy" >&2; exit 2; }
...
for bits in 2 3 4; do
  for tr in base 3inst lr16 lr32 3inst-lr32; do
    ...
    tag="qtipR-${bits}-${tr}"; out="$W/$tag.hfq"
    if [ ! -e "$out" ]; then
      echo "[+] $tag"
      "${env_pfx[@]}" "$Q" --input "$HF" --output "$out" --format "qtip${bits}-sim" --imatrix "$IM" > "$W/quant.$tag.log" 2>&1 || { echo "  FAIL: $(tail -1 "$W/quant.$tag.log")"; continue; }
    fi
