...
def kv_cache_bytes(n_layers: int, n_kv_heads: int, head_dim: int, seq: int, mode: str) -> int:
...
def main():
    ...
    parser.add_argument(
        "--kv-mode", default="q8", choices=["q8", "asym3", "asym4", "fp32"], help="KV cache mode (default: q8)"
    )
    parser.add_argument(
        "--full-seq", type=int, default=0, help="Full KV sequence length without sidecar (default: from config)"
    )
