...
./target/release/hipfire lock acquire "qtip-treatments-sweep" --watch-pid "$$" || { echo "lock busy" >&2; exit 2; }
...
for bits in 2 3; do
  for tr in base 3inst bbt 3inst+bbt; do
    cb=1mad; bbt=off
    case "$tr" in 3inst) cb=3inst;; bbt) bbt=on;; 3inst+bbt) cb=3inst; bbt=on;; esac
