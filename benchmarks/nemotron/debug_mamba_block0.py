...
def _stub():
    ...
    def rmsnorm_fn(x, weight, bias=None, z=None, eps=1e-6, group_size=None, norm_before_gate=True, **kw):
        ...
        if z is not None and not norm_before_gate: x = x * F.silu(z.float())
        if group_size is None:
            x = x * torch.rsqrt(x.pow(2).mean(-1, keepdim=True) + eps)
        else:
            s = x.shape; xg = x.view(*s[:-1], s[-1] // group_size, group_size)
            x = (xg * torch.rsqrt(xg.pow(2).mean(-1, keepdim=True) + eps)).view(s)
        out = x * weight.float()
        if bias is not None: out = out + bias.float()
        if z is not None and norm_before_gate: out = out * F.silu(z.float())
        return out.to(dtype)

    def m(n, **a):
        mod = types.ModuleType(n); mod.__spec__ = importlib.util.spec_from_loader(n, loader=None); mod.__path__ = []
        for k, v in a.items(): setattr(mod, k, v)
        sys.modules[n] = mod
    m("mamba_ssm", __version__="2.2.2"); m("mamba_ssm.ops"); m("mamba_ssm.ops.triton")
...
def cmp(label, a, b):
    ...
    d = np.abs(a - b); rel = d.mean() / (np.abs(b).mean() + 1e-9)
...
def main():
    ...
    model = AutoModelForCausalLM.from_pretrained(args.model, trust_remote_code=True, torch_dtype=torch.float32).to("cuda")
    model.eval()
    ...
    h.append(mixer.norm.register_forward_hook(lambda m, i, o: caps.__setitem__(
        "norm_in", i[0].detach().cpu()) or caps.__setitem__("norm_gate", i[1].detach().cpu()) or caps.__setitem__("norm_out", o.detach().cpu())))
    ...
    d_inner = nh * hd; eps = cfg.rms_norm_eps
    conv_dim = mixer.conv1d.weight.shape[0]; K = mixer.conv1d.weight.shape[-1]
    proj = caps["in_proj"][0, 0].numpy().astype(np.float64)   # pos 0
    print("in_proj.shape[0] =", mixer.in_proj.weight.shape[0],
          "expected(d_mlp=0) =", d_inner + conv_dim + nh,
          "=> d_mlp =", (mixer.in_proj.weight.shape[0] - d_inner - conv_dim - nh) // 2)

    # My split [z | xBC | dt] at offset 0:
    z = proj[:d_inner]; xBC = proj[d_inner:d_inner + conv_dim]; dt_raw = proj[d_inner + conv_dim:]
    ...
    my_conv = mixer.conv1d.weight[:, 0, K - 1].detach().cpu().numpy().astype(np.float64) * xBC + mixer.conv1d.bias.detach().cpu().numpy().astype(np.float64)
    ...
    hf_xBC_act = (caps["conv1d"][0, :, 0]); hf_xBC_act = (hf_xBC_act * torch.sigmoid(hf_xBC_act)).numpy().astype(np.float64)
    x = hf_xBC_act[:d_inner]
    B = hf_xBC_act[d_inner:d_inner + ng * ns].reshape(ng, ns)
    C = hf_xBC_act[d_inner + ng * ns:].reshape(ng, ns)
    A_log = mixer.A_log.detach().cpu().numpy().astype(np.float64); D = mixer.D.detach().cpu().numpy().astype(np.float64); dt_bias = mixer.dt_bias.detach().cpu().numpy().astype(np.float64)
    ...
    mx = mact[:d_inner]; mB = mact[d_inner:d_inner + ng * ns].reshape(ng, ns); mC = mact[d_inner + ng * ns:].reshape(ng, ns)
