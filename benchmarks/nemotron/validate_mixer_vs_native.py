...
def _stub():
    ...
    def rn(*A, **K):
        x = A[0] if A else K["x"]; w = A[1] if len(A) > 1 else K["weight"]
        z = K.get("z"); eps = K.get("eps", 1e-6); gs = K.get("group_size"); nbg = K.get("norm_before_gate", True)
        d = x.dtype; x = x.float()
        if z is not None and not nbg: x = x * F.silu(z.float())
        if gs is None: x = x * torch.rsqrt(x.pow(2).mean(-1, keepdim=True) + eps)
        else:
            s = x.shape; xg = x.view(*s[:-1], s[-1] // gs, gs); x = (xg * torch.rsqrt(xg.pow(2).mean(-1, keepdim=True) + eps)).view(s)
...
def main():
    ...
    ap = argparse.ArgumentParser(); ap.add_argument("--model", required=True); ap.add_argument("--len", type=int, default=6)
    args = ap.parse_args()
    dev = "cuda"
    mdl = AutoModelForCausalLM.from_pretrained(args.model, trust_remote_code=True, torch_dtype=torch.float32).to(dev).eval()
    ...
    K = mixer.conv1d.weight.shape[-1]; eps = cfg.rms_norm_eps; L = args.len
    ...
    with torch.no_grad():
        native = Mamba2Mixer.torch_forward(mixer, x.clone(), cache_params=None, cache_position=None, attention_mask=None)
    ...
    in_proj = g(mixer.in_proj.weight); cw = g(mixer.conv1d.weight)[:, 0, :]; cb = g(mixer.conv1d.bias)
    A_log = g(mixer.A_log); D = g(mixer.D); dt_bias = g(mixer.dt_bias); mix_norm = g(mixer.norm.weight); out_proj = g(mixer.out_proj.weight)
    gs = d_inner // ng  # gated-norm group size
    xn = x[0].float().cpu().numpy().astype(np.float64)
    conv_state = np.zeros((conv_dim, K - 1)); ssm = np.zeros((nh, hd, ns))
    ...
    for t in range(L):
        ...
        conv_state[:, K - 2] = xbc
        xx = a[:d_inner].reshape(nh, hd); B = a[d_inner:d_inner + ng * ns].reshape(ng, ns); C = a[d_inner + ng * ns:].reshape(ng, ns)
    ...
    for t in range(L):
        d = np.abs(native[t] - my[t]); rel = d.mean() / (np.abs(native[t]).mean() + 1e-9)
        print(f"  pos {t}: max|Δ|={d.max():.3e} rel={rel:.5f}  (native std={native[t].std():.4f} hipfire std={my[t].std():.4f})")
