...
# the hipfire dump (bisect_nano4b example) layer-by-layer at the last position,
...
def main():
    ...
    first_bad = None
    for i in range(n):
        hf_h = hf[f"hidden_{i}"][pos].astype(np.float64)
    ...
    # logits
    hf_logits = hf["logits"][pos].astype(np.float64)
    hp = hp_logits.astype(np.float64)
    d = np.abs(hf_logits - hp)
    print(f"\nlogits: max|Δ|={d.max():.4e} rel={d.mean()/(np.abs(hf_logits).mean()+1e-9):.4f}")
    print(f"  HF top5:      {np.argsort(-hf_logits)[:5].tolist()}")
    print(f"  hipfire top5: {np.argsort(-hp)[:5].tolist()}")
