...
model_order = [
    "LFM2.5-350M.bf16.hfq",
    "qwen3.5-0.8b.bf16.hfq",
    "llama-3.2-1b-instruct.bf16.hfq",
    "qwen3.5-2b.bf16.hfq",
    "qwen3.6-35b-a3b.bf16.hfq",
    "qwen3.5-4b.bf16.hfq",
    "qwen3.5-9b.bf16.hfq",
    "qwen3.6-27b.bf16.hfq",
]
...
def fkey(f):
    return fmt_order.index(f) if f in fmt_order else 99


out = []
out.append("# Quant x KV speed matrix\n")
n_pass = sum(1 for r in rows if r["status"] == "pass")
out.append(f"Total rows: {len(rows)} | pass: {n_pass} | non-pass: {len(rows) - n_pass}\n")
out.append(
    "Metric shown: warm-decode `tok_s` (decode_tok_s for qwen) / prefill_tok_s. "
...
for m, f in sorted(by_mf, key=lambda x: (mkey(x[0]), fkey(x[1]))):
    ...
    for kv in kvs:
        ...
        if v is None:
            ...
            cells.append(f"_{v[1]}_")
        else:
            cells.append(f"{v[0]:.1f}" if v[0] is not None else "?")
...
if not fails:
    ...
    for r in rows:
        ...
        seen.add(k)
        out.append(
            f"| {r['model'].replace('.bf16.hfq', '')} | {r['format']} | {r['status']} | {r['kv']} | {r['reason'][:80]} |"
        )
