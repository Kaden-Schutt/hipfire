...
if [ -n "${ONLY_FMTS:-}" ]; then
    read -ra NONHESS <<<"$ONLY_FMTS"
    read -ra HESS <<<""
fi
keep_model() {
    [ -z "${ONLY_MODELS:-}" ] && return 0
    case " ${ONLY_MODELS} " in *" $1 "*) return 0 ;; *)
        for t in $ONLY_MODELS; do [[ "$1" == *"$t"* ]] && return 0; done
        return 1
        ;;
    esac
}
filter_kv() {
    ...
    local o=""
    for k in "$@"; do case " $ONLY_KV " in *" $k "*) o="$o $k" ;; esac done
    echo "$o"
}
...
bench_artifact() {
    local model="$1" arch="$2" fmt="$3" art="$4"
    shift 4
    local kvs=("$@")
    local bytes
    bytes=$(stat -c %s "$art" 2>/dev/null || echo 0)
    local kv
    for kv in "${kvs[@]}"; do
        if row_done "$model" "$fmt" "$kv"; then
            log "  skip (done): $model $fmt $kv"
            continue
        fi
...
quant_and_bench() {
    local model="$1" arch="$2" fmt="$3" extra="$4"
    shift 4
    local kvs=("$@")
    ...
    if [ "$need" = 0 ]; then
        log "skip model=$model fmt=$fmt (all kv done)"
        return
    fi
    ...
    if [ ! -f "$art" ]; then
        ...
        if ! timeout 5400 $QZ --input "$src" --output "$art" --format "$fmt" $extra \
            ...
            local r
            r=$(tail -1 "$EVDIR/$base.$fmt.quant.log" | tr ',\n' ';;' | cut -c1-200)
            for kv in "${kvs[@]}"; do row_done "$model" "$fmt" "$kv" \
...
for entry in "${MODELS[@]}"; do
    ...
    if [ "$need" = 0 ]; then
        log "skip Hessian for $model (all done)"
        continue
    fi
    ...
    if [ ! -f "$calib" ]; then
        fg=$(free_gb)
        if [ "$fg" -lt "$MIN_FREE_GB" ]; then
            log "DISK GUARD skip collect-artifacts $model (${fg}GB)"
            continue
        fi
