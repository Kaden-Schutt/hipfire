...
impl AsRawFd for Stream {
    fn as_raw_fd(&self) -> RawFd {
        match self {
            Stream::Http(connection) => connection.as_raw_fd(),
            #[cfg(any(feature = "ssl-openssl", feature = "ssl-rustls"))]
            Stream::Https(_) => panic!("attempted to get raw fd of secure stream"),
        }
    }
}
...
impl Stream {
    #[cfg(unix)]
    pub(crate) fn as_raw_fd_opt(&self) -> Option<RawFd> {
        match self {
            Stream::Http(connection) => Some(connection.as_raw_fd()),
            #[cfg(any(feature = "ssl-openssl", feature = "ssl-rustls"))]
            Stream::Https(_) => None,
        }
    }
...
#[cfg(unix)]
impl AsRawFd for RefinedTcpStream {
    fn as_raw_fd(&self) -> RawFd {
