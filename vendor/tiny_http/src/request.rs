...
pub struct Request {
    ...
    // response socket raw fd captured before writer type erasure; valid while Request lives.
    // Some for Unix TCP/Unix HTTP, None for secure/TestRequest. Unix-only.
    #[cfg(unix)] response_raw_fd: Option<RawFd>,
}
    ...
    W: Write + Send + 'static,
{
    ...
    Ok(Request {
        ...
        #[cfg(unix)] response_raw_fd: if secure { None } else { response_raw_fd },
    })
}
...
impl Request {
    ...
    /// Returns the raw fd for the response socket, captured before writer type erasure.
    ///
    /// On Unix, returns `Some(fd)` for TCP and Unix HTTP connections, `None` for
    /// secure (HTTPS) connections and for `TestRequest`. The fd is valid while
    /// `self` lives and refers to the same underlying socket as the response
    /// writer. Unix-only.
    #[cfg(unix)]
    #[inline]
    pub fn response_raw_fd(&self) -> Option<RawFd> { // cfg(unix)
        self.response_raw_fd
    }
