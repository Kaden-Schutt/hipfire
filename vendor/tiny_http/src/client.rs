...
pub struct ClientConnection {
    ...
    // raw fd for response socket, captured before writer type erasure.
    // Valid while the underlying connection lives; None for secure or non-HTTP cases. Unix-only.
...
impl ClientConnection {
    ...
    pub fn new(
        ...
        #[cfg(unix)]
        let response_raw_fd = if secure {
            None
        } else {
            write_socket.as_raw_fd_opt()
        };
