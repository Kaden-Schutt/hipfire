...
/// Decomposed EP MoE for ONE layer — the sole EP path in `forward_ep`.
///
/// Drives the down-projection + EP all-reduce through the axis-keyed
/// [`execute_steps_parallel`] executor.
///
...
fn minimax_ep_moe_step(
    ...
    use hipfire_dispatch::pipeline::{
        execute_steps_parallel, GemvInput, MoeActivationVariant, MoeProj, ScoreActKind, Step,
        StepCollective,
    };
    ...
    let (per_rank_steps, collectives, zero_before): (
        ...
        let zbefore: Vec<bool> = vec![false, false, false, false, false, true];
        (steps, colls, zbefore)
    };
