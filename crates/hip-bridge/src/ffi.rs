...
impl HipRuntime {
    ...
    /// Launch a COOPERATIVE kernel: all blocks co-resident, enabling grid-wide
    /// `grid.sync()`. Used by the fused MoE megakernel. Capture-safe on gfx1201/ROCm7.2
    /// (HIP value-copies kernargs into the graph node at capture — verified), so this
    /// records correctly under `hipStreamBeginCapture`. Errors if the runtime lacks
    /// `hipModuleLaunchCooperativeKernel` so the caller can fall back to the split path.
    ///
    /// # Safety
    /// `params` must contain valid pointers matching the kernel signature; `grid` must fit
    /// the co-resident limit (query via `module_occupancy_max_active_blocks_per_multiprocessor`).
    pub unsafe fn launch_cooperative_kernel(
        ...
        let t = std::time::Instant::now();
        let code = f(
            func.0, grid[0], grid[1], grid[2], block[0], block[1], block[2], shared_mem, stream_raw,
            params.as_mut_ptr(),
        );
        crate::ffi::launch_counters::record(t.elapsed().as_nanos() as u64);
    ...
    /// Max co-resident blocks per multiprocessor (WGP on RDNA wave32) for `func` at the
    /// given block size. Multiply by the RAW `hipDeviceAttributeMultiprocessorCount` (NOT
    /// the ×2 physical-CU conversion) to get the largest cooperative grid.
    ///
    /// # Safety
