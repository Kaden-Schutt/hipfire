...
//!
//! This is a **compile gate + on-demand GPU check**, not a CI test.  When the
//! drafter sidecar or the CPU reference directories are absent the example
//! prints a `skipped:` line and exits 0, so it always builds and runs as a
//! no-op without artifacts present.
//!
//! ## What is validated (when artifacts are present)
//!
//! **ctx_len=1 (backward-compat baseline)**
//! Checks (a)–(d) against the CPU reference produced by `qwen3_dspark_cpu_ref.py`:
...
//! `qwen3_dspark_cpu_ref.py --ctx-len 3`:
...
//!
//! | env var                | selects                          | fallback |
//! |------------------------|----------------------------------|----------|
//! | `DSPARK_DRAFT_HFQ`     | Qwen3-8B DSpark drafter sidecar  | `$HOME/.hipfire/models/qwen3-8b-dspark.hfq` |
//! | `DSPARK_REF_DIR`       | ctx_len=1 CPU reference dir      | `$HOME/dspark-work/qwen3_parity_refs` |
//! | `DSPARK_REF_DIR_CTX3`  | ctx_len=3 CPU reference dir      | `$HOME/dspark-work/qwen3_parity_refs_ctx3` |
//!
//! Positional args `[hfq] [refs-dir-ctx1] [refs-dir-ctx3]` override the env
//! vars if provided.  If the sidecar or either reference directory is missing,
//! the example prints `skipped: set DSPARK_DRAFT_HFQ / DSPARK_REF_DIR ...` and
//! exits 0.
//!
//! ## CPU reference
//!
//! Run BEFORE this binary (from the DSpark work tree):
//! ```text
//!   python3 qwen3_dspark_cpu_ref.py
//!   python3 qwen3_dspark_cpu_ref.py <ckpt> <refs_ctx3_dir> 42 12345 3
//! ```
//!
//! ## Usage
//! ```text
//! hipfire lock acquire dspark-qwen3
//! cargo build --release -p hipfire-arch-llama --example qwen3_dspark_parity
//! DSPARK_DRAFT_HFQ=... DSPARK_REF_DIR=... DSPARK_REF_DIR_CTX3=... \
//!     ./target/release/examples/qwen3_dspark_parity
...
/// Resolve an artifact path: positional arg (if present) wins over the env var,
/// which wins over the fallback.
fn resolve(arg: Option<String>, env_key: &str, fallback: impl FnOnce() -> String) -> String {
...
fn main() -> Result<(), String> {
    ...
    let hfq_path = resolve(std::env::args().nth(1), "DSPARK_DRAFT_HFQ", || {
        format!("{home}/.hipfire/models/qwen3-8b-dspark.hfq")
    });
    let refs_dir1 = resolve(std::env::args().nth(2), "DSPARK_REF_DIR", || {
        format!("{home}/dspark-work/qwen3_parity_refs")
    });
    let refs_dir3 = resolve(std::env::args().nth(3), "DSPARK_REF_DIR_CTX3", || {
        format!("{home}/dspark-work/qwen3_parity_refs_ctx3")
    });

    // ── Skip when artifacts are missing (compile gate / no-op mode) ────────────
    let mut missing: Vec<String> = Vec::new();
    if !Path::new(&hfq_path).is_file() {
        missing.push(format!("DSPARK_DRAFT_HFQ ({hfq_path})"));
    }
    if !Path::new(&refs_dir1).is_dir() {
        missing.push(format!("DSPARK_REF_DIR ({refs_dir1})"));
    }
    if !Path::new(&refs_dir3).is_dir() {
        missing.push(format!("DSPARK_REF_DIR_CTX3 ({refs_dir3})"));
    }
    if !missing.is_empty() {
        println!(
            "skipped: set DSPARK_DRAFT_HFQ / DSPARK_REF_DIR (and DSPARK_REF_DIR_CTX3) — missing: {}",
            missing.join(", ")
        );
        return Ok(());
    }
    ...
    // Build lm_head_f16 (same for both parity cases).  chaingun's GpuTensor has
    // no `shallow_clone`; build a non-owning full-buffer view (`sub_offset`) and
    // relabel it as F16 `[vocab]` metadata (the stored lm_head weight is F16).
    // The view must NOT be freed — it aliases `assets.weights.output.buf`.
    let full_numel = assets.weights.output.buf.numel();
    let mut lm_head_f16 = assets.weights.output.buf.sub_offset(0, full_numel);
