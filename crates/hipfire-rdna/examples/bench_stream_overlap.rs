...
fn main() {
    let m: usize = std::env::var("BENCH_M")
        .ok()
    ...
    let batch: usize = std::env::var("BENCH_BATCH")
        ...
        .and_then(|s| s.parse().ok())
        .unwrap_or(16);
    let warmup_secs: f64 = std::env::var("HIPFIRE_DPM_WARMUP_SECS")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(0.0);
    ...
    eprintln!(
        "weight tensor: {:.2} MiB",
        (m * gpr * ROW_BYTES) as f64 / (1024.0 * 1024.0)
    );
    ...
    let a_raw = gpu
        .upload_raw(&weight_bytes, &[m * gpr * ROW_BYTES])
        .expect("upload weights");
    let x_host: Vec<f32> = (0..batch * k)
        .map(|i| ((i as f32) * 1e-4) % 1.0 - 0.5)
        .collect();
    let y_init_host: Vec<f32> = (0..batch * m)
    ...
    gpu.hip
        .memcpy_htod(&y_a.buf, bytes_of(&y_init_host))
        .unwrap();
    gpu.hip
        .memcpy_htod(&y_b.buf, bytes_of(&y_init_host))
        .unwrap();
    ...
    for _ in 0..20 {
        gpu.gemm_hfq4g256_residual(&a_raw, &x_a, &y_a, m, k, batch)
            .unwrap();
    }
    gpu.hip.device_synchronize().unwrap();
    ...
    for &n_total in &n_total_list {
        ...
        for _ in 0..(n_total - half) {
            gpu.gemm_hfq4g256_residual(&a_raw, &x_b, &y_b, m, k, batch)
                .unwrap();
        }
        gpu.hip.device_synchronize().unwrap();
    ...
    let draft_m = std::env::var("BENCH_DRAFT_M")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(5120usize);
    let draft_k = std::env::var("BENCH_DRAFT_K")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(5120usize);
    let draft_n = std::env::var("BENCH_DRAFT_N")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(16usize);
    ...
    let a_draft = gpu
        .upload_raw(&draft_weights, &[draft_m * draft_gpr * ROW_BYTES])
        .expect("a_draft");
    let x_draft_host: Vec<f32> = (0..draft_n * draft_k)
        .map(|i| ((i as f32) * 1e-4) % 1.0 - 0.5)
        .collect();
    let y_draft_init: Vec<f32> = (0..draft_n * draft_m)
        .map(|i| ((i as f32) * 7e-5) % 0.5 - 0.25)
        .collect();
    ...
    gpu.hip
        .memcpy_htod(&y_draft.buf, bytes_of(&y_draft_init))
        .unwrap();
    gpu.hip.device_synchronize().unwrap();

    let verify_n = 16;
    let draft_layers: usize = std::env::var("BENCH_DRAFT_LAYERS")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(5usize);
    let verify_layers: usize = std::env::var("BENCH_VERIFY_LAYERS")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(5usize);
    ...
    for _ in 0..verify_layers {
        gpu.gemm_hfq4g256_residual(&a_raw, &x_a, &y_a, m, k, verify_n)
            .unwrap();
    }
    gpu.hip.device_synchronize().unwrap();
    ...
    for _ in 0..draft_layers {
        gpu.gemm_hfq4g256_residual(&a_draft, &x_draft, &y_draft, draft_m, draft_k, draft_n)
            .unwrap();
    }
    gpu.hip.device_synchronize().unwrap();
    ...
    eprintln!(
        "  ratio < 1.3               → A-full doomed on gfx1100, pivot to #74/#75 kernel grinds"
    );
}
