//! Phase B1 parity test for the 3 fused MQ4-Lloyd WMMA GEMMs (qkvza / qkv /
...
use hipfire_rdna::{Gpu, LLOYD_MQ4_GROUP_BYTES};
...
/// MQ4-Lloyd nibble-pair packing (matches the Phase A residual test): 256
/// indices → 128 bytes, byte[i] low nibble = idx[2i], high nibble = idx[2i+1].
...
/// Build one MQ4-Lloyd weight matrix [m × k] with **per-projection-distinct**
/// codebooks (16 entries each) so a swapped pointer in dispatch produces a
/// non-zero parity error. proj_id mixes into the codebook seed.
...
fn build_lloyd_matrix(
    ...
    for row in 0..m {
        ...
        for g in 0..groups_per_row {
            ...
            // in centroid magnitudes, so a swapped A pointer produces a
