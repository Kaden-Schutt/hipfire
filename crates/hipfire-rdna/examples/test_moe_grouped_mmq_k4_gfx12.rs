//! Tolerance-bounded A/B validation: gemm_hfq4g256_moe_grouped_mmq_k4_gfx12
//! against gemm_hfq4g256_moe_grouped_wmma_gfx12 (the FP16 grouped base).
//!
//! Same shape coverage as `test_moe_grouped_mmq_gfx12.rs` — the k4 kernel
//! is just a deeper-unroll variant of the k2 sibling. Outputs differ from
//! FP16 within the Q8_1 (7-bit) quantization noise envelope (~1-3% rel
//! error); same NRMSE bound applies.
//!
//! GFX12 ONLY. The kernel is only registered for gfx1200/gfx1201 (RDNA4).
//! Skips with a clear message on other archs.
//!
//! Run:
//!   cargo run --release -p hipfire-rdna --example test_moe_grouped_mmq_k4_gfx12
...
fn run_case(
    ...
    if !arch.starts_with("gfx12") {
        println!(
            "  SKIP — arch {} is not gfx12; i8 MMQ MoE grouped k4 kernel only registered for gfx12",
            arch
        );
        return;
    }
    ...
    let y_i8_k4 = alloc_f32_zeros(&mut gpu, m_total * m);

    // Run FP16 reference (gfx12 FP16 grouped WMMA, i8 disabled via env opt-out).
    ...
    // Run i8 MMQ k4 path (gfx12 only — direct method call so we exercise
    // the k4 kernel regardless of env-var routing).
    gpu.gemm_hfq4g256_moe_grouped_mmq_k4_gfx12(
        &expert_weight_ptrs,
        &expert_tile_ids,
        &sorted_slot_index,
        &x_src,
    ...
    .expect("i8 MMQ k4 kernel launch");
    gpu.hip.device_synchronize().expect("sync after i8 MMQ k4");

    let y_fp16_v = download_f32(&gpu, &y_fp16, m_total * m);
    let y_i8_v = download_f32(&gpu, &y_i8_k4, m_total * m);
    ...
    let rmse = (sum_sq_err / (y_fp16_v.len() as f64)).sqrt() as f32;
    ...
    println!(
        "  max_abs_diff = {:.6e} (at {}: fp16={:.6}, i8_k4={:.6})",
        max_abs, argmax_abs, y_fp16_v[argmax_abs], y_i8_v[argmax_abs]
    );
    println!(
        "  max_rel_diff = {:.6e} (at {}: fp16={:.6}, i8_k4={:.6})",
...
fn main() {
    // Toy: 1 expert, single tile_y, M=16 K=512 m_total=16 (need K≥512 so
    // kb_in_group covers both halves of a group; K=256 would only exercise
    // one Q8_1 block).
    run_case("toy", 16, 512, 16, 1, 0xDEAD_BEEF, 0xCAFE_BABE, 0.05, 0.05);
