...
//!   --example test_gated_delta_net_tree -p hipfire-rdna`
...
fn main() {
    ...
    let q: Vec<f32> = (0..N_TOKENS * N_HEADS * HD)
        .map(|i| sin_det(i, 3))
        .collect();
    let k: Vec<f32> = (0..N_TOKENS * N_HEADS * HD)
        .map(|i| sin_det(i, 5))
        .collect();
    let v: Vec<f32> = (0..N_TOKENS * N_HEADS * HD)
        .map(|i| sin_det(i, 7))
        .collect();
    ...
    for t in 0..N_TOKENS {
        let q1 = upload_f32(
            &mut gpu,
            &q[t * N_HEADS * HD..(t + 1) * N_HEADS * HD],
            &[1, N_HEADS * HD],
        );
        let k1 = upload_f32(
            &mut gpu,
            &k[t * N_HEADS * HD..(t + 1) * N_HEADS * HD],
            &[1, N_HEADS * HD],
        );
        let v1 = upload_f32(
            &mut gpu,
            &v[t * N_HEADS * HD..(t + 1) * N_HEADS * HD],
            &[1, N_HEADS * HD],
        );
        let g1 = upload_f32(
            &mut gpu,
            &gate[t * N_HEADS..(t + 1) * N_HEADS],
            &[1, N_HEADS],
        );
        let b1 = upload_f32(
            &mut gpu,
            &beta[t * N_HEADS..(t + 1) * N_HEADS],
            &[1, N_HEADS],
        );
    ...
    let tape_q8 = gpu
        .alloc_tensor(&[N_TOKENS * N_HEADS * HD * HD], DType::Raw)
        .unwrap();
...
#[cfg(feature = "deltanet")]
fn upload_i8(gpu: &mut hipfire_rdna::Gpu, data: &[i8], shape: &[usize]) -> hipfire_rdna::GpuTensor {
    let t = gpu.alloc_tensor(shape, hipfire_rdna::DType::Raw).unwrap();
    let bytes: &[u8] =
...
fn cmp_exact(label: &str, a: &[f32], b: &[f32]) -> usize {
    ...
    for (i, (x, y)) in a.iter().zip(b.iter()).enumerate() {
        if x.to_bits() != y.to_bits() {
            ...
            if n < 5 {
                eprintln!("  {label}[{i}]: ref={x} tree={y} diff={d}");
            }
            n += 1;
        }
    }
