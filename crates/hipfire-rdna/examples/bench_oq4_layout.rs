...
//!
//!   cargo run --release -p hipfire-rdna --example bench_oq4_layout
...
fn main() {
    ...
    println!(
        "decode-GEMV layout microbench (K={k}, B=1, {iters} iters/measure) on {}",
        gpu.arch
    );
    println!(
        "{:>8}  {:>12}  {:>12}  {:>8}  {:>8}",
        "M", "split us", "interleaved us", "speedup", "GB/s il"
    );
    for &m in &[1024usize, 4864, 6144] {
        let nib = lcg(1, m * (k / 2));
        let sc: Vec<f32> = lcgf(0x11, m * ng)
            .iter()
            .map(|v| 0.01 + v.abs() * 0.2)
            .collect();
        // split: [nibbles m*k/2 | scales m*ng*4]
        let mut split = nib.clone();
        for s in &sc {
            split.extend_from_slice(&s.to_le_bytes());
        }
        ...
        gpu.gemv_oq4_grouped(&wd, &ws, &xd, &ys, m, k, group)
            .unwrap();
        gpu.gemv_oq4_interleaved(&wid, &xd, &yi, m, k, group)
            .unwrap();
        ...
        println!(
            "{:>8}  {:>12.2}  {:>12.2}  {:>7.2}x  {:>7.1}  (max_diff {:.4})",
            m,
            split_us,
            il_us,
            split_us / il_us,
