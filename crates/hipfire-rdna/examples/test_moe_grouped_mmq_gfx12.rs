//! Tolerance-bounded A/B validation: gemm_hfq4g256_moe_grouped_mmq_gfx12
...
//! is ~1-3%; we use a conservative rtol/atol initially.
//!
//! GFX12 ONLY. The kernel is only registered for gfx1200/gfx1201 (RDNA4).
//! Skips with a clear message on other archs.
//!
//! Run:
//!   cargo run --release -p hipfire-rdna --example test_moe_grouped_mmq_gfx12
...
fn run_case(
    ...
    // Run FP16 reference (i8 disabled via env opt-out).
    ...
    // Run i8 MMQ path (gfx12 only — explicit direct call so the test is
    // robust to whether the wrapper dispatch sets defaults).
    gpu.gemm_hfq4g256_moe_grouped_mmq_gfx12(
        &expert_weight_ptrs,
        &expert_tile_ids,
        &sorted_slot_index,
        &x_src,
