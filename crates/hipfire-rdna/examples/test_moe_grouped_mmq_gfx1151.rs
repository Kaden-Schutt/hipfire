//! Tolerance-bounded A/B validation: gemm_hfq4g256_moe_grouped_mmq_gfx1151
...
//!
//! GFX1151 ONLY. The kernel is only registered for gfx1151 (Strix Halo
//! iGPU). Skips with a clear message on other archs.
//!
//! Run:
//!   cargo run --release -p hipfire-rdna --example test_moe_grouped_mmq_gfx1151
...
fn run_case(
    ...
    if !arch.starts_with("gfx1151") {
        println!("  SKIP — arch {} is not gfx1151; i8 MMQ MoE grouped kernel only registered for gfx1151", arch);
        return;
    }
    ...
    // test is robust to whether the wrapper dispatch sets defaults).
