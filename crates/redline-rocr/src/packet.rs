...
impl PacketImage {
    ...
    pub fn pm4_indirect_buffer(
        ...
        // Vendor-specific is packet type zero. Match ROCr's ExecutePM4
        // publication contract: the barrier orders this queue while system
        // acquire/release scopes transfer ownership of host/HIP-mutated state
        // into the retained IB and publish its completion back to the host.
        let aql_header = packet_header(0, HeaderPolicy::RECORDED_DISPATCH);
