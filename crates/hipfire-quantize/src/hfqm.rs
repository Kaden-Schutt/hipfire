...
impl std::fmt::Display for HfqmError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            HfqmError::InvalidMagic(m) => write!(f, "invalid HFQM magic: got {m:?}, expected {HFQM_MAGIC:?}"),
            HfqmError::UnsupportedVersion(v) => write!(f, "unsupported HFQM version {v}, expected {HFQM_VERSION_SUPPORTED}"),
            HfqmError::TruncatedFile { needed, have } => write!(f, "HFQM truncated: needed {needed} bytes, file is {have}"),
...
impl From<std::io::Error> for HfqmError {
    fn from(e: std::io::Error) -> Self { HfqmError::Io(e) }
}
    ...
    for (i, &b) in bytes.iter().enumerate() {
        ...
        if !in_string {
            if b == b'{' { brace_depth += 1; }
                ...
                brace_depth -= 1;
                if brace_depth == 0 { return Some(i + 1); }
            }
        }
    }
    None
...
impl HfqmPackage {
    pub fn open(path: &Path) -> Result<Self, HfqmError> {
        ...
        #[cfg(unix)] { mmap.advise(Advice::Sequential).ok(); }
        let _ = Advice::Sequential;

        if mmap.len() < HEADER_SIZE {
            return Err(HfqmError::TruncatedFile { needed: HEADER_SIZE, have: mmap.len() });
        }
        ...
        if metadata_offset > data_offset || data_offset > mmap.len() {
            return Err(HfqmError::InvalidData(format!("offsets metadata={metadata_offset} data={data_offset} len={}", mmap.len())));
        }
        let meta_bytes = &mmap[metadata_offset..data_offset];
        let json_end = json_blob_end(meta_bytes).ok_or_else(|| HfqmError::InvalidData("metadata JSON did not end".into()))?;
        ...
        if idx_n != n_entries {
            return Err(HfqmError::InvalidData(format!("index count {idx_n} != header {n_entries}")));
        }
        pos += 4;
        ...
        for _ in 0..n_entries {
            if pos + 2 > data_offset { return Err(HfqmError::InvalidData("index truncated at name len".into())); }
            let name_len = LittleEndian::read_u16(&mmap[pos..pos+2]) as usize;
            pos += 2;
            if pos + name_len + 2 > data_offset { return Err(HfqmError::InvalidData("index truncated at name/header".into())); }
            ...
            if pos + n_dims * 4 + 12 > data_offset { return Err(HfqmError::InvalidData("index truncated at shape/data_size".into())); }
            let mut shape = Vec::with_capacity(n_dims);
            for _ in 0..n_dims { shape.push(LittleEndian::read_u32(&mmap[pos..pos+4]) as usize); pos += 4; }
            pos += 4; // group_size
            let data_size = LittleEndian::read_u64(&mmap[pos..pos+8]) as usize; pos += 8;
            let payload_offset = cumulative_offset;
            cumulative_offset += data_size;
            if cumulative_offset > mmap.len() { return Err(HfqmError::TruncatedFile{ needed: cumulative_offset, have: mmap.len() }); }
            ...
            if let Some(base) = name.strip_suffix(HESSIAN_SUFFIX) {
                ...
                let dtype = match quant_type {
                    QUANT_TYPE_F32 => {
                        if k*k*4 != data_size { return Err(HfqmError::InvalidData(format!("{name} dense F32 K={k} bytes mismatch")));}
                        HfqmHessianDtype::F32
                    }
                    QUANT_TYPE_HESSIAN_BF16_TRIL_DIAG_F32 => {
                        let exp = compact_hessian_bytes(k);
                        if exp != data_size { return Err(HfqmError::InvalidData(format!("{name} compact K={k} exp {exp} != {data_size}"))); }
                        HfqmHessianDtype::Bf16TrilDiagF32
                    }
                    _ => continue,
                };
                index.insert(base.to_string(), TensorEntry{ name: base.to_string(), k, dtype, payload_offset, payload_bytes: data_size });
                continue;
            }
            if let Some(base) = name.strip_suffix(IMATRIX_SUFFIX) {
                if quant_type != QUANT_TYPE_F32 || shape.len()!=1 { continue; }
                let k = shape[0];
                if k*4 != data_size { return Err(HfqmError::InvalidData(format!("{name} imatrix bytes mismatch"))); }
                imatrix_index.insert(base.to_string(), ImatrixEntry{ name: base.to_string(), k, payload_offset, payload_bytes: data_size });
            }
        }
        Ok(Self{ mmap, _file: file, index, imatrix_index })
    }

    pub fn get(&self, name: &str) -> Option<HfqmHessianRef<'_>> {
        let e = self.index.get(name)?;
        Some(HfqmHessianRef{ name: &e.name, k: e.k, dtype: e.dtype, bytes: &self.mmap[e.payload_offset..e.payload_offset+e.payload_bytes] })
    }
    /// Alias used by calibration dispatch (expert_idx ignored).
    pub fn get_with_expert(&self, name: &str, _expert_idx: u32) -> Option<HfqmHessianRef<'_>> { self.get(name) }
    ...
    pub fn imatrix(&self, name: &str) -> Option<HfqmImatrixRef<'_>> {
        ...
        Some(HfqmImatrixRef{ name: &e.name, k: e.k, bytes: &self.mmap[e.payload_offset..e.payload_offset+e.payload_bytes] })
    }
    pub fn tensors(&self) -> impl Iterator<Item = HfqmHessianRef<'_>> + '_ {
        self.index.values().map(|e| HfqmHessianRef{ name: &e.name, k: e.k, dtype: e.dtype, bytes: &self.mmap[e.payload_offset..e.payload_offset+e.payload_bytes] })
    }
    pub fn imatrices(&self) -> impl Iterator<Item = HfqmImatrixRef<'_>> + '_ {
        self.imatrix_index.values().map(|e| HfqmImatrixRef{ name: &e.name, k: e.k, bytes: &self.mmap[e.payload_offset..e.payload_offset+e.payload_bytes] })
    }
    pub fn n_tensors(&self) -> usize { self.index.len() }
    pub fn n_imatrix_tensors(&self) -> usize { self.imatrix_index.len() }
}
...
pub fn expand_compact_hessian(compact: &[u8], k: usize) -> Vec<f32> {
    ...
    for i in 0..k { out[i*k+i] = LittleEndian::read_f32(&compact[i*4..i*4+4]); }
    for i in 0..k {
        for j in 0..i {
            let idx = lower_strict_index(i, j);
            let off = k*4 + idx*2;
...
mod tests {
    ...
    fn write_hfqm_package(entries: Vec<(String, u8, Vec<u32>, Vec<u8>)>) -> NamedTempFile {
        ...
        for (name, qt, shape, payload) in &entries {
            index_bytes.write_u16::<LittleEndian>(name.len() as u16).unwrap();
            index_bytes.extend_from_slice(name.as_bytes());
            index_bytes.push(*qt);
            index_bytes.push(shape.len() as u8);
            for &d in shape { index_bytes.write_u32::<LittleEndian>(d).unwrap(); }
            index_bytes.write_u32::<LittleEndian>(256).unwrap(); // group_size dummy
            index_bytes.write_u64::<LittleEndian>(payload.len() as u64).unwrap();
        }
    ...
    fn qt130_expansion_roundtrip_known_symmetric() {
        ...
        for i in 0..k {
            assert_eq!(expanded[i*k+i], mat[i*k+i], "diagonal must be exact at {i}");
        }
        ...
        for i in 0..k {
            for j in 0..k {
                ...
                assert!(diff <= tol, "off-diag [{i},{j}] diff {diff} exceeds {tol}: {a} vs {b}");
                // symmetry
                assert_eq!(expanded[i*k+j], expanded[j*k+i], "symmetry fail [{i},{j}]");
            }
        }
    }
    ...
    fn qt130_ref_expansion_via_package() {
        ...
        for &v in &imat { imat_bytes.extend_from_slice(&v.to_le_bytes()); }
        let tf = write_hfqm_package(vec![
            ("model.layers.0.mlp.down_proj".to_string()+".hessian", QUANT_TYPE_HESSIAN_BF16_TRIL_DIAG_F32, vec![k as u32, k as u32], h_compact),
            ("model.layers.0.mlp.down_proj".to_string()+".imatrix", QUANT_TYPE_F32, vec![k as u32], imat_bytes),
        ]);
        ...
        // diagonal exact
        for i in 0..k { assert_eq!(href.at(i,i) as f32, h[i*k+i]); }
        let dense = href.to_dense_f32();
        // dense symmetric
        for i in 0..k { for j in 0..k { assert_eq!(dense[i*k+j], dense[j*k+i]); } }
    ...
    fn dense_f32_and_compact_both_open() {
        ...
        for &v in &h_dense { h_bytes.extend_from_slice(&v.to_le_bytes()); }
        let tf = write_hfqm_package(vec![
            ("a.hessian".to_string(), QUANT_TYPE_F32, vec![k as u32,k as u32], h_bytes),
        ]);
