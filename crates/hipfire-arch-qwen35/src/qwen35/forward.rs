...
use hipfire_dispatch::families::moe::MoeRouterBackend;
...
use hipfire_runtime::moe_plan::execute_lowered_moe;
use hipfire_runtime::moe_plan::lower_moe_steps;
use hipfire_runtime::moe_plan::MoEExecutionPolicy;
use hipfire_runtime::moe_plan::MoeExecutionTarget;
use hipfire_runtime::moe_plan::MoeProgramParts;
use hipfire_runtime::moe_plan::RoutedMoeStepPhases;
...
fn moe_ffn_decode(
    ...
    // Heap debug path is Legacy-only (it borrows `ffn` directly; production
    // decode routes through `weights.moe_ffn_view`). The plan authority is
    // resolved per call here (the debug path owns no weights instance); the
    // production paths borrow the model-owned cached plans.
    let plans = Qwen35MoeGroupPlans::resolve(config)
        .map_err(|e| HipError::new(0, &format!("moe group plans: {e}")))?;
...
///
/// Works for both Legacy and Frozen via the view's descriptor metadata.
pub(crate) fn moe_ffn_has_mq3_structural(view: &MoeFfnView<'_>) -> bool {
    let is_mq3_any = |dt: DType| matches!(dt, DType::MQ3G256 | DType::MQ3G256Lloyd);
    is_mq3_any(view.router_dtype())
        || is_mq3_any(view.shared_expert_gate_dtype())
        || is_mq3_any(view.shared_gate_dtype())
        || is_mq3_any(view.shared_up_dtype())
        || is_mq3_any(view.shared_down_dtype())
}
...
pub(crate) fn moe_ffn_has_mq3_experts_uniform(view: &MoeFfnView<'_>) -> bool {
    let is_mq3_any = |dt: DType| matches!(dt, DType::MQ3G256 | DType::MQ3G256Lloyd);
    !view.expert_dtype_tags_present()
        && (0..view.expert_count()).any(|i| {
            is_mq3_any(view.expert_gate_up_dtype(i)) || is_mq3_any(view.expert_down_dtype(i))
        })
}
...
pub(crate) fn moe_ffn_has_unsupported_mq3_experts_uniform(
    ...
    unsupported_mq3_experts_uniform_from_dtypes(
        view.expert_dtype_tags_present(),
        (0..view.expert_count()).map(|i| (view.expert_gate_up_dtype(i), view.expert_down_dtype(i))),
        admit_codebook,
    )
}
...
/// Map a [`Qwen35MoeBindError`] from view tensor binding into `HipError`.
...
/// `Some` only when it spans >1 distinct tier. Mirrors the collapse inside
/// [`super::weights::per_expert_tier_tables`] for the Frozen projection path
/// (the weights-side helper takes `&MoeFfnWeights` and is Legacy-only).
fn collapse_uniform_tiers(tiers: Vec<DType>) -> Option<Vec<DType>> {
    match tiers.first() {
        // Empty (paged-like projection) or uniform → uniform fast path.
        None => None,
...
/// `MoeDtypes` for the decode family, sourced from a [`MoeFfnView`].
///
/// Legacy keeps the mainline field semantics exactly: V2-inclusive
/// all-gate-up-MQ4 and EP global-table routing when present (`None` on
/// ordinary single-GPU loads). Frozen uses the resident projection's
/// descriptors under the same V2-inclusive rules (Frozen never carries EP
/// global tables — the projection is the full layer).
fn moe_dtypes_from_ffn_view(view: &MoeFfnView<'_>) -> hipfire_dispatch::families::moe::MoeDtypes {
    let (per_expert_gate_up, per_expert_down) = match view {
        MoeFfnView::Legacy(ffn) => per_expert_tier_tables(ffn),
        MoeFfnView::Frozen(b) => {
            let p = b.descriptors();
            (
                collapse_uniform_tiers(p.expert_gate_up.iter().map(|d| d.dtype).collect()),
                collapse_uniform_tiers(p.expert_down.iter().map(|d| d.dtype).collect()),
            )
        }
    };
    let (experts_all_gate_up_mq4, routed_gate_up, routed_down) = match view {
        MoeFfnView::Legacy(ffn) => {
        ...
        MoeFfnView::Frozen(b) => {
            ...
            (
                p.expert_gate_up
                    .iter()
                    .all(|d| matches!(d.dtype, DType::MQ4G256 | DType::MQ4G256V2)),
                p.expert_gate_up
                    .first()
                    .map(|d| d.dtype)
                    .unwrap_or(DType::F32),
                p.expert_down.first().map(|d| d.dtype).unwrap_or(DType::F32),
            )
        }
    };
    hipfire_dispatch::families::moe::MoeDtypes {
        ...
        // Gate-side AWQ disables the fused gate kernel (each weight uses its
        // individual WeightRef path, which applies the per-weight scale);
        // routed-down AWQ suppresses the batched prefill Path 2 grouped GEMM.
        gate_side_has_awq: view.router_has_awq()
...
/// (STEP-002 Phase 1; qwen-local port of the pre-merge dispatch builder's
/// `MoeStepPhases`).
///
...
/// `routed`. The phase bucketers flatten exactly this order.
pub(crate) struct MoeStepPhases<'a> {
...
/// Map a [`Qwen35StepBucketError`] into the decode/prefill error domain with
/// the bucketer's context.
pub(crate) fn map_bucket_err(e: Qwen35StepBucketError) -> HipError {
    HipError::new(0, &e.to_string())
}
...
pub(crate) fn qwen35_prefill_phase_buckets(
    ...
    if let Some(step) = phases.gate_side.first() {
        return Err(Qwen35StepBucketError::UnsupportedStep {
            lane: "prefill",
            phase: "gate_side",
            kind: step_kind_label(step),
    ...
    if let Some(step) = phases.shared_down.first() {
        return Err(Qwen35StepBucketError::UnsupportedStep {
            lane: "prefill",
            phase: "shared_down",
            kind: step_kind_label(step),
...
/// them), which is why the builder takes them as parameters. O(1) — the
/// pointer tables + dims only; no per-expert materialization.
pub(crate) fn decode_expert_refs<'a>(p: &'a MoeParams<'a>) -> (MoeExpertRef<'a>, MoeExpertRef<'a>) {
    (
        MoeExpertRef {
...
/// qwen-local port of the pre-merge dispatch `build_moe_decode_steps`).
///
...
pub(crate) fn build_qwen_decode_steps<'a>(
    p: &'a MoeParams<'a>,
    res: &MoeResolution,
    gu_experts: &'a MoeExpertRef<'a>,
    dn_experts: &'a MoeExpertRef<'a>,
    router_backend: MoeRouterBackend,
    ...
    if !res.use_gpu_topk {
        return Err(DispatchError::UnsupportedVariant {
            family: "moe",
            variant: "decode-step-build-on-cpu-fallback",
            arch: "",
            quant: "",
        });
    }
    ...
    // The router backend is the architecture selection (exact-wave64 /
    // wave64 / generic two-launch) computed by
    // `select_moe_router_backend` from the current DispatchCtx — the same
    // gfx/n_exp/env rules the direct `run_moe_decode` executor applies, so
    // the sealed route never silently substitutes the generic softmax.
    ...
    let activation = if res.routed_indexable_paro {
        ...
        hipfire_dispatch::pipeline::MoeActivationVariant::QwenParo {
            pairs: paro.pairs,
            theta: paro.theta,
            scales: paro.scales,
            krot: paro.krot,
        ...
        hipfire_dispatch::pipeline::MoeActivationVariant::QwenAwqIndexed {
            awq_ptrs,
            topk_indices: p.topk_indices,
        }
    } else {
        hipfire_dispatch::pipeline::MoeActivationVariant::MinimaxFused { awq_scale: None }
    };
    ...
    // down: Lloyd AND GL self-combine atomically (no combine follows) —
    // mirroring the direct `run_moe_decode` gate (MQ2L/MQ3L/MQ2GL/MQ3GL,
    // incl. the mixed Lloyd×GL cross-products admitted by
    // `routed_indexable_mixed_lloyd`; the expanded launcher has no GL arm).
    // Every other indexable dtype writes the expanded buffer + shared
    // combine. Per-expert mixed tags force the merged expanded kernel
    // (combine MUST run).
    ...
    if routed_down_self_combines {
        ...
        phases.routed.push(Step::MoeDownIndexed {
            ...
            k_top: p.k,
            batch_size: 1,
            mode: hipfire_dispatch::pipeline::QwenDownMode::Expanded,
            dtype_tags: tags,
        });
        // Deferred-expanded protocol (combine-next-RMS): when
        // `defer_routed_combine` is set, the expanded down producer carries
        // NO local combine — the next layer's fused combine+rmsnorm
        // (`moe_down_combine_rmsnorm_mq_rotate_vecsum_gfx1100`) folds
        // `down_expanded` while producing its normalized MQ activation, so
        // emitting a combine here would double-add. The runtime lowerer
        // admits the zero-combine shape only under the explicit
        // `MoeProgramParts.deferred_combine` marker (set by the caller in
        // lockstep); the non-deferred default keeps the exact-one-combine
        // contract.
...
> {
    ...
    let lane = if res.use_gpu_topk {
        QwenDecodeLane::Gpu(build_qwen_decode_steps(
            params,
            &res,
            gu_experts,
            dn_experts,
...
/// Model-wide MQ6 fence over the assembled layers. Single implementation
/// shared with the Legacy assembly seam and the Frozen resident publication
/// (`crate::store::assembled_legacy_layers_have_mq6` →
/// `MoeFfnMetaView::has_mq6`), so the storage kinds cannot diverge. Frozen
/// markers carry no local tensors and contribute false — the resident
/// publication derives the fence separately.
...
pub(crate) fn moe_ffn_decode_with_scratch(
    ...
    moe_ffn_decode_impl(
        gpu, view, x_norm, x_residual, config, plans, &refs, false, None, false, false,
    )
}
...
pub(crate) fn moe_ffn_decode_with_scratch_prerotated(
    ...
    let refs = MoeScratchRef::from_scratch(scratch);
    moe_ffn_decode_impl(
        gpu, view, x_norm, x_residual, config, plans, &refs, true, None, false, false,
    )
}
...
fn moe_ffn_decode_impl(
    ...
    // `experts` is empty) yields None → unchanged uniform fast path. The
    // Legacy arm keeps the weights-side helper (EP global-table aware); the
    // Frozen arm collapses the projection descriptors identically.
    let (per_expert_gate_up, per_expert_down) = match &view {
        MoeFfnView::Legacy(ffn) => per_expert_tier_tables(*ffn),
        MoeFfnView::Frozen(b) => {
            let p = b.descriptors();
    ...
    let moe_dtypes = moe_dtypes_from_ffn_view(&view);
    let layer_idx = match &view {
        MoeFfnView::Legacy(ffn) => ffn.layer_idx,
        MoeFfnView::Frozen(b) => b.descriptors().layer_idx as u16,
    };
    // Deferral seam (STEP-002 Task 9): the O(n_exp) Legacy (gate_up, down)
    // ref Vec is NEVER materialized for classification or the GPU-top-K
    // indexed lane. The storage-derived predicate (Legacy layers materialize
    // per-expert refs; Frozen layers pass an EMPTY slice — the C2 indexed GPU
    // route is guaranteed) is exactly equivalent to `!routed_experts.is_empty()`
    ...
    // Paged Legacy storage keeps an EMPTY `experts` Vec (the WeightPager owns
    // the buffers) — the CPU-top-K fallback cannot index it, so residency is
    // false for paged layers exactly like Frozen: the indexed GPU route
    // (pointer tables + dtype tags) is the only admitted path, and the
    // family's `check_moe_decode_supported` guard rejects an empty-ref
    // fallback loudly instead of indexing an empty Vec by global id.
    let routed_experts_resident = matches!(
        &view,
        MoeFfnView::Legacy(ffn) if !ffn.experts.is_empty()
    );
    ...
    let moe_params = hipfire_dispatch::families::moe::MoeParams {
        ...
        router: view.router_ref().map_err(map_bind_err)?,
        shared_expert_gate: view.shared_expert_gate_ref().map_err(map_bind_err)?,
        shared_gate_w: view.shared_gate_ref().map_err(map_bind_err)?,
        shared_up_w: view.shared_up_ref().map_err(map_bind_err)?,
        shared_down_w: view.shared_down_ref().map_err(map_bind_err)?,
        expert_gate_up_ptrs: view.expert_gate_up_ptrs_tensor().map_err(map_bind_err)?,
        expert_down_ptrs: view.expert_down_ptrs_tensor().map_err(map_bind_err)?,
        ...
        expert_down_awq_ptrs: view.expert_down_awq_ptrs_tensor().map_err(map_bind_err)?,
        ...
        expert_dtype_tags: view.expert_dtype_tags_tensor().map_err(map_bind_err)?,
        routed_gate_up_k: view.expert_gate_up_k(0),
        routed_down_m: view.expert_down_m(0),
        routed_down_k: view.expert_down_k(0),
        ...
        routed_experts: &[],
        routed_gate_up_paro: view.first_expert_gate_up_paro(),
        routed_down_paro: view.first_expert_down_paro(),
    ...
    // Architecture-selected router backend: the same gfx/n_exp/env rules the
    // direct `run_moe_decode` executor applies, encoded into the decode Step
    // so the sealed route launches the identical kernel family (generic
    // softmax+renorm is numerically distinct from the wave64 fused routers).
    let router_backend = hipfire_dispatch::families::moe::select_moe_router_backend(&ctx, n_exp);
    ...
    match lane {
        ...
        QwenDecodeLane::Gpu(phases) => {
            ...
            let parts = MoeProgramParts {
                ...
                // Explicit deferred-combine marker, set in lockstep with the
                // builder's combine omission under `defer_routed_combine`:
                // only this marker admits the zero-combine ExpandedIndexed
                // program the combine-next-RMS schedule produces.
                deferred_combine: moe_params.defer_routed_combine,
    ...
    for layer_idx in 0..config.n_layers {
        match (&weights.layers[layer_idx], config.layer_types[layer_idx]) {
            ...
            (LayerWeights::DeltaNetMoe(layer), LayerType::LinearAttention) => {
                ...
                // ── MoE FFN ──
                // Weights-level dispatch path: binds Legacy or Frozen via the
                // model's resident (Frozen layers have no local tensors).
                let moe_view = weights.moe_ffn_view(layer_idx).map_err(map_bind_err)?;
...
/// Gate-side fused-rotate eligibility for [`moe_ffn_dispatch`].
///
/// Legacy keeps the mainline V2-inclusive check (MQ4G256 | MQ4G256V2, no AWQ
/// consideration — gate-side AWQ sidecars cannot occur on assembled files).
/// Frozen mirrors the same dtype rule and additionally refuses when a
/// gate-side AWQ companion is present (the resident's validated snapshot
/// semantics: `gate_side_has_awq` disables the fused path so every weight
/// uses its individual `WeightRef`, which applies the per-weight scale).
fn view_gate_side_mq4_for_dispatch(view: &MoeFfnView<'_>) -> bool {
    match view {
        MoeFfnView::Legacy(ffn) => ffn_gate_side_mq4_for_moe(ffn),
        MoeFfnView::Frozen(b) => {
            let p = b.descriptors();
            let mq4 = |dt: DType| matches!(dt, DType::MQ4G256 | DType::MQ4G256V2);
            !(p.router.awq_companion_key.is_some()
                || p.shared_expert_gate.awq_companion_key.is_some()
                || p.shared_gate.awq_companion_key.is_some()
                || p.shared_up.awq_companion_key.is_some())
                && mq4(p.router.dtype)
                && mq4(p.shared_expert_gate.dtype)
                && mq4(p.shared_gate.dtype)
                && mq4(p.shared_up.dtype)
        }
    }
}

/// All gate-side + routed gate-up MQ4 eligibility for the prerotated decode
/// fast path. Legacy is exactly [`ffn_all_mq4_for_moe`]; Frozen mirrors it on
/// the projection (V2-inclusive, with the gate-side AWQ guard from
/// [`view_gate_side_mq4_for_dispatch`]).
fn view_all_mq4_for_moe(view: &MoeFfnView<'_>) -> bool {
    view_gate_side_mq4_for_dispatch(view)
        && match view {
...
fn moe_ffn_dispatch(
    ...
    let r = if view_gate_side_mq4_for_dispatch(&view) {
    ...
    for layer in weights.layers.iter_mut() {
        match layer {
            LayerWeights::DeltaNetMoe(l) => {
                // EP sharding mutates owned expert storage; Frozen markers own
                // nothing (the resident owns the tensors) — refuse explicitly.
                let ffn = l.ffn.as_legacy_mut().ok_or_else(|| {
                    HipError::new(
                        0,
                        "shard_all_moe_layers: Frozen MoE storage cannot be sharded \
                         (Legacy-only EP path)",
                    )
                })?;
...
pub(crate) struct Qwen35Bindings<'a> {
    ...
    /// Model-owned weights for the weights-level MoE view/plan resolution
    /// (`moe_ffn_view` + `moe_group_plans`): the default-on lowered decode
    /// path binds Legacy AND Frozen MoE storage through this authority.
    pub(crate) weights: &'a Qwen35Weights,
    /// STEP-002 Task 9: model-owned expert-group plans — borrowed ONCE per
    /// forward from the weights owner's OnceLock authority and threaded into
    /// the bindings, so the SuperOp MoE ops never re-resolve the manifest or
    /// rebuild plans per layer/token/rank.
    pub(crate) plans: &'a Qwen35MoeGroupPlans,
...
impl<'a> ForwardBindings for Qwen35Bindings<'a> {
    ...
    fn run_moe(
        ...
        let s = self.s;
        let ffn_norm = match self.layer {
            LayerWeights::DeltaNetMoe(l) => &l.ffn_norm,
            LayerWeights::FullAttnMoe(l) => &l.ffn_norm,
            _ => return Err(DispatchError::Hip("MOE on dense layer".into())),
        };
        // The superop executor resolves the execution view through the
        // model-owned weights: Legacy binds the owned `MoeFfnWeights`,
        // Frozen binds the resident store projection — the default-on
        // lowered decode path serves BOTH storage kinds, with no Frozen
        // rejection on this lane (mirrors the hand arms, which bind via
        // `weights.moe_ffn_view`).
        let view = self
            .weights
            .moe_ffn_view(self.layer_idx)
            .map_err(|e| DispatchError::Hip(format!("qwen35 run_moe: {e}")))?;
        // STEP-002 Task 9: model-owned expert-group plans — borrowed once per
        // forward into the bindings; never resolved per layer/token here.
        moe_ffn_dispatch(
            gpu,
            view,
            &s.x,
    ...
    fn run_moe_ep(
        ...
        //
        // The superop EP executor resolves the view through the model-owned
        // weights (same authority as `run_moe`; the EP driver is Legacy-only
        // via reject_frozen_multi, so the Frozen arm is unreachable here but
        // the weights-level bind keeps both lanes on one resolution path).
        let view = self
            .weights
            .moe_ffn_view(self.layer_idx)
            .map_err(|e| DispatchError::Hip(format!("qwen35 run_moe_ep: {e}")))?;
        // STEP-002 Task 9: model-owned expert-group plans — borrowed once per
        // forward into the bindings; never resolved per layer/token/rank here.
        moe_ffn_dispatch_ep(
            gpu,
            view,
            &s.x,
...
fn moe_combine_next_rms_enabled(gpu: &Gpu, weights: &Qwen35Weights, config: &Qwen35Config) -> bool {
    ...
    weights.layers.iter().all(|layer| match layer {
        LayerWeights::DeltaNetMoe(l) => {
            // Assembled-Legacy admission (backup routing): the merged layer
            // carries `MoeFfnStorage`, so the FFN is reached through
            // `as_legacy`. Frozen layers are not admitted and yield false.
            l.ffn.as_legacy().map_or(false, |f| {
                has_expanded_down(f) && ffn_gate_side_mq4_for_moe(f)
            }) && mq4(&l.wqkv)
                && mq4(&l.wz)
                && mq4(&l.w_beta)
                && mq4(&l.w_alpha)
...
fn forward_scratch_layers_lowered(
    ...
    // STEP-002 Task 9: model-owned expert-group plans — resolved ONCE per
    // forward through the weights owner's OnceLock authority and borrowed by
    // the SuperOp MoE bindings per layer (never re-resolved per layer/token).
    let moe_plans = weights
        .moe_group_plans(config)
...
mod tests {
    ...
    // ── STEP-002 Task 9: decode Step program → sealed Single lowering ─────
    // CPU-pure: synthetic tensors with honest byte capacities (never touched
    // by HIP); the lowerer's capacity checks see real sizes.

    use hipfire_dispatch::families::moe::{ExpertExecutionPlan, MoeResolution, RouterPlan};
    use hipfire_runtime::moe_plan::{lower_moe_steps, MoEExecutionPolicy, MoeProgramParts};
    ...
    fn qwen35_decode_builder_reaches_runtime_lowering() {
        ...
        let phases =
            build_qwen_decode_steps(&params, &res, &gu, &dn, MoeRouterBackend::Default).unwrap();
    ...
    fn qwen35_cpu_fallback_remains_explicit_leaf() {
        ...
        let (gu, dn) = decode_expert_refs(&params);
        let (lane, res2) = classify_decode_lane(
            &params,
            &gu,
            &dn,
            4,
        ...
        .expect("classify k=4");
        assert!(!res2.use_gpu_topk);
        assert!(
            matches!(lane, QwenDecodeLane::CpuFallback),
            "k != 8 must classify to the CPU-top-K fallback lane"
        );
        // The builder refuses to express the fallback as a program.
        let build = build_qwen_decode_steps(&params, &res, &gu, &dn, MoeRouterBackend::Default);
        assert!(
            build.is_err(),
            "the CPU fallback must never be a Step program"
        );
    }

    #[test]
    fn qwen35_decode_gl_down_is_atomic_self_combining() {
        // MQ2/MQ3-G256-GL down (incl. mixed Lloyd×GL cross-products) is
        // atomic self-combining in the direct executor — the builder must
        // emit the DownResidual IndexedMoeGemv with NO MoeCombine (the
        // expanded launcher has no GL arm).
        let mut dtypes = mq4_dtypes();
        dtypes.routed_gate_up = DType::MQ2G256GL;
        dtypes.routed_down = DType::MQ3G256GL;
        let params = decode_params(8);
        let params = hipfire_dispatch::families::moe::MoeParams { dtypes, ..params };
        let res = MoeResolution::resolve_arch(&params.dtypes, 8, false);
        assert!(
            res.use_gpu_topk,
            "GL cross-products are GPU-top-K admissible (routed_indexable_mixed_lloyd)"
        );
        let (gu, dn) = decode_expert_refs(&params);
        let phases =
            build_qwen_decode_steps(&params, &res, &gu, &dn, MoeRouterBackend::Default).unwrap();
        ...
        assert_eq!(
            buckets.combine.len(),
            0,
            "GL down self-combines atomically; no MoeCombine may follow"
        );
    }

    #[test]
    fn qwen35_decode_deferred_combine_omits_moe_combine_step() {
        // mq4r lowered decode (defer_routed_combine=true) keeps the expanded
        // down write with NO local combine: the next layer's fused
        // combine+rmsnorm folds `down_expanded` (combine-next-RMS), so a
        // combine here would double-add. The runtime lowerer admits the
        // zero-combine shape only under the explicit
        // `MoeProgramParts.deferred_combine` marker, which the caller sets
        // in lockstep with the builder's omission.
        let params = decode_params(8);
        ...
        let phases =
            build_qwen_decode_steps(&params, &res, &gu, &dn, MoeRouterBackend::Default).unwrap();
        let buckets = qwen35_decode_phase_buckets(phases).unwrap();
        assert_eq!(buckets.down.len(), 1, "expanded down stays under defer");
        assert!(matches!(buckets.down[0], Step::MoeDownIndexed { .. }));
        assert_eq!(
            buckets.combine.len(),
            0,
            "the deferred producer carries no local combine (no double-add)"
        );
        // The deferred program lowers end-to-end under the explicit marker.
        let plans = Qwen35MoeGroupPlans::resolve(&task9_moe_config()).unwrap();
        let group = plans.by_layer(0);
        let policy = MoEExecutionPolicy::single();
        let parts = MoeProgramParts {
            router: RouterPlan::SoftmaxTopK {
            ...
            execution: ExpertExecutionPlan::IndexedQuantized,
            deferred_combine: true,
            ranks: vec![buckets],
        };
        lower_moe_steps(group, &policy, parts).expect("deferred program must lower");
        // The non-deferred default keeps the exact-one-combine contract.
        let params2 = decode_params(8);
        let (gu2, dn2) = decode_expert_refs(&params2);
        let phases2 =
            build_qwen_decode_steps(&params2, &res, &gu2, &dn2, MoeRouterBackend::Default).unwrap();
        let buckets2 = qwen35_decode_phase_buckets(phases2).unwrap();
        assert_eq!(
            buckets2.combine.len(),
            1,
            "non-deferred decode keeps the combine"
        );
        let parts2 = MoeProgramParts {
            router: RouterPlan::SoftmaxTopK {
                scores: params2.router_logits,
                topk_indices: params2.topk_indices,
                topk_weights: params2.topk_weights,
                k_top: 8,
                normalize: true,
                route_scale: 1.0,
            },
        ...
        lower_moe_steps(group, &policy, parts2).expect("ordinary program must lower");
    }

    #[test]
    fn qwen35_decode_paro_givens_and_indexed_gate_up_reach_lowering() {
        // Paro decode: the router-phase Givens pre-rotation + specialized
        // `MoeGateUpIndexed` + indexed Paro down lower end-to-end (the
        // runtime Givens-pairing rule admits the pre-rotation only when the
        // specialized indexed gate-up accompanies it).
        let mut dtypes = mq4_dtypes();
        dtypes.routed_gate_up = DType::ParoQ4G128;
        dtypes.routed_down = DType::ParoQ4G128;
        dtypes.has_paro_shared = true;
        let params = decode_params(8);
        let params = hipfire_dispatch::families::moe::MoeParams {
            dtypes,
            routed_gate_up_paro: Some(hipfire_dispatch::families::gemv::GivensRef {
                pairs: synth_f32(4),
                theta: synth_f32(4),
                scales: synth_f32(4),
                krot: 4,
            }),
            routed_down_paro: Some(hipfire_dispatch::families::gemv::GivensRef {
                pairs: synth_f32(4),
                theta: synth_f32(4),
                scales: synth_f32(4),
                krot: 4,
        ...
        assert!(
            ...
            "Paro k=8 must be the GPU-top-K indexed route"
        );
        let (gu, dn) = decode_expert_refs(&params);
        let phases =
            build_qwen_decode_steps(&params, &res, &gu, &dn, MoeRouterBackend::Default).unwrap();
        ...
        lower_moe_steps(group, &policy, parts).expect("Paro decode program must lower");
    }
    ...
    fn qwen35_decode_buckets_refuse_unsupported_routed_step() {
        // A prefill-only grouped gate-up deinterleave injected into a decode
        // program is refused with the fail-closed bucketing error.
        let params = decode_params(8);
        let res = MoeResolution::resolve_arch(&params.dtypes, 8, false);
        let (gu, dn) = decode_expert_refs(&params);
        let mut phases = match classify_decode_lane(
            &params,
            &gu,
            &dn,
            8,
        ...
        .0
        {
            QwenDecodeLane::Gpu(phases) => phases,
            QwenDecodeLane::CpuFallback => panic!("k=8 uniform MQ4 must build a Step program"),
        };
        phases.routed.push(Step::MoeGateUpUnscatter {
            y_grouped: synth_f32(128),
            sorted_slot_index: synth_i32_raw(128),
            gate_batch: synth_f32(8 * 128),
            up_batch: synth_f32(8 * 128),
            inter: 128,
            k_top: 8,
        ...
        match err {
            Qwen35StepBucketError::UnsupportedStep { lane, phase, kind } => {
                ...
                assert_eq!(kind, "MoeGateUpUnscatter");
            }
    ...
    fn qwen35_decode_buckets_refuse_unsupported_pre_routed_step() {
        // A grouped scatter injected into the decode ROTATE phase is refused.
        let params = decode_params(8);
        let res = MoeResolution::resolve_arch(&params.dtypes, 8, false);
        let (gu, dn) = decode_expert_refs(&params);
        let mut phases = match classify_decode_lane(
        ...
        phases.rotate = Some(Step::MoeScatter {
            topk_indices: synth_i32_raw(8),
            expert_token_counts: synth_i32_raw(8),
            expert_offsets: synth_i32_raw(9),
            sorted_slot_index: synth_i32_raw(128),
            expert_tile_ids: synth_i32_raw(8),
            inverse_perm: synth_i32_raw(8),
            total_slots: 8,
            n_experts: 8,
            m_total_max: 128,
            block_m: hipfire_dispatch::families::moe::MOE_GROUPED_BLOCK_M,
        });
        let err = match qwen35_decode_phase_buckets(phases) {
            Err(err) => err,
    ...
    fn qwen35_decode_buckets_refuse_backward_permuted_routed_steps() {
        ...
        // Swap activation and down → every variant is SUPPORTED; only the
        // order is wrong — the bucketer must refuse, not silently repair.
        let params = decode_params(8);
        let res = MoeResolution::resolve_arch(&params.dtypes, 8, false);
        let (gu, dn) = decode_expert_refs(&params);
        let mut phases = match classify_decode_lane(
        ...
        let err = match qwen35_decode_phase_buckets(phases) {
            Err(err) => err,
            Ok(_) => panic!("backward-permuted supported steps must be refused"),
        };
        match err {
            Qwen35StepBucketError::ReorderedStep {
                lane,
    ...
    fn qwen35_bucketers_label_exhaustive_unsupported_variants() {
        ...
        // `Step` variant — no wildcard fallback. A dense residual-add
        // injected into the decode routed phase carries its exact label.
        let params = decode_params(8);
        let res = MoeResolution::resolve_arch(&params.dtypes, 8, false);
        let (gu, dn) = decode_expert_refs(&params);
        let mut phases = match classify_decode_lane(
