        ...
        match (&weights.layers[layer_idx], config.layer_types[layer_idx]) {
            ...
            (LayerWeights::DeltaNetMoe(layer), LayerType::LinearAttention) => {
                ...
                let mtp_conv_qki_b4 = *MTP_CONV_QKI_B4.get_or_init(|| {
                    hipfire_config::developer_var("HIPFIRE_MTP_CONV_QKI_B4")
                        .ok()
                        .as_deref()
                        == Some("1")
                }) && gpu.arch_caps.is_gfx1201()
                    && n == 4
                    && batch_semantics.is_sequential()
                    && !mtp_golden_gdn_compact2
                ...
                let fused_mtp_qk_interleave = *FUSED_MTP_QK_INTERLEAVE.get_or_init(|| {
                    hipfire_config::developer_var("HIPFIRE_MTP_FUSED_QK_INTERLEAVE")
                        .ok()
                        .as_deref()
                        == Some("1")
                });
