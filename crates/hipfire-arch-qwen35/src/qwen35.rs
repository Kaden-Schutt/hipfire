...
fn load_weight_tensor_from_slabs(
    ...
    Some((entry_name.clone(), WeightTensor {
...
pub fn load_weights(hfq: &mut HfqFile, config: &Qwen35Config, gpu: &mut Gpu) -> HipResult<Qwen35Weights> {
    ...
    let (token_embd, embd_fmt) = if let Some((qt, tensor)) = load_gpu_tensor_from_slabs(slabs, "embed_tokens.weight") {
        match qt {
            6 => (tensor, EmbeddingFormat::HFQ4G256),
            7 => (tensor, EmbeddingFormat::HFQ4G128),
            3 => (tensor, EmbeddingFormat::Q8_0),
            _ => {
                let (embd_meta, embd_data) = qwen35_tensor_data_vec(hfq, "embed_tokens.weight").expect("embed_tokens not found");
        ...
        let (_, embd_data) = qwen35_tensor_data_vec(hfq, "embed_tokens.weight").expect("embed_tokens not found");
        loaded_bytes += embd_data.len();
