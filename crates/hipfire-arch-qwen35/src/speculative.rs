...
// Seam types relocated to the arch-agnostic `hipfire-specdecode` core (P1);
// re-exported so internal references and the dflash/mtp demo examples resolve
// unchanged.
...
use hipfire_specdecode::VerifyGraphPolicy;
use hipfire_specdecode::{
    argmax_u32, f32_diff_stats, first_mismatched_f32, logit_diff_stats, sample_residual,
    softmax_temp_into, F32DiffStats,
};
pub use hipfire_specdecode::{
    read_ddtree_meta_stats, record_ddtree_meta_nodes, reset_ddtree_meta_stats,
    spec_rollback_parity_decision, spec_rollback_parity_decision_for_step, DdtreeMetaStats, KvMode,
    SpecRollbackParityDecision, SpecRollbackReplayKind, SpecStats, SpecStepResult,
    SpecVerifyGraphMode,
};
use std::path::Path;
