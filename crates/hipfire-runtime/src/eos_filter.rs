...
impl EosFilter {
    ...
    /// Terminal flush alias for the stream-parser driver: drain any bytes
    /// held back due to UTF-8 boundary / marker-prefix buffering, excluding
    /// open `<think>` content. Identical to [`flush_pending`](Self::flush_pending);
    /// kept as a distinct name because the `StreamParser` trait calls it
    /// `finish`.
