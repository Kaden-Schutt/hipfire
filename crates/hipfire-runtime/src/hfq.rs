...
/// BF16 (bfloat16) → f32: bf16 is the high 16 bits of an IEEE-754 f32,
/// so widening is a left shift by 16 — no table, no rounding.
#[inline]
fn bf16_to_f32(bits: u16) -> f32 {
    f32::from_bits((bits as u32) << 16)
}

/// Decode raw safetensors FP bytes to f32, dispatching on the dtype
/// string. Handles the dense FP dtypes safetensors models ship weights,
/// norms, and embeddings in: F16, BF16, F32.
fn decode_fp_to_f32(dtype: &str, data: &[u8]) -> Vec<f32> {
    match dtype {
        "BF16" => data
            .chunks_exact(2)
        ...
        "F32" => data
            ...
            .map(|c| f32::from_le_bytes([c[0], c[1], c[2], c[3]]))
            .collect(),
        // F16 (and any unspecified 2-byte half) — existing default.
        _ => data
            .chunks_exact(2)
            .map(|c| f16_to_f32(u16::from_le_bytes([c[0], c[1]])))
            .collect(),
...
fn load_fp16_weight_tensor_from_source(
    ...
    let f32_data: Vec<f32> = decode_fp_to_f32(&info.dtype, data);
    let bytes: &[u8] =
...
fn paro_load_llama_norm_raw(
    ...
    let v: Vec<f32> = match info.dtype.as_str() {
        "F16" => data
            .chunks_exact(2)
            .map(|c| f16_to_f32(u16::from_le_bytes([c[0], c[1]])))
            .collect(),
...
pub fn load_weights_paroquant_llama(
    ...
    let output = if source.tensor_info("lm_head.weight").is_some() {
        ...
        let f: Vec<f32> = decode_fp_to_f32(&info.dtype, td);
        let bytes: &[u8] =
