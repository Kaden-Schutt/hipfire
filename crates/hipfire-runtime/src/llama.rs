...
// Canonical CPU half<->f32 conversions now live in `rdna_compute::fp16`.
// Re-exported here so the public `hipfire_runtime::llama::{f16_to_f32,
// f32_to_f16}` API (used by tooling/examples) stays stable.
pub use rdna_compute::{bf16_to_f32, f16_to_f32, f32_to_f16};
