...
mod sp_dummy_prefix_tests {
    ...
    fn gemma4_fixture_metadata() -> String {
        ...
        serde_json::json!({
            ...
            // gemma4's quantizer embeds generation_config; bos=2 (<bos>),
            // eos=1 (<eos>) — the from_hf_json vocab heuristic alone would
            // guess bos=1 (no <s>/<|endoftext|> in the vocab).
    ...
    fn gemma4_no_dummy_prefix_first_word_matches_hf() {
        // HF ground truth (tokenizers 0.22.2 on the real gemma4-12b-qat
        // tokenizer.json): encode("The capital of France is",
        // add_special_tokens=True) = [2, 818, 5279, 529, 7001, 563].
        // The pre-fix encoder produced [2, 669, …] — first word shifted
        // onto "▁The".
    ...
    fn gemma4_chat_tail_thought_channel_matches_hf() {
        // The daemon chat path renders `…<|turn>model\n<|channel>thought\n
        // <channel|>` and encodes the whole string; the segments between
        // special tokens must also encode WITHOUT the dummy prefix.
        // HF ground truth: [100, 45518, 107, 101, 818, 5279, 529, 7001, 563]
        // ("thought" = 45518, NOT "▁thought" = 3305; answer prefix "The…"
        // unshifted). This pins the chat-path expectation from the hiptrx
        // validation (first-5 ids 100,45518,107,101,818).
        let t = Tokenizer::from_hfq_metadata(&gemma4_fixture_metadata())
            .expect("fixture parses");
        assert_eq!(
    ...
    fn llama_style_prepend_normalizer_keeps_dummy_prefix() {
        // LLaMA-2-style tokenizer.json: `Sequence[Prepend("▁"), Replace]`.
        // The config-driven flag must keep the dummy prefix ON — the
        // gemma4 fix must NOT regress SP models that genuinely use it.
        let tok_json = serde_json::json!({
            "normalizer": {
                "type": "Sequence",
                "normalizers": [
    ...
    fn detection_signals() {
        ...
        // Metaspace pre_tokenizer prepend_scheme variants.
        ...
        // Legacy Metaspace (T5-style Precompiled charsmap + Metaspace):
        // `add_prefix_space` bool, default true when absent.
        assert!(sp_dummy_prefix_from_hf_json(&json!({
        ...
        // Nested: Metaspace inside a pre_tokenizer Sequence.
        assert!(sp_dummy_prefix_from_hf_json(&json!({
    ...
    fn gguf_add_space_prefix_default_on_and_explicit_off() {
        use serde_json::json;
        // GGUF SP (llama.cpp SPM convention): absent flag → prefix ON —
        // the pre-existing behavior every GGUF llama consumer relies on.
        let meta = json!({
        ...
        // Explicit opt-out via tokenizer.ggml.add_space_prefix=false.
        let meta = json!({
            "tokenizer.ggml.tokens": ["▁hello", "▁world", "hello"],
            "tokenizer.ggml.model": "llama",
