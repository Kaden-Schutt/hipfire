...
fn system_snapshot() -> SystemSnapshot {
    ...
    if let Ok(s) = std::fs::read_to_string("/sys/class/drm/card1/device/hwmon/hwmon2/temp1_input") {
        ...
        std::fs::read_to_string("/sys/class/drm/card1/device/hwmon/hwmon1/temp1_input")
    {
        snap.gpu_temp_c = s.trim().parse::<f64>().unwrap_or(0.0) / 1000.0;
    }
    if let Ok(s) =
        std::fs::read_to_string("/sys/class/drm/card1/device/hwmon/hwmon2/power1_average")
    {
        snap.gpu_power_w = s.trim().parse::<f64>().unwrap_or(0.0) / 1_000_000.0;
    } else if let Ok(s) =
        std::fs::read_to_string("/sys/class/drm/card1/device/hwmon/hwmon1/power1_average")
    {
        snap.gpu_power_w = s.trim().parse::<f64>().unwrap_or(0.0) / 1_000_000.0;
    }
...
fn main() {
    ...
    let model_path = args
        .get(1)
        .expect("Usage: profile_layers <model.hfq> [n_tokens]");
    ...
    eprintln!(
        "Prompt processed ({} tokens), now profiling {} generation tokens...",
        prompt_tokens.len(),
        n_tokens
    );
    ...
    for i in 0..n_tokens {
        ...
        eprintln!(
            "  token {i}: id={next_token} text={text:?} total={:.0}µs",
            timings.total_us
        );
