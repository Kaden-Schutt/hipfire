...
fn main() {
    ...
    let model_path = args.get(1).unwrap_or_else(|| {
        eprintln!("Usage: smoke_llama_prefill_batch <model.hfq>");
        std::process::exit(1);
    });
    ...
    eprintln!(
        "Model: dim={}, layers={}, heads={}, kv_heads={}, head_dim={}",
    ...
    let prompt_text = std::env::args()
        .nth(2)
        .unwrap_or_else(|| "The quick brown fox jumps over the lazy dog. ".repeat(40));
    ...
    for (pos, &tok) in prompt_tokens.iter().enumerate() {
        llama::forward_scratch_embed(&mut gpu, &weights, &config, tok, pos, &scratch)
            .expect("embed");
        llama::forward_scratch_compute(&mut gpu, &weights, &config, pos, &mut kv_a, &scratch)
            .expect("compute");
    }
    ...
    let pbs = PrefillBatchScratch::new(
        &mut gpu,
        &config,
        llama::PREFILL_MAX_BATCH.min(prompt_tokens.len().max(4)),
        kv_seq_len,
    )
    .expect("pbs");
    let t1 = Instant::now();
    ...
    eprintln!(
        "argmax {} (ref={ref_argmax} bat={bat_argmax})",
