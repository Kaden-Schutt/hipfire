...
//!
//! Usage: bench_qwen35_speed <model.hfq> [--prefill <N>] [--prefill-list <N,N,...>] [--prefill-runs <N>] [--gen <N>] [--warmup <N>]
...
fn main() {
    ...
    let prefill_lengths = args.prefill_lengths();
    ...
    let prefill_list = prefill_lengths
        .iter()
        .map(std::string::ToString::to_string)
        .collect::<Vec<_>>()
        .join(", ");
    eprintln!(
        "Phases: prefill=[{prefill_list}] prefill_runs={prefill_runs} warmup={warmup_len} gen={gen_len}"
    );
    ...
    for &prefill_len in prefill_lengths.iter() {
        ...
        let case_label = match prefill_len {
            32 => "pp32_prefill_decode",
            128 => "pp128_prefill_decode",
            _ => "other_prefill_decode",
        };
        ...
        if let Some(ref atlas_path) = args.atlas_out {
            ...
            row.set_metric_f64("prefill_tok_s", prefill_tok_s)
                ...
                .set_metric_str("case_label", case_label)
                .set_extra(
                    "captured_at_unix_s",
            ...
            if let Some(ref report) = prefill_rocprof_report {
                if !report.rocprof.is_empty() {
                    let mut rocprof_sorted: Vec<hipfire_atlas::AtlasRocprofKernel> = report
                        ...
                        .collect();
                    rocprof_sorted
                        .sort_by(|a, b| b.duration_us.partial_cmp(&a.duration_us).unwrap());
        ...
        let do_profile_decode =
            std::env::var("HIPFIRE_PROFILE_DECODE").ok().as_deref() == Some("1");
        ...
        stats.p50_ms,
        speed_bench::split_prefill_summary(prefill_len, prefill_ms, prefill_kernel_ms));
        if case_label != "other_prefill_decode" {
            eprintln!(
            "CASE_SUMMARY label={case_label} prefill_tokens={prefill_len} prefill_tok_s={prefill_tok_s:.1} prefill_wall_ms={prefill_ms:.2} gen_tok_s={gen_tok_s:.1} avg_ms={:.2} p50_ms={:.2}",
            stats.avg_ms,
            stats.p50_ms
        );
        }
