...
fn main() {
    ...
    // Exact K=2048 MQ4V2 MoE gate_up X_BUFFER vs admitted no-LDS route.
    // gfx1100 only; production M=1024 K=2048 k_top=8. Independent canary
    // buffers so unwritten/shared outputs fail. Direct exact wrappers so the
    // oracle does not depend on HIPFIRE_GFX1100_MQ4V2_GATE_UP_X_BUFFER.
    {
        const M: usize = 1024;
        const K: usize = 2_048;
        const K_TOP: usize = 8;
        ...
        if !gpu.arch_caps.is_gfx1100() {
            eprintln!(
                "gate_up_k2048_x_buffer_vs_nolds: skip (arch={} not gfx1100)",
                gpu.arch
            );
        } else {
            let (_, d_a) = pack_upload_mq4v2(&mut gpu, M, K, 0xD048_6A7E);
            let expert_ptr = d_a.buf.as_ptr() as u64;
            let mut ptr_bytes = [0u8; 8];
            ptr_bytes.copy_from_slice(&expert_ptr.to_le_bytes());
            let d_ptrs = gpu.upload_raw(&ptr_bytes, &[1]).unwrap();
            let topk = [0i32; K_TOP];
            let topk_bytes: Vec<u8> = topk.iter().flat_map(|v| v.to_le_bytes()).collect();
            let d_topk = gpu.upload_raw(&topk_bytes, &[K_TOP]).unwrap();
            let x: Vec<f32> = (0..K).map(|i| prng(i, 0xC0FF_D048) * 2.0 - 1.0).collect();
            let d_x = gpu.upload_f32(&x, &[K]).unwrap();

            let (d_ref_g, y_g_len) = upload_y_canary(&mut gpu, K_TOP, MI);
            let (d_ref_u, y_u_len) = upload_y_canary(&mut gpu, K_TOP, MI);
            gpu.gemv_mq4g256v2_moe_gate_up_k8_indexed_k2048_nolds_gfx1100_exact(
                &d_ptrs, &d_topk, &d_x, &d_ref_g, &d_ref_u, M, K,
            )
            .unwrap_or_else(|e| panic!("nolds gate_up K=2048 exact launch: {e}"));
            gpu.hip.device_synchronize().unwrap();

            let (d_cand_g, cand_g_len) = upload_y_canary(&mut gpu, K_TOP, MI);
            let (d_cand_u, cand_u_len) = upload_y_canary(&mut gpu, K_TOP, MI);
            assert_eq!(cand_g_len, y_g_len);
            assert_eq!(cand_u_len, y_u_len);
            gpu.gemv_mq4g256v2_moe_gate_up_k8_indexed_k2048_x_buffer_gfx1100_exact(
                &d_ptrs, &d_topk, &d_x, &d_cand_g, &d_cand_u, M, K,
            )
            .unwrap_or_else(|e| panic!("x_buffer gate_up K=2048 exact launch: {e}"));
            gpu.hip.device_synchronize().unwrap();

            let parts = ["y_gate", "y_up"];
            let refs = [&d_ref_g, &d_ref_u];
            let cands = [&d_cand_g, &d_cand_u];
            let lens = [y_g_len, y_u_len];
            let mut all_exact = true;
            let mut all_canaries = true;
            let mut nondegenerate = true;
            let mut detail = String::new();
            for i in 0..2 {
                let (reference, ref_canary) = download_y_canary(&gpu, refs[i], lens[i], K_TOP, MI);
                let (candidate, cand_canary) =
                    download_y_canary(&gpu, cands[i], lens[i], K_TOP, MI);
                let exact = reference
                    .iter()
                    .zip(&candidate)
                    .all(|(a, b)| a.to_bits() == b.to_bits());
                ...
                if !exact {
                    failures.push(format!(
                        "gate_up_k2048_x_buffer_vs_nolds/{} bit mismatch rms {rms:.3e}",
                        parts[i]
                    ));
                }
                if !canary {
                    failures.push(format!(
                        "gate_up_k2048_x_buffer_vs_nolds/{} canary clobbered",
                        parts[i]
                    ));
                }
                if !varied {
                    failures.push(format!(
                        "gate_up_k2048_x_buffer_vs_nolds/{} degenerate output",
                        parts[i]
                    ));
                }
            }
            ...
            eprintln!(
                "  gate_up_k2048_x_buffer_vs_nolds M={M} K={K} k_top={K_TOP}:{detail} {status}"
            );
        }
    }

    if failures.is_empty() {
        eprintln!("\nmq4v2_fused_parity: PASS — all fused families half-select/pointer-swap/canary correct at K=256/512, N=16/64; scalar fused_qkv / fused_qkvza / fused_gate_up N=1 match standalone gemv_mq4g256v2; residual_sigmoid_scaled_k512 bit-exact vs reference on gfx1100|gfx1201; K2048 hoist_x32 bit-exact vs generic on gfx1100; K2048 qkv x_buffer bit-exact vs generic on gfx1100; K2048 gate_up x_buffer bit-exact vs nolds on gfx1100");
    } else {
