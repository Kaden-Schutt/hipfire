...
fn main() {
    ...
    // Exact K=2048 MQ4V2 QKVZA LDSX8 vs HOIST_X32 at production dims.
    // gfx1100 only; Ms=(qkv,z,beta,alpha)=(256,1,512,512). Independent
    // weight salts exercise both half headers; independent nontrivial-seeded
    // output buffers + tail canaries; bit-exact outputs; inputs intact.
    {
        const K: usize = 2_048;
        if !gpu.arch_caps.is_gfx1100() {
            eprintln!(
                "qkvza_k2048_ldsx8_vs_hoist: skip (arch={} not gfx1100)",
                gpu.arch
            );
        } else {
            let ms = [256usize, 1, 512, 512];
            let salts = [0xD048_0001u32, 0xD048_0002, 0xD048_0003, 0xD048_0004];
            let y_salts = [0xE048_1001u32, 0xE048_1002, 0xE048_1003, 0xE048_1004];
            let parts = ["qkv", "z", "beta", "alpha"];
            let x: Vec<f32> = (0..K).map(|i| prng(i, 0xC0FF_D048) * 2.0 - 1.0).collect();
            let d_x = gpu.upload_f32(&x, &[K]).unwrap();

            let mut host_blobs = Vec::with_capacity(4);
            let mut d_as = Vec::with_capacity(4);
            for i in 0..4 {
                let (blob, d_a) = pack_upload_mq4v2(&mut gpu, ms[i], K, salts[i]);
                host_blobs.push(blob);
                d_as.push(d_a);
            }

            // HoistX32 reference — nontrivial seed + trailing canary per Y.
            let mut d_refs = Vec::with_capacity(4);
            let mut y_lens = Vec::with_capacity(4);
            for i in 0..4 {
                let y_len = n1 * ms[i] + 1;
                let mut y_host = vec![0.0f32; y_len];
                for j in 0..(n1 * ms[i]) {
                    y_host[j] = prng(j, y_salts[i]) * 2.0 - 1.0;
                }
                y_host[y_len - 1] = CANARY_F32;
                let d_ref = gpu.upload_f32(&y_host, &[y_len]).unwrap();
                d_refs.push(d_ref);
                y_lens.push(y_len);
            }
            gpu.fused_qkvza_hfq4g256_mq4v2_k2048_hoist_x32_exact(
            ...
            .unwrap_or_else(|e| panic!("hoist_x32 qkvza K=2048 prod launch: {e}"));
            gpu.hip.device_synchronize().unwrap();

            // LDSX8 candidate — independent nontrivial-seeded canary buffers.
            let mut d_cands = Vec::with_capacity(4);
            for i in 0..4 {
                let y_len = y_lens[i];
                let mut y_host = vec![0.0f32; y_len];
                for j in 0..(n1 * ms[i]) {
                    // Distinct seed from the reference path so a no-op write fails.
                    y_host[j] = prng(j, y_salts[i] ^ 0x5A5A_5A5A) * 2.0 - 1.0;
                }
                y_host[y_len - 1] = CANARY_F32;
                let d_cand = gpu.upload_f32(&y_host, &[y_len]).unwrap();
                d_cands.push(d_cand);
            }
            gpu.fused_qkvza_hfq4g256_mq4v2_k2048_ldsx8_gfx1100_exact(
                &d_as[0],
                &d_as[1],
                &d_as[2],
                &d_as[3],
            ...
            .unwrap_or_else(|e| panic!("ldsx8 qkvza K=2048 prod launch: {e}"));
            gpu.hip.device_synchronize().unwrap();

            // Inputs must survive both launches (x + four weight blobs).
            let got_x = gpu.download_f32(&d_x).unwrap();
            let x_intact = got_x
                .iter()
                .zip(x.iter())
                .all(|(a, b)| a.to_bits() == b.to_bits());
            let mut weights_intact = true;
            for i in 0..4 {
                let mut got = vec![0u8; host_blobs[i].len()];
                gpu.hip
                    .memcpy_dtoh(&mut got, &d_as[i].buf)
                    .unwrap_or_else(|e| panic!("weight download {}: {e}", parts[i]));
                if got.as_slice() != host_blobs[i].as_slice() {
                    weights_intact = false;
                    failures.push(format!(
                        "qkvza_k2048_ldsx8_vs_hoist/{} weight input mutated",
                        parts[i]
                    ));
                }
            }
            if !x_intact {
                failures.push("qkvza_k2048_ldsx8_vs_hoist x input mutated".into());
            }
            ...
            for i in 0..4 {
                ...
                let varied = if ms[i] <= 1 {
                    reference.iter().any(|value| value.abs() > 1.0e-6)
                } else {
                    reference.iter().any(|value| value.abs() > 1.0e-6)
                        && reference
                            .iter()
                ...
                if !exact {
                    failures.push(format!(
                        "qkvza_k2048_ldsx8_vs_hoist/{} bit mismatch rms {rms:.3e}",
                        parts[i]
                    ));
                }
                if !canary {
                    failures.push(format!(
                        "qkvza_k2048_ldsx8_vs_hoist/{} canary clobbered",
                        parts[i]
                    ));
                }
                if !varied {
                    failures.push(format!(
                        "qkvza_k2048_ldsx8_vs_hoist/{} degenerate output",
                        parts[i]
                    ));
                }
            }
            let inputs_ok = x_intact && weights_intact;
            let status = if all_exact && all_canaries && nondegenerate && inputs_ok {
                "ok"
            } else {
                "FAIL"
            };
            eprintln!(
                "  qkvza_k2048_ldsx8_vs_hoist K={K} N={n1} Ms=({},{},{},{}):{detail} inputs_ok={inputs_ok} {status}",
                ms[0], ms[1], ms[2], ms[3]
            );
        }
    }

    // Exact K=2048 MQ4V2 QKVZA X_BUFFER vs HOIST_X32 at scalar dims.
    // gfx1100 only; Ms=(40,24,16,8). Independent weight salts; independent
    // nontrivial-seeded output buffers + tail canaries; bit-exact outputs;
    // inputs intact.
    {
        const K: usize = 2_048;
        if !gpu.arch_caps.is_gfx1100() {
            eprintln!(
                "qkvza_k2048_x_buffer_vs_hoist: skip (arch={} not gfx1100)",
                gpu.arch
            );
        } else {
            let ms = [40usize, 24, 16, 8];
            let salts = [0xF048_0001u32, 0xF048_0002, 0xF048_0003, 0xF048_0004];
            let y_salts = [0xF148_1001u32, 0xF148_1002, 0xF148_1003, 0xF148_1004];
            let parts = ["qkv", "z", "beta", "alpha"];
            let x: Vec<f32> = (0..K).map(|i| prng(i, 0xC0FF_F048) * 2.0 - 1.0).collect();
            let d_x = gpu.upload_f32(&x, &[K]).unwrap();
            ...
            .unwrap_or_else(|e| panic!("hoist_x32 qkvza K=2048 scalar launch: {e}"));
            gpu.hip.device_synchronize().unwrap();

            // X_BUFFER candidate — independent nontrivial-seeded canary buffers.
            let mut d_cands = Vec::with_capacity(4);
            for i in 0..4 {
                let y_len = y_lens[i];
                let mut y_host = vec![0.0f32; y_len];
            ...
            gpu.fused_qkvza_hfq4g256_mq4v2_k2048_x_buffer_gfx1100_exact(
                &d_as[0],
                &d_as[1],
                &d_as[2],
                &d_as[3],
            ...
            .unwrap_or_else(|e| panic!("x_buffer qkvza K=2048 scalar launch: {e}"));
            gpu.hip.device_synchronize().unwrap();
            ...
            for i in 0..4 {
                ...
                if got.as_slice() != host_blobs[i].as_slice() {
                    weights_intact = false;
                    failures.push(format!(
                        "qkvza_k2048_x_buffer_vs_hoist/{} weight input mutated",
                        parts[i]
                    ));
                }
            }
            if !x_intact {
                failures.push("qkvza_k2048_x_buffer_vs_hoist x input mutated".into());
            }
            ...
            for i in 0..4 {
                ...
                if !exact {
                    failures.push(format!(
                        "qkvza_k2048_x_buffer_vs_hoist/{} bit mismatch rms {rms:.3e}",
                        parts[i]
                    ));
                }
                if !canary {
                    failures.push(format!(
                        "qkvza_k2048_x_buffer_vs_hoist/{} canary clobbered",
                        parts[i]
                    ));
                }
                if !varied {
                    failures.push(format!(
                        "qkvza_k2048_x_buffer_vs_hoist/{} degenerate output",
                        parts[i]
                    ));
                }
            }
            ...
            eprintln!(
                "  qkvza_k2048_x_buffer_vs_hoist K={K} N={n1} Ms=({},{},{},{}):{detail} inputs_ok={inputs_ok} {status}",
                ms[0], ms[1], ms[2], ms[3]
            );
        }
    }
    ...
    if failures.is_empty() {
        eprintln!("\nmq4v2_fused_parity: PASS — all fused families half-select/pointer-swap/canary correct at K=256/512, N=16/64; scalar fused_qkv / fused_qkvza / fused_gate_up N=1 match standalone gemv_mq4g256v2; residual_sigmoid_scaled_k512 bit-exact vs reference on gfx1100|gfx1201; K2048 hoist_x32 bit-exact vs generic on gfx1100; K2048 qkv x_buffer bit-exact vs generic on gfx1100; qkvza_k2048_ldsx8_vs_hoist bit-exact on gfx1100; qkvza_k2048_x_buffer_vs_hoist bit-exact on gfx1100; ninepath_rpb8_vs_rpb16 bit-exact on gfx1100");
    } else {
