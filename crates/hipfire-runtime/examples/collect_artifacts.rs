...
fn main() {
    ...
    let output = arg("--output", None).unwrap_or_else(|| {
        eprintln!("--output <path.calib.hfq> required (no default; artifact is ~30 GB for a dense 27B)");
        print_usage_and_exit(2);
    });
    ...
    let mut hfq = HfqFile::open(Path::new(&model)).unwrap_or_else(|e| panic!("open model {}: {e}", model));
    ...
    let peak_group_bytes = estimate_peak_group_bytes(&config, &weights, 0, layers_per_pass.min(config.n_layers));
    eprintln!(
...
fn run_calib_sequences(
    ...
    eprintln!("output: {output}  size: {out_bytes} bytes  elapsed: {:.1}s", elapsed);
    eprintln!(
    ...
    eprintln!("config: n_layers={} dim={} hidden_dim={} head_dim={} layers_per_pass={}", config.n_layers, config.dim, config.hidden_dim, config.head_dim, layers_per_pass);
