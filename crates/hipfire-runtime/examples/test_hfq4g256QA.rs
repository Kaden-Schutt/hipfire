...
fn run() -> Result<String, Outcome> {
    ...
    let d_a = gpu
        .upload_raw(&quantized, &[quantized.len()])
        .map_err(|e| Outcome::Fail(format!("upload quantized failed: {e}")))?;
    let d_x = gpu
        .upload_f32(&x, &[k])
    ...
    gpu.gemv_hfq4g256(&d_a, &d_x, &d_y, m, k)
        .map_err(|e| Outcome::Fail(format!("gemv_hfq4g256 failed: {e}")))?;
    let y_gpu = gpu
        .download_f32(&d_y)
    ...
    if max_gpu_cpu_err > 0.05 {
        return Err(Outcome::Fail(format!(
            "GPU vs CPU dequant mismatch too large: {max_gpu_cpu_err:.6}"
        )));
    }
    ...
    let emb = gpu
        .download_f32(&d_out)
        .map_err(|e| Outcome::Fail(format!("download embedding failed: {e}")))?;
    if !emb[0].is_finite() {
        return Err(Outcome::Fail(format!(
            "embedding output non-finite: {}",
            emb[0]
        )));
    }
    ...
    let mmq_result = if supports_mmq_i8_wmma(&gpu.arch) {
            ...
            for row in 0..mmq_m {
                ...
                for col in 0..mmq_k {
                    ...
                    let nibble = if col % 2 == 0 {
                        mmq_q[off + 8 + byte_idx] & 0xF
                    } else {
                        mmq_q[off + 8 + byte_idx] >> 4
                    };
        ...
        let d_mmq_a = gpu
            .upload_raw(&mmq_q, &[mmq_q.len()])
            .map_err(|e| Outcome::Fail(format!("upload mmq A failed: {e}")))?;
        let d_mmq_x = gpu
        ...
        let mmq_gpu = gpu
            .download_f32(&d_mmq_y)
            .map_err(|e| Outcome::Fail(format!("download mmq Y failed: {e}")))?;
        let max_mmq_err = mmq_gpu
            .iter()
            .zip(&mmq_ref)
            .map(|(a, b)| (a - b).abs())
            .fold(0.0f32, f32::max);
        if max_mmq_err > 0.5 {
            return Err(Outcome::Fail(format!(
                "MMQ residual mismatch too large: {max_mmq_err:.6} gpu0={} ref0={}",
                mmq_gpu[0], mmq_ref[0]
            )));
        }
        gpu.free_tensor(d_mmq_a)
            .map_err(|e| Outcome::Fail(format!("free mmq A failed: {e}")))?;
        gpu.free_tensor(d_mmq_x)
            .map_err(|e| Outcome::Fail(format!("free mmq X failed: {e}")))?;
        gpu.free_tensor(d_mmq_y)
            .map_err(|e| Outcome::Fail(format!("free mmq Y failed: {e}")))?;
        format!("mmq_err={max_mmq_err:.6}")
    } else {
        format!("mmq_skipped_arch={}", gpu.arch)
    };

    let max_ref_err = y_cpu_dequant
        .iter()
        .zip(&y_ref)
        .map(|(a, b)| (a - b).abs())
        .fold(0.0f32, f32::max);
    Ok(format!(
        "gpu_cpu_err={max_gpu_cpu_err:.6} quant_ref_err={max_ref_err:.6} {mmq_result}"
    ))
}
