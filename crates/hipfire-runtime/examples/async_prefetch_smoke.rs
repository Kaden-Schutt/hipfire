...
fn main() {
    ...
    println!(
        "async_prefetch_smoke: issued {} ranges ({} MiB)",
        reqs.len(),
        N * 3 >> 20
    );
    ...
    gpu.hip.memcpy_dtoh(&mut got, &dst.buf).expect("readback");
    ...
    println!(
        "async_prefetch_smoke: {} MiB byte-exact after event wait",
        N * 3 >> 20
    );
    ...
    t.prefetch_batch_into(&reqs2, &dst, &mut gpu)
        .expect("reissue");
    ...
    println!(
        "  prefetch blocked for {:.2} ms total",
        t.prefetch_wait_ns() as f64 / 1e6
    );
