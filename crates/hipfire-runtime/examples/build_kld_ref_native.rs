...
fn main() {
    ...
    fn write_scored_position(
        ...
        let cmp_desc = |a: &(u32, f32), b: &(u32, f32)| b.1.partial_cmp(&a.1).unwrap_or(Ordering::Equal);
        if k < log_probs.len() {
        ...
        let top_p_sum: f64 = log_probs[..kk].iter().map(|&(_, lp)| (lp as f64).exp()).sum();
    ...
    if is_qwen35 {
        ...
        for c in 0..n_chunk {
            ...
            for pos in 0..(n_ctx - 1) {
                ...
                write_scored_position(&cand_logits, actual_next, k, &mut out, &mut log_probs, &mut nll_sum, &mut nll_count);
        ...
        let weights = hipfire_runtime::hfq::load_weights_hfq(&hfq, &cfg, &mut gpu).expect("llama load_weights");
        eprintln!(
        ...
        out.write_all(&HIPFIRE_VERSION.to_le_bytes()).unwrap();
        out.write_all(&(n_ctx as u32).to_le_bytes()).unwrap();
        out.write_all(&(cfg.vocab_size as u32).to_le_bytes()).unwrap();
        out.write_all(&(n_chunk as u32).to_le_bytes()).unwrap();
        out.write_all(&(top_k as u16).to_le_bytes()).unwrap();
        ...
        let kv_max = n_ctx + 16;
        let mut kv_cache = KvCache::new_gpu(&mut gpu, cfg.n_layers, cfg.n_kv_heads, cfg.head_dim, kv_max).expect("llama kv");
        let scratch = ForwardScratch::new_with_max_seq(&mut gpu, &cfg, kv_max).expect("llama scratch");
        ...
        for c in 0..n_chunk {
            ...
            for pos in 0..(n_ctx - 1) {
                ...
                hipfire_runtime::llama::forward_scratch(&mut gpu, &weights, &cfg, chunk[pos], pos, &mut kv_cache, &scratch, 0.0, 0.0, 0, 0, 0.0).expect("llama forward_scratch");
                if pos < scoring_start {
                    continue;
                }
                let cand_logits = gpu.download_f32(&scratch.logits).expect("download logits");
        ...
        lowered::init_scratch_constants(&mut gpu, &scratch, cfg.full_head_dim).expect("gemma4 init_scratch_constants");
        let mut kv_sliding = KvCache::new_gpu_q8(&mut gpu, cfg.n_layers, cfg.sliding_n_kv_heads, cfg.sliding_head_dim, kv_max).expect("gemma sliding q8 kv");
        let mut kv_full = KvCache::new_gpu_q8(&mut gpu, cfg.n_layers, cfg.full_n_kv_heads, cfg.full_head_dim, kv_max).expect("gemma full q8 kv");
        ...
        for c in 0..n_chunk {
            kv_sliding.clear_gpu(&mut gpu).expect("gemma kv sliding clear");
            ...
            for pos in 0..(n_ctx - 1) {
                lowered::forward_scratch(&mut gpu, &weights, &cfg, chunk[pos], pos, &mut kv_sliding, &mut kv_full, &scratch).expect("gemma forward_scratch");
                if pos < scoring_start {
                    continue;
                }
                let cand_logits = gpu.download_f32(&scratch.logits).expect("download logits");
        ...
        let kv_max = n_ctx + 16;
        let mut state = GlimmerState::new_with_max_seq(&mut gpu, &cfg, kv_max).expect("glimmer state");
        ...
        for c in 0..n_chunk {
            ...
            for pos in 0..(n_ctx - 1) {
                let cand_logits = forward::decode_step(&cfg, &weights, &mut state, &mut gpu, chunk[pos], pos as u32).expect("glimmer decode_step");
                if pos < scoring_start {
                    continue;
                }
                let actual_next = chunk[pos + 1] as usize;
        ...
        let kv_max = n_ctx + 16;
        let max_batch = n_ctx;
        let mut state = Lfm2DecodeBatchState::new(&mut gpu, &cfg, max_batch, kv_max, 32).expect("lfm2 batch state");
        ...
        for c in 0..n_chunk {
            ...
            forward_decode_batch_lfm(&mut gpu, &weights, &cfg, toks, &positions, &mut state).expect("lfm2 forward_decode_batch_lfm");
            ...
            let all_logits = gpu.download_f32(&state.logits).expect("lfm2 download logits");
            let vocab = cfg.vocab_size;
            for pos in scoring_start..b {
                let row_start = pos * vocab;
                let row_end = row_start + vocab;
                if row_end > all_logits.len() {
                    eprintln!("lfm2 logits buffer too small: pos={} vocab={} len={}", pos, vocab, all_logits.len());
                    std::process::exit(1);
                }
