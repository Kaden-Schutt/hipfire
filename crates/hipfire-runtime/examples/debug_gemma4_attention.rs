...
fn main() {
    ...
    let config = <Gemma4 as Architecture>::config_from_hfq(&hfq).expect("failed config");
    let weights = <Gemma4 as Architecture>::load_weights(&mut hfq, &config, &mut gpu).expect("failed weights");
    let mut scratch = <Gemma4 as Architecture>::new_state(&mut gpu, &config).expect("failed scratch");
    ...
    gemma4::forward_scratch(
        &mut gpu, &weights, &config, 2, 0, &mut kv_sliding, &mut kv_full, &mut scratch,
    ).expect("BOS forward failed");
    
    println!("Running Layer 0 forward with 'Hello' (9259)...");
    gemma4::forward_scratch(
        &mut gpu, &weights, &config, 9259, 1, &mut kv_sliding, &mut kv_full, &mut scratch,
