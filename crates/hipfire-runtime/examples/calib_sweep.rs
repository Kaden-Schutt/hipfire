...
fn arg(flag: &str, default: Option<String>) -> Option<String> {
    ...
    a.iter().position(|x| x == flag).and_then(|i| a.get(i + 1).cloned()).or(default)
}
...
fn corpus_md5(path: &str) -> String {
    ...
    std::fs::metadata(path).map(|m| format!("{:016x}", m.len())).unwrap_or_else(|_| "unknown".into())
}
fn compact_hessian_bytes(k: usize) -> u64 { (k * 4 + k * (k - 1)) as u64 }
...
fn derive_calib_prefix(hfq: &HfqFile) -> String {
        ...
        for suffix in CANDIDATES {
            if name.ends_with(suffix) {
                if let Some(pos) = name.find("layers.") {
                    ...
                    eprintln!("calib prefix derived: '{}' from tensor '{}' (matched suffix '{}')", prefix, name, suffix);
                    return prefix;
                }
            }
        }
    ...
    eprintln!("  HFQ contains {} tensors, first few: {:?}", hfq.tensors().len(), hfq.tensors().iter().take(5).map(|t| t.name.as_str()).collect::<Vec<_>>());
...
fn verify_capture_names_or_exit(hfq: &HfqFile, capture_names: &[String]) {
    ...
    if !missing.is_empty() {
        ...
        eprintln!("  HFQ contains {} tensors, e.g.: {:?}", hfq.tensors().len(), hfq.tensors().iter().take(5).map(|t| t.name.as_str()).collect::<Vec<_>>());
        std::process::exit(1);
    }
}
fn estimate_peak_llama(config: &hipfire_runtime::llama::LlamaConfig, weights: &hipfire_runtime::llama::LlamaWeights, start: usize, end: usize) -> u64 {
    let mut b = 0u64;
...
fn build_capture_llama(weights: &hipfire_runtime::llama::LlamaWeights, start: usize, end: usize, prefix: &str) -> HashMap<usize, String> {
    let mut m = HashMap::new();
    for i in start..end.min(weights.layers.len()) {
        let l = &weights.layers[i];
        let p = format!("{prefix}layers.{i}");
        for (wt, name) in [(&l.wq, format!("{p}.self_attn.q_proj.weight")), (&l.wk, format!("{p}.self_attn.k_proj.weight")), (&l.wv, format!("{p}.self_attn.v_proj.weight")), (&l.wo, format!("{p}.self_attn.o_proj.weight")), (&l.w_gate, format!("{p}.mlp.gate_proj.weight")), (&l.w_up, format!("{p}.mlp.up_proj.weight")), (&l.w_down, format!("{p}.mlp.down_proj.weight"))] {
            m.insert(wt.buf.buf.as_ptr() as usize, name);
        }
    }
    m
}
fn build_capture_gemma(weights: &hipfire_arch_gemma4::lowered::Gemma4Weights, start: usize, end: usize, prefix: &str) -> HashMap<usize, String> {
    let mut m = HashMap::new();
    for i in start..end.min(weights.layers.len()) {
        let p = format!("{prefix}layers.{i}");
        match &weights.layers[i] {
            hipfire_arch_gemma4::lowered::LayerWeights::Sliding(s) => {
                m.insert(s.q_proj.buf.buf.as_ptr() as usize, format!("{p}.self_attn.q_proj.weight"));
                m.insert(s.k_proj.buf.buf.as_ptr() as usize, format!("{p}.self_attn.k_proj.weight"));
                m.insert(s.v_proj.buf.buf.as_ptr() as usize, format!("{p}.self_attn.v_proj.weight"));
                m.insert(s.o_proj.buf.buf.as_ptr() as usize, format!("{p}.self_attn.o_proj.weight"));
                m.insert(s.gate_proj.buf.buf.as_ptr() as usize, format!("{p}.mlp.gate_proj.weight"));
                m.insert(s.up_proj.buf.buf.as_ptr() as usize, format!("{p}.mlp.up_proj.weight"));
                m.insert(s.down_proj.buf.buf.as_ptr() as usize, format!("{p}.mlp.down_proj.weight"));
            }
            hipfire_arch_gemma4::lowered::LayerWeights::Full(f) => {
                m.insert(f.q_proj.buf.buf.as_ptr() as usize, format!("{p}.self_attn.q_proj.weight"));
                m.insert(f.k_proj.buf.buf.as_ptr() as usize, format!("{p}.self_attn.k_proj.weight"));
                // no v_proj on Full — V reuses k_proj pre-norm
                m.insert(f.o_proj.buf.buf.as_ptr() as usize, format!("{p}.self_attn.o_proj.weight"));
                m.insert(f.gate_proj.buf.buf.as_ptr() as usize, format!("{p}.mlp.gate_proj.weight"));
                m.insert(f.up_proj.buf.buf.as_ptr() as usize, format!("{p}.mlp.up_proj.weight"));
                m.insert(f.down_proj.buf.buf.as_ptr() as usize, format!("{p}.mlp.down_proj.weight"));
            }
        }
    }
    m
}
fn build_capture_glimmer(weights: &hipfire_arch_muse_glimmer::glimmer::GlimmerWeights, start: usize, end: usize, prefix: &str) -> HashMap<usize, String> {
    let mut m = HashMap::new();
    for i in start..end.min(weights.layers.len()) {
        let p = format!("{prefix}layers.{i}");
        let l = &weights.layers[i];
        m.insert(l.attn_gate_proj.buf.buf.as_ptr() as usize, format!("{p}.self_attn.gate_proj.weight"));
        m.insert(l.q_proj.buf.buf.as_ptr() as usize, format!("{p}.self_attn.q_proj.weight"));
        m.insert(l.k_proj.buf.buf.as_ptr() as usize, format!("{p}.self_attn.k_proj.weight"));
        m.insert(l.v_proj.buf.buf.as_ptr() as usize, format!("{p}.self_attn.v_proj.weight"));
        m.insert(l.o_proj.buf.buf.as_ptr() as usize, format!("{p}.self_attn.o_proj.weight"));
        m.insert(l.gate_proj.buf.buf.as_ptr() as usize, format!("{p}.mlp.gate_proj.weight"));
        m.insert(l.up_proj.buf.buf.as_ptr() as usize, format!("{p}.mlp.up_proj.weight"));
        m.insert(l.down_proj.buf.buf.as_ptr() as usize, format!("{p}.mlp.down_proj.weight"));
    }
    m
}
fn build_capture_lfm(weights: &hipfire_arch_lfm2moe::lfm2moe::Lfm2MoeWeights, start: usize, end: usize, prefix: &str) -> HashMap<usize, String> {
    let mut m = HashMap::new();
    for i in start..end.min(weights.layers.len()) {
        ...
        match &layer.mixer {
            hipfire_arch_lfm2moe::lfm2moe::Mixer::Conv(c) => {
                m.insert(c.in_proj.buf.buf.as_ptr() as usize, format!("{p}.conv.in_proj.weight"));
                m.insert(c.out_proj.buf.buf.as_ptr() as usize, format!("{p}.conv.out_proj.weight"));
            }
            hipfire_arch_lfm2moe::lfm2moe::Mixer::Attention(a) => {
                m.insert(a.wq.buf.buf.as_ptr() as usize, format!("{p}.self_attn.q_proj.weight"));
                m.insert(a.wk.buf.buf.as_ptr() as usize, format!("{p}.self_attn.k_proj.weight"));
                m.insert(a.wv.buf.buf.as_ptr() as usize, format!("{p}.self_attn.v_proj.weight"));
                // LFM2 names the attention output `out_proj`, not `o_proj`.
                m.insert(a.wo.buf.buf.as_ptr() as usize, format!("{p}.self_attn.out_proj.weight"));
            }
        }
        match &layer.ffn {
            hipfire_arch_lfm2moe::lfm2moe::Ffn::Dense(d) => {
                ...
                m.insert(d.w1.buf.buf.as_ptr() as usize, format!("{p}.feed_forward.w1.weight"));
                m.insert(d.w3.buf.buf.as_ptr() as usize, format!("{p}.feed_forward.w3.weight"));
                m.insert(d.w2.buf.buf.as_ptr() as usize, format!("{p}.feed_forward.w2.weight"));
            }
...
fn estimate_peak_gemma(weights: &hipfire_arch_gemma4::lowered::Gemma4Weights, start: usize, end: usize) -> u64 {
    let mut b = 0u64;
    for i in start..end.min(weights.layers.len()) {
        let ks: Vec<usize> = match &weights.layers[i] {
            hipfire_arch_gemma4::lowered::LayerWeights::Sliding(s) => vec![s.q_proj.k, s.k_proj.k, s.v_proj.k, s.o_proj.k, s.gate_proj.k, s.up_proj.k, s.down_proj.k],
            hipfire_arch_gemma4::lowered::LayerWeights::Full(f) => vec![f.q_proj.k, f.k_proj.k, f.o_proj.k, f.gate_proj.k, f.up_proj.k, f.down_proj.k],
        };
        for k in ks { b += compact_hessian_bytes(k) + (k*4) as u64; }
    }
    b
}
fn estimate_peak_glimmer(weights: &hipfire_arch_muse_glimmer::glimmer::GlimmerWeights, start: usize, end: usize) -> u64 {
    let mut b = 0u64;
    for i in start..end.min(weights.layers.len()) {
        let l = &weights.layers[i];
        for k in [l.attn_gate_proj.k, l.q_proj.k, l.k_proj.k, l.v_proj.k, l.o_proj.k, l.gate_proj.k, l.up_proj.k, l.down_proj.k] {
            b += compact_hessian_bytes(k) + (k*4) as u64;
        }
    }
    b
}
fn estimate_peak_lfm(weights: &hipfire_arch_lfm2moe::lfm2moe::Lfm2MoeWeights, start: usize, end: usize) -> u64 {
    let mut b = 0u64;
    for i in start..end.min(weights.layers.len()) {
        let layer = &weights.layers[i];
        let mut ks = Vec::new();
        match &layer.mixer {
            hipfire_arch_lfm2moe::lfm2moe::Mixer::Conv(c) => { ks.push(c.in_proj.k); ks.push(c.out_proj.k); }
            hipfire_arch_lfm2moe::lfm2moe::Mixer::Attention(a) => { ks.push(a.wq.k); ks.push(a.wk.k); ks.push(a.wv.k); ks.push(a.wo.k); }
        }
        match &layer.ffn {
            hipfire_arch_lfm2moe::lfm2moe::Ffn::Dense(d) => { ks.push(d.w1.k); ks.push(d.w3.k); ks.push(d.w2.k); }
            hipfire_arch_lfm2moe::lfm2moe::Ffn::Moe(_) => {}
        }
        for k in ks { b += compact_hessian_bytes(k) + (k*4) as u64; }
    }
...
fn try_rocblas_syrk(gpu: &mut rdna_compute::Gpu, x: &rdna_compute::GpuTensor, h: &rdna_compute::GpuTensor, k: usize, n: usize) -> Result<(), String> {
    let rb = gpu.rocblas.as_ref().ok_or_else(|| "rocblas_syrk: rocBLAS not initialized".to_string())?;
    if let Some(stream) = gpu.active_stream.as_ref() { rb.set_stream(stream).map_err(|e| format!("rocblas_set_stream syrk: {}", e.context))?; }
    let alpha = 1.0f32; let beta = 1.0f32;
    unsafe {
        rb.gemm_ex(
            hip_bridge::RocblasOperation::Transpose, hip_bridge::RocblasOperation::None,
            k as i32, k as i32, n as i32,
            &alpha as *const f32 as *const std::ffi::c_void,
            x.buf.as_ptr(), hip_bridge::RocblasDatatype::F32, k as i32,
            x.buf.as_ptr(), hip_bridge::RocblasDatatype::F32, k as i32,
            &beta as *const f32 as *const std::ffi::c_void,
            h.buf.as_ptr(), hip_bridge::RocblasDatatype::F32, k as i32,
            h.buf.as_ptr(), hip_bridge::RocblasDatatype::F32, k as i32,
...
fn choose_syrk(gpu: &rdna_compute::Gpu, mode: &str) -> (&'static str, bool) {
    match mode { "rocblas" => ("rocblas", gpu.rocblas.is_some()), "kernel" => ("kernel", true), _ => if gpu.rocblas.is_some() { ("rocblas", true) } else { ("kernel", true) } }
}
...
fn check_identity(output: &Path, n_layers: usize, prefix: &str) -> Result<(), String> {
    let mut hfq = HfqFile::open(output).map_err(|e| format!("open {} identity: {e}", output.display()))?;
    use std::collections::hash_map::DefaultHasher; use std::hash::{Hash, Hasher};
    let mut bad = Vec::new();
    for i in 0..n_layers {
        let chk = |names: &[String], lab: &str| -> Option<String> {
            let mut hs = Vec::new(); let mut miss = Vec::new();
            for n in names { if let Some((_, d)) = hfq.tensor_data(n) { let mut h=DefaultHasher::new(); d.hash(&mut h); hs.push(h.finish()); } else { miss.push(n.clone()); } }
            if !miss.is_empty() { return Some(format!("layer {i} {lab} missing {}", miss.join(","))); }
            if hs.windows(2).any(|w| w[0]!=w[1]) { return Some(format!("layer {i} {lab} diff hashes {:?}", hs)); }
            None
        };
        if let Some(m) = chk(&[format!("{prefix}layers.{i}.self_attn.q_proj.weight.hessian"), format!("{prefix}layers.{i}.self_attn.k_proj.weight.hessian"), format!("{prefix}layers.{i}.self_attn.v_proj.weight.hessian")], "q/k/v") { bad.push(m); }
        if let Some(m) = chk(&[format!("{prefix}layers.{i}.mlp.gate_proj.weight.hessian"), format!("{prefix}layers.{i}.mlp.up_proj.weight.hessian")], "gate/up") { bad.push(m); }
    }
    drop(hfq);
...
fn check_coverage(output: &Path, expected_names: &[String]) -> Result<(), String> {
    ...
    let mut hfq = HfqFile::open(output).map_err(|e| format!("open {} coverage: {e}", output.display()))?;
    ...
    if !missing.is_empty() {
        ...
        let suffix = if list.len() > 20 { format!(" + {} more", list.len() - 20) } else { String::new() };
        return Err(format!("coverage FAIL: {}/{} tensors missing hessian/imatrix: {}{}", missing.len(), total, shown.join(", "), suffix));
    }
    Ok(())
}
fn print_per_proj_summary(output: &Path, expected_names: &[String]) {
    ...
    let mut hfq = match HfqFile::open(output) { Ok(f) => f, Err(_) => return };
    use std::collections::HashMap;
...
fn check_identity_generic(output: &Path, n_layers: usize, is_glimmer: bool, _is_gemma_full: bool, prefix: &str) -> Result<(), String> {
    ...
    let mut hfq = HfqFile::open(output).map_err(|e| format!("open {} identity generic: {e}", output.display()))?;
    use std::collections::hash_map::DefaultHasher; use std::hash::{Hash, Hasher};
    let mut bad = Vec::new();
    for i in 0..n_layers {
        ...
        if !is_conv {
            ...
            if is_glimmer { attn_expected.push(v_name.clone()); attn_expected.push(gate_name.clone()); }
            ...
            for n in &attn_expected {
                if let Some((_, d)) = hfq.tensor_data(n) {
                    let mut h=DefaultHasher::new(); d.hash(&mut h); attn_hs.push(h.finish()); attn_present.push(n.clone());
                } else { attn_missing.push(n.clone()); }
            }
            if !attn_missing.is_empty() {
                bad.push(format!("layer {i} attn q/k/v/gate missing {}", attn_missing.join(",")));
            } else if attn_present.len() >= 2 && attn_hs.windows(2).any(|w| w[0]!=w[1]) {
                bad.push(format!("layer {i} attn q/k/v/gate diff hashes {:?} present {:?}", attn_hs, attn_present));
            }
        }
        // FFN gate/up — REQUIRED for every dense FFN layer. Both must exist or it's FAIL.
        let ffn_names = [format!("{prefix}layers.{i}.mlp.gate_proj.weight.hessian"), format!("{prefix}layers.{i}.mlp.up_proj.weight.hessian")];
        ...
        for n in &ffn_names {
            if let Some((_, d)) = hfq.tensor_data(n) {
                let mut h=DefaultHasher::new(); d.hash(&mut h); fhs.push(h.finish()); fpresent.push(n.clone());
            } else { fmissing.push(n.clone()); }
        }
        if !fmissing.is_empty() {
            if fmissing.len()==2 && !has_down {
                // Both gate/up and down missing => likely MoE layer with no dense capture, skip.
            } else {
                bad.push(format!("layer {i} ffn gate/up missing {}", fmissing.join(",")));
            }
        } else if fhs.windows(2).any(|w| w[0]!=w[1]) {
            bad.push(format!("layer {i} ffn gate/up diff hashes {:?} present {:?}", fhs, fpresent));
        }
    }
    drop(hfq);
    if bad.is_empty() { Ok(()) } else { Err(bad.join("; ")) }
}
fn run_llama_batched(gpu: &mut rdna_compute::Gpu, w: &hipfire_runtime::llama::LlamaWeights, c: &hipfire_runtime::llama::LlamaConfig, toks: &[u32], seq: usize) -> Result<(), String> {
    let seq = seq.max(1);
    let mut kv = hipfire_runtime::llama::KvCache::new_gpu(gpu, c.n_layers, c.n_kv_heads, c.head_dim, seq + 16).map_err(|e| format!("llama kv alloc {seq}: {e}"))?;
    for (si, chunk) in toks.chunks(seq).enumerate() {
        kv.clear_gpu(gpu).map_err(|e| format!("kv clear {si}: {e:?}"))?;
        hipfire_runtime::llama::prefill_forward(gpu, w, c, chunk, &mut kv).map_err(|e| format!("prefill_forward seq {si} B {}: {e:?}", chunk.len()))?;
    }
    Ok(())
}
fn run_gemma_batched(gpu: &mut rdna_compute::Gpu, weights: &hipfire_arch_gemma4::lowered::Gemma4Weights, cfg: &hipfire_arch_gemma4::lowered::Gemma4Config, toks: &[u32], seq: usize, kv_sliding: &mut hipfire_runtime::llama::KvCache, kv_full: &mut hipfire_runtime::llama::KvCache, scratch: &hipfire_arch_gemma4::lowered::Gemma4Scratch) -> Result<(), String> {
    let seq = seq.max(1);
    for (si, chunk) in toks.chunks(seq).enumerate() {
        kv_sliding.clear_gpu(gpu).map_err(|e| format!("gemma kv sliding clear {si}: {e:?}"))?;
        kv_full.clear_gpu(gpu).map_err(|e| format!("gemma kv full clear {si}: {e:?}"))?;
        ...
        while offset < chunk.len() {
            ...
            hipfire_arch_gemma4::lowered::forward_prefill_batch(gpu, weights, cfg, sub, pos, kv_sliding, kv_full, scratch).map_err(|e| format!("gemma forward_prefill_batch seq {si} sub {} B {}: {e:?}", offset, sub.len()))?;
            pos += sub.len();
            offset = end;
        }
    }
...
fn run_glimmer_batched(gpu: &mut rdna_compute::Gpu, weights: &hipfire_arch_muse_glimmer::glimmer::GlimmerWeights, cfg: &hipfire_arch_muse_glimmer::config::GlimmerConfig, state: &mut hipfire_arch_muse_glimmer::glimmer::GlimmerState, toks: &[u32], seq: usize) -> Result<(), String> {
    let seq = seq.max(1);
    for (si, chunk) in toks.chunks(seq).enumerate() {
        state.reset();
        ...
        hipfire_arch_muse_glimmer::forward::prefill_with_capture(cfg, weights, state, gpu, chunk, 0, &[], &mut Vec::new()).map_err(|e| format!("glimmer prefill_with_capture seq {si} B {}: {e}", chunk.len()))?;
    }
    Ok(())
}
fn run_lfm_batched(gpu: &mut rdna_compute::Gpu, weights: &hipfire_arch_lfm2moe::lfm2moe::Lfm2MoeWeights, cfg: &hipfire_arch_lfm2moe::config::Lfm2MoeConfig, state: &mut hipfire_arch_lfm2moe::batch::Lfm2DecodeBatchState, toks: &[u32], seq: usize) -> Result<(), String> {
    let seq = seq.max(1);
    ...
    for (si, chunk) in toks.chunks(seq).enumerate() {
        ...
        state.reset(gpu).map_err(|e| format!("lfm state reset {si}: {e:?}"))?;
        let positions: Vec<usize> = (0..chunk.len()).collect();
        hipfire_arch_lfm2moe::forward_batch::forward_decode_batch_lfm(gpu, weights, cfg, chunk, &positions, state).map_err(|e| format!("lfm forward_decode_batch_lfm seq {si} B {}: {e:?}", chunk.len()))?;
    }
    Ok(())
}
fn build_capture_qwen35(weights: &hipfire_arch_qwen35::qwen35::Qwen35Weights, start: usize, end: usize, prefix: &str) -> HashMap<usize, String> {
    ...
    for i in start..end.min(weights.layers.len()) {
        ...
        match &weights.layers[i] {
            LW::DeltaNet(l) => {
                m.insert(l.wqkv.buf.buf.as_ptr() as usize, format!("{p}.linear_attn.in_proj_qkv.weight"));
                m.insert(l.wz.buf.buf.as_ptr() as usize, format!("{p}.linear_attn.in_proj_z.weight"));
                m.insert(l.w_alpha.buf.buf.as_ptr() as usize, format!("{p}.linear_attn.in_proj_a.weight"));
                m.insert(l.w_beta.buf.buf.as_ptr() as usize, format!("{p}.linear_attn.in_proj_b.weight"));
                m.insert(l.wo.buf.buf.as_ptr() as usize, format!("{p}.linear_attn.out_proj.weight"));
                m.insert(l.w_gate.buf.buf.as_ptr() as usize, format!("{p}.mlp.gate_proj.weight"));
                m.insert(l.w_up.buf.buf.as_ptr() as usize, format!("{p}.mlp.up_proj.weight"));
                m.insert(l.w_down.buf.buf.as_ptr() as usize, format!("{p}.mlp.down_proj.weight"));
            }
            LW::FullAttn(l) => {
                m.insert(l.wq.buf.buf.as_ptr() as usize, format!("{p}.self_attn.q_proj.weight"));
                m.insert(l.wk.buf.buf.as_ptr() as usize, format!("{p}.self_attn.k_proj.weight"));
                m.insert(l.wv.buf.buf.as_ptr() as usize, format!("{p}.self_attn.v_proj.weight"));
                m.insert(l.wo.buf.buf.as_ptr() as usize, format!("{p}.self_attn.o_proj.weight"));
                m.insert(l.w_gate.buf.buf.as_ptr() as usize, format!("{p}.mlp.gate_proj.weight"));
                m.insert(l.w_up.buf.buf.as_ptr() as usize, format!("{p}.mlp.up_proj.weight"));
                m.insert(l.w_down.buf.buf.as_ptr() as usize, format!("{p}.mlp.down_proj.weight"));
            }
...
fn estimate_peak_qwen35(weights: &hipfire_arch_qwen35::qwen35::Qwen35Weights, start: usize, end: usize) -> u64 {
    use hipfire_arch_qwen35::qwen35::LayerWeights as LW;
    let mut b = 0u64;
    for i in start..end.min(weights.layers.len()) {
        let ks: Vec<usize> = match &weights.layers[i] {
            LW::DeltaNet(l) => vec![l.wqkv.k, l.wz.k, l.w_alpha.k, l.w_beta.k, l.wo.k, l.w_gate.k, l.w_up.k, l.w_down.k],
            LW::FullAttn(l) => vec![l.wq.k, l.wk.k, l.wv.k, l.wo.k, l.w_gate.k, l.w_up.k, l.w_down.k],
...
fn check_identity_qwen35(output: &Path, n_layers: usize, prefix: &str) -> Result<(), String> {
    ...
    let mut hfq = HfqFile::open(output).map_err(|e| format!("open {} qwen35 identity: {e}", output.display()))?;
    use std::collections::hash_map::DefaultHasher; use std::hash::{Hash, Hasher};
    let mut bad = Vec::new();
    for i in 0..n_layers {
        ...
        for name in [&gate_name, &up_name] {
            if let Some((_, d)) = hfq.tensor_data(name) {
                let mut h=DefaultHasher::new(); d.hash(&mut h); ghs.push(h.finish()); gpresent.push(name.clone());
            } else { gmissing.push(name.clone()); }
        }
        let down_name = format!("{prefix}layers.{i}.mlp.down_proj.weight.hessian");
        let has_down = hfq.tensor_data(&down_name).is_some();
        if !gmissing.is_empty() {
            ...
            bad.push(format!("layer {i} gate/up diff hashes {:?} present {:?}", ghs, gpresent));
        }
        ...
        let k_name = format!("{prefix}layers.{i}.self_attn.k_proj.weight.hessian");
        let v_name = format!("{prefix}layers.{i}.self_attn.v_proj.weight.hessian");
        let has_linear = hfq.tensor_data(&lin_qkv).is_some() || hfq.tensor_data(&lin_z).is_some() || hfq.tensor_data(&lin_a).is_some() || hfq.tensor_data(&lin_b).is_some();
        let has_qkv = hfq.tensor_data(&q_name).is_some() || hfq.tensor_data(&k_name).is_some() || hfq.tensor_data(&v_name).is_some();
        if !has_linear && !has_qkv {
            ...
            for name in lin_names {
                if let Some((_, d)) = hfq.tensor_data(name) {
                    let mut h=DefaultHasher::new(); d.hash(&mut h); dhs.push(h.finish()); dpresent.push(name.clone());
                } else { dmissing.push(name.clone()); }
            }
            if !dmissing.is_empty() {
                bad.push(format!("layer {i} linear_attn in_proj qkv/z/a/b missing {}", dmissing.join(",")));
            } else if dhs.windows(2).any(|w| w[0]!=w[1]) {
                bad.push(format!("layer {i} linear_attn in_proj qkv/z/a/b diff hashes {:?} present {:?}", dhs, dpresent));
            }
            ...
            for name in qkv_names {
                if let Some((_, d)) = hfq.tensor_data(name) {
                    let mut h=DefaultHasher::new(); d.hash(&mut h); qhs.push(h.finish()); qpresent.push(name.clone());
                } else { qmissing.push(name.clone()); }
            }
            if !qmissing.is_empty() {
                bad.push(format!("layer {i} q/k/v missing {}", qmissing.join(",")));
            } else if qhs.windows(2).any(|w| w[0]!=w[1]) {
                bad.push(format!("layer {i} q/k/v diff hashes {:?} present {:?}", qhs, qpresent));
            }
        } else {
            ...
            let mut qhs = Vec::new();
            let mut qpresent = Vec::new();
            for name in qkv_names { if let Some((_, d)) = hfq.tensor_data(name) { let mut h=DefaultHasher::new(); d.hash(&mut h); qhs.push(h.finish()); qpresent.push(name.clone()); } else { qmissing.push(name.clone()); } }
            if !qmissing.is_empty() { bad.push(format!("layer {i} q/k/v missing {}", qmissing.join(","))); } else if qhs.windows(2).any(|w| w[0]!=w[1]) { bad.push(format!("layer {i} q/k/v diff hashes {:?} present {:?}", qhs, qpresent)); }
            let lin_names = [&lin_qkv, &lin_z, &lin_a, &lin_b];
            let mut dmissing = Vec::new();
            let mut dhs = Vec::new();
            let mut dpresent = Vec::new();
            for name in lin_names { if let Some((_, d)) = hfq.tensor_data(name) { let mut h=DefaultHasher::new(); d.hash(&mut h); dhs.push(h.finish()); dpresent.push(name.clone()); } else { dmissing.push(name.clone()); } }
            if !dmissing.is_empty() { bad.push(format!("layer {i} linear_attn in_proj qkv/z/a/b missing {}", dmissing.join(","))); } else if dhs.windows(2).any(|w| w[0]!=w[1]) { bad.push(format!("layer {i} linear_attn in_proj qkv/z/a/b diff hashes {:?} present {:?}", dhs, dpresent)); }
        }
    }
    drop(hfq);
    if bad.is_empty() { Ok(()) } else { Err(bad.join("; ")) }
...
fn run_qwen35_batched(
    ...
    let seq = seq.max(1);
    for (si, chunk) in toks.chunks(seq).enumerate() {
        kv_cache.clear_gpu(gpu).map_err(|e| format!("qwen35 kv clear {si}: {e:?}"))?;
        dn_state.reset(gpu).map_err(|e| format!("qwen35 dn reset {si}: {e:?}"))?;
        ...
        if use_batched {
            match hipfire_arch_qwen35::qwen35::forward_prefill_batch(gpu, weights, cfg, chunk, 0, kv_cache, dn_state, scratch, None, None, None, None) {
                Ok(()) => {},
                Err(e) => {
                    let msg = format!("{e:?}");
                    // If batched refused (e.g. MQ3 in MoE, or KV config), fall back to per-token with loud WARN
                    if msg.contains("MQ3") || msg.contains("batch") || msg.contains("tree") || msg.contains("Moe") {
                        ...
                        for (pos, &tok) in chunk.iter().enumerate() {
                            hipfire_arch_qwen35::qwen35::forward_scratch(gpu, weights, cfg, tok, pos, kv_cache, dn_state, scratch).map_err(|e| format!("qwen35 per-token fallback seq {si} pos {pos}: {e:?}"))?;
                        }
                    } else {
                        return Err(format!("qwen35 forward_prefill_batch seq {si} B {}: {e:?}", chunk.len()));
                    }
                }
            }
        } else {
            ...
            for (pos, &tok) in chunk.iter().enumerate() {
                hipfire_arch_qwen35::qwen35::forward_scratch(gpu, weights, cfg, tok, pos, kv_cache, dn_state, scratch).map_err(|e| format!("qwen35 per-token seq {si} pos {pos}: {e:?}"))?;
            }
        }
    }
    Ok(())
...
fn main() {
    unsafe { std::env::set_var("HIPFIRE_NORMALIZE_PROMPT", "0"); std::env::set_var("HIPFIRE_GRAPH", "0"); std::env::set_var("HIPFIRE_GRAPH_MOE", "0"); }
    let args: Vec<String> = std::env::args().collect();
    if args.iter().any(|a| a == "--help" || a == "-h") { print_usage_and_exit(0); }
    let model = arg("--model", None).unwrap_or_else(|| { eprintln!("--model <bf16.hfq> required"); print_usage_and_exit(2); });
    let corpus = arg("--corpus", None).unwrap_or_else(|| { eprintln!("--corpus <txt> required"); print_usage_and_exit(2); });
    let output = arg("--output", None).unwrap_or_else(|| { eprintln!("--output <path.calib.hfq> required (no default; ~30GB for 27B)"); print_usage_and_exit(2); });
    let seq_len: usize = arg("--seq-len", Some("2048".into())).unwrap().parse().unwrap_or_else(|_| { eprintln!("--seq-len must be usize"); print_usage_and_exit(2); });
    let max_tokens: usize = arg("--max-tokens", Some("262144".into())).unwrap().parse().unwrap_or_else(|_| { eprintln!("--max-tokens must be usize"); print_usage_and_exit(2); });
    let lpp: usize = arg("--layers-per-pass", Some("64".into())).unwrap().parse().unwrap_or_else(|_| { eprintln!("--layers-per-pass must be usize"); print_usage_and_exit(2); });
    let syrk_mode = arg("--syrk", Some("auto".into())).unwrap();
    if !matches!(syrk_mode.as_str(), "auto"|"rocblas"|"kernel") { eprintln!("--syrk must be auto|rocblas|kernel"); print_usage_and_exit(2); }
    let md5 = corpus_md5(&corpus);
    let mut hfq = HfqFile::open(Path::new(&model)).unwrap_or_else(|e| panic!("open model {model}: {e}"));
    ...
    let arch = arg("--arch", None).and_then(|s| s.parse::<u32>().ok()).unwrap_or(hfq.arch_id);
    let llama = matches!(arch, 0|1);
    let qwen35 = matches!(arch, 5|6);
    let gemma = arch == 13; let glimmer = arch == 14; let lfm2 = arch == 11;
    if !(llama||qwen35||gemma||glimmer||lfm2) { eprintln!("unsupported arch {arch}: dense shim wires 0/1 (tap1 llama),5/6 (tap2 qwen35),13+14 (tap2). MoE 9/10 out of scope."); std::process::exit(2); }
    let tok = Tokenizer::from_hfq_metadata(&hfq.metadata_json).unwrap_or_else(|e| panic!("tokenizer: {e}"));
    ...
    let n_tok = toks.len().min(max_tokens); toks.truncate(n_tok);
    ...
    let syrk_log = if gpu.rocblas.is_some() { format!("syrk mode={syrk_mode} → {syrk_chosen} (rocBLAS present)") } else { format!("syrk mode={syrk_mode} → kernel (rocBLAS absent soft-fail fallback)") };
    eprintln!("{syrk_log}");
    if syrk_mode=="rocblas" && !avail { eprintln!("error: --syrk rocblas but gpu.rocblas None"); std::process::exit(1); }
    ...
    if llama {
        let cfg = hipfire_runtime::hfq::config_from_hfq(&hfq).unwrap_or_else(|e| panic!("config_from_hfq: {e}"));
        eprintln!("model arch={arch} n_layers={} dim={} hidden={} vocab={} head_dim={} qk_norm={}", cfg.n_layers, cfg.dim, cfg.hidden_dim, cfg.vocab_size, cfg.head_dim, cfg.has_qk_norm);
        let weights = hipfire_runtime::hfq::load_weights_hfq(&hfq, &cfg, &mut gpu).unwrap_or_else(|e| panic!("load_weights: {e}"));
        ...
        {
            let all_names: Vec<String> = build_capture_llama(&weights, 0, cfg.n_layers, &calib_prefix).values().cloned().collect();
        ...
        eprintln!("grouped n_layers={} lpp={} n_groups={} peak≈{:.1} MB", cfg.n_layers, lpp, (cfg.n_layers+lpp-1)/lpp, peak as f64/1_048_576.0);
        ...
        let summary = if grouped {
            ...
            collect_grouped(&mut gpu, arch, cfg.n_layers, lpp, Vec::new(), Path::new(&output),
                &[("source_model", serde_json::json!(model.clone())), ("corpus", serde_json::json!(corpus.clone())), ("corpus_md5", serde_json::json!(md5.clone())), ("n_calib_tokens", serde_json::json!(n_tok)), ("source_arch_id", serde_json::json!(arch)), ("seq_len", serde_json::json!(seq_len)), ("batch_size", serde_json::json!(batch_actual)), ("syrk_chosen", serde_json::json!(syrk_chosen)), ("syrk_mode", serde_json::json!(syrk_mode.clone())), ("layers_per_pass", serde_json::json!(lpp)), ("batches", serde_json::json!(n_seqs)), ("calib_driver", serde_json::json!("calib_sweep llama prefill_forward→weight_gemm:1446 tap1"))],
                |s,e| build_capture_llama(&weights,s,e, &calib_prefix_grouped),
                |gpu,_| { run_llama_batched(gpu,&weights,&cfg,&tc,seq_len)?; Ok(CalibForward::default()) }
            ).unwrap_or_else(|e| { eprintln!("collect_grouped: {e}"); std::process::exit(1); })
        } else {
            let cap = build_capture_llama(&weights,0,cfg.n_layers, &calib_prefix_single);
            let tc = toks.clone(); let cc = cfg.clone();
            collect(&mut gpu, arch, cap, Vec::new(), Path::new(&output),
                &[("source_model", serde_json::json!(model.clone())), ("corpus", serde_json::json!(corpus.clone())), ("corpus_md5", serde_json::json!(md5.clone())), ("n_calib_tokens", serde_json::json!(n_tok)), ("source_arch_id", serde_json::json!(arch)), ("seq_len", serde_json::json!(seq_len)), ("batch_size", serde_json::json!(batch_actual)), ("syrk_chosen", serde_json::json!(syrk_chosen)), ("syrk_mode", serde_json::json!(syrk_mode.clone())), ("layers_per_pass", serde_json::json!(cfg.n_layers)), ("batches", serde_json::json!(n_seqs)), ("calib_driver", serde_json::json!("calib_sweep llama prefill_forward→weight_gemm:1446 tap1"))],
                |gpu| { run_llama_batched(gpu,&weights,&cc,&tc,seq_len)?; Ok(CalibForward::default()) }
            ).unwrap_or_else(|e| { eprintln!("collect: {e}"); std::process::exit(1); })
        };
        ...
        let expected_names: Vec<String> = build_capture_llama(&weights, 0, cfg.n_layers, &calib_prefix).values().cloned().collect();
        match check_coverage(Path::new(&output), &expected_names) {
            Ok(()) => eprintln!("coverage {}/{} [PASS]", expected_names.len(), expected_names.len()),
            Err(m) => { eprintln!("{m}"); print_per_proj_summary(Path::new(&output), &expected_names); std::process::exit(1); }
        }
        ...
        eprintln!("collected {} hessian {} imatrix {:.1}s max_consistency {:.3e} {}", summary.n_hessian, summary.n_imatrix, elapsed, summary.max_consistency, if summary.max_consistency < 1e-4 {"[CONSISTENT]"} else {"[MISMATCH]"});
        if summary.max_consistency >= 1e-4 { eprintln!("FAIL diag(H) vs Σx² >=1e-4"); std::process::exit(1); }
        match check_identity(Path::new(&output), cfg.n_layers, &calib_prefix) { Ok(()) => eprintln!("gate q/k/v identical gate/up identical [PASS]"), Err(m) => { eprintln!("FAIL identity: {m}"); std::process::exit(1); } }
        ...
        eprintln!("n_hessian {} n_imatrix {}", summary.n_hessian, summary.n_imatrix);
        eprintln!("output: {output} {bytes} bytes elapsed {:.1}s", elapsed);
        eprintln!("peak per-group {:.1} MB lpp={} dense ~{:.1} MB", peak as f64/1_048_576.0, lpp, { let mut d=0u64; for i in 0..lpp.min(cfg.n_layers){ let l=&weights.layers[i]; for k in [l.wq.k,l.wk.k,l.wv.k,l.wo.k,l.w_gate.k,l.w_up.k,l.w_down.k]{ d+=(k*k*4) as u64 + (k*4) as u64; } } d as f64/1_048_576.0 });
        eprintln!("config n_layers={} dim={} hidden={} head_dim={} lpp={}", cfg.n_layers, cfg.dim, cfg.hidden_dim, cfg.head_dim, lpp);
        ...
        let cfg = hipfire_arch_qwen35::qwen35::config_from_hfq(&hfq).unwrap_or_else(|e| panic!("qwen35 config_from_hfq: {e}"));
        ...
        eprintln!("qwen35 arch={} n_layers={} dim={} vocab={} hidden={} layer_types len={} moe={}", arch, cfg.n_layers, cfg.dim, cfg.vocab_size, cfg.hidden_dim, cfg.layer_types.len(), cfg.num_experts);
        ...
        let weights = hipfire_arch_qwen35::qwen35::load_weights(&mut source, std::slice::from_mut(&mut gpu), &layout).unwrap_or_else(|e| panic!("qwen35 load_weights: {e:?}"));
        ...
        {
            let all_names: Vec<String> = build_capture_qwen35(&weights, 0, cfg.n_layers, &calib_prefix).values().cloned().collect();
            verify_capture_names_or_exit(&hfq, &all_names);
            eprintln!("calib prefix '{}' verified: {} capture names all resolve in HFQ (safetensors convention)", calib_prefix, all_names.len());
        }
        drop(hfq);
        ...
        let mut kv_cache = hipfire_runtime::llama::KvCache::new_gpu_q8(&mut gpu, cfg.n_layers, cfg.n_kv_heads, cfg.head_dim, kv_max).unwrap_or_else(|e| panic!("qwen35 kv alloc q8 kv_max={kv_max}: {e:?}"));
        let scratch = hipfire_arch_qwen35::qwen35::Qwen35Scratch::new_with_kv_max(&mut gpu, &cfg, 64, kv_max).unwrap_or_else(|e| panic!("qwen35 scratch kv_max={kv_max}: {e:?}"));
        let mut dn_state = hipfire_arch_qwen35::qwen35::DeltaNetState::new(&mut gpu, &cfg).unwrap_or_else(|e| panic!("qwen35 dn_state: {e:?}"));
        let lpp = lpp.max(1);
        ...
        let elapsed = t0.elapsed().as_secs_f64();
        let bytes = std::fs::metadata(&output).map(|m| m.len()).unwrap_or(0);
        let expected_names: Vec<String> = build_capture_qwen35(&weights, 0, cfg.n_layers, &calib_prefix).values().cloned().collect();
        match check_coverage(Path::new(&output), &expected_names) {
            Ok(()) => eprintln!("coverage {}/{} [PASS]", expected_names.len(), expected_names.len()),
            Err(m) => { eprintln!("{m}"); print_per_proj_summary(Path::new(&output), &expected_names); std::process::exit(1); }
        }
        ...
        eprintln!("--- run summary (provenance) ---");
        eprintln!("model: {model} arch={arch} n_layers={} dim={} hidden={} kv_max={} lpp={}", cfg.n_layers, cfg.dim, cfg.hidden_dim, kv_max, lpp);
        eprintln!("HIPFIRE_NORMALIZE_PROMPT=0 HIPFIRE_GRAPH=0 HIPFIRE_GRAPH_MOE=0");
        ...
        let cfg = hipfire_arch_gemma4::lowered::config_from_hfq(&hfq).unwrap_or_else(|| panic!("gemma4 cfg: missing/invalid config in HFQ"));
        eprintln!("gemma4 arch=13 n_layers={} dim={} vocab={} hidden={} layer_types={:?} enable_moe={}", cfg.n_layers, cfg.dim, cfg.vocab_size, cfg.hidden_dim, cfg.layer_types, cfg.enable_moe_block);
        ...
        let mut weights = hipfire_arch_gemma4::lowered::load_weights(&mut hfq, &cfg, &mut gpu).unwrap_or_else(|e| panic!("gemma4 weights: {e}"));
        {
            let all_names: Vec<String> = build_capture_gemma(&weights, 0, cfg.n_layers, &calib_prefix).values().cloned().collect();
            verify_capture_names_or_exit(&hfq, &all_names);
            eprintln!("calib prefix '{}' verified: {} capture names all resolve in HFQ (safetensors convention)", calib_prefix, all_names.len());
        }
        drop(hfq);
        let kv_max = seq_len + 16;
        let scratch = hipfire_arch_gemma4::lowered::Gemma4Scratch::new(&mut gpu, &cfg, 1).unwrap_or_else(|e| panic!("gemma4 scratch: {e:?}"));
        hipfire_arch_gemma4::lowered::init_scratch_constants(&mut gpu, &scratch, cfg.full_head_dim).unwrap_or_else(|e| panic!("gemma4 init_scratch_constants: {e:?}"));
        ...
        let mut kv_sliding = hipfire_runtime::llama::KvCache::new_gpu_q8(&mut gpu, cfg.n_layers, cfg.sliding_n_kv_heads, cfg.sliding_head_dim, kv_max).unwrap_or_else(|e| panic!("gemma sliding kv alloc q8 {kv_max}: {e:?}"));
        let mut kv_full = hipfire_runtime::llama::KvCache::new_gpu_q8(&mut gpu, cfg.n_layers, cfg.full_n_kv_heads, cfg.full_head_dim, kv_max).unwrap_or_else(|e| panic!("gemma full kv alloc q8 {kv_max}: {e:?}"));
        ...
        let elapsed = t0.elapsed().as_secs_f64();
        let bytes = std::fs::metadata(&output).map(|m| m.len()).unwrap_or(0);
        let expected_names: Vec<String> = build_capture_gemma(&weights, 0, cfg.n_layers, &calib_prefix).values().cloned().collect();
        match check_coverage(Path::new(&output), &expected_names) {
            Ok(()) => eprintln!("coverage {}/{} [PASS]", expected_names.len(), expected_names.len()),
            Err(m) => { eprintln!("{m}"); print_per_proj_summary(Path::new(&output), &expected_names); std::process::exit(1); }
        }
        ...
        let cfg = hipfire_arch_muse_glimmer::config::GlimmerConfig::from_hfq(&hfq).unwrap_or_else(|e| panic!("glimmer cfg: {e}"));
        eprintln!("glimmer arch=14 n_layers={} dim={} vocab={} hidden={} sliding_window={}", cfg.n_layers, cfg.dim, cfg.vocab_size, cfg.hidden_dim, cfg.sliding_window);
        let weights = hipfire_arch_muse_glimmer::glimmer::GlimmerWeights::load(&hfq, &cfg, &mut gpu).unwrap_or_else(|e| panic!("glimmer weights: {e}"));
        {
            let all_names: Vec<String> = build_capture_glimmer(&weights, 0, cfg.n_layers, &calib_prefix).values().cloned().collect();
            verify_capture_names_or_exit(&hfq, &all_names);
            eprintln!("calib prefix '{}' verified: {} capture names all resolve in HFQ (safetensors convention)", calib_prefix, all_names.len());
        }
        drop(hfq);
        let kv_max = seq_len + 16;
        let mut state = hipfire_arch_muse_glimmer::glimmer::GlimmerState::new_with_max_seq(&mut gpu, &cfg, kv_max).unwrap_or_else(|e| panic!("glimmer state alloc kv_max {kv_max}: {e}"));
        let lpp = lpp.max(1);
        ...
        let elapsed = t0.elapsed().as_secs_f64();
        let bytes = std::fs::metadata(&output).map(|m| m.len()).unwrap_or(0);
        let expected_names: Vec<String> = build_capture_glimmer(&weights, 0, cfg.n_layers, &calib_prefix).values().cloned().collect();
        match check_coverage(Path::new(&output), &expected_names) {
            Ok(()) => eprintln!("coverage {}/{} [PASS]", expected_names.len(), expected_names.len()),
            Err(m) => { eprintln!("{m}"); print_per_proj_summary(Path::new(&output), &expected_names); std::process::exit(1); }
        }
        ...
        eprintln!("collected {} hessian {} imatrix {:.1}s max_consistency {:.3e} {}", summary.n_hessian, summary.n_imatrix, elapsed, summary.max_consistency, if summary.max_consistency < 1e-4 {"[CONSISTENT]"} else {"[MISMATCH]"});
        if summary.max_consistency >= 1e-4 { eprintln!("FAIL diag(H) vs Σx² >=1e-4"); std::process::exit(1); }
        match check_identity_generic(Path::new(&output), cfg.n_layers, true, false, &calib_prefix) { Ok(()) => eprintln!("glimmer identity: q/k/v/attn_gate identical gate/up identical [PASS]"), Err(m) => { eprintln!("FAIL glimmer identity: {m}"); std::process::exit(1); } }
        eprintln!("wrote {output} {bytes} bytes");
        eprintln!("--- run summary (provenance) ---");
        eprintln!("model: {model} arch={arch} n_layers={} dim={} hidden={} kv_max={} lpp={}", cfg.n_layers, cfg.dim, cfg.hidden_dim, kv_max, lpp);
        eprintln!("HIPFIRE_NORMALIZE_PROMPT=0 HIPFIRE_GRAPH=0 HIPFIRE_GLIMMER_KV_VMM=1");
        eprintln!("batched: GlimmerState [seq_len+16] prefill_chunk_batched chunk {}", hipfire_arch_muse_glimmer::forward::glimmer_prefill_chunk_size(seq_len));
    } else if lfm2 {
        let cfg = hipfire_arch_lfm2moe::config::Lfm2MoeConfig::from_hfq(&hfq).unwrap_or_else(|e| panic!("lfm2 cfg: {e}"));
        ...
        let weights = hipfire_arch_lfm2moe::lfm2moe::Lfm2MoeWeights::load(&mut hfq, &cfg, &mut gpu).unwrap_or_else(|e| panic!("lfm2 weights: {e}"));
        {
            let all_names: Vec<String> = build_capture_lfm(&weights, 0, cfg.num_hidden_layers, &calib_prefix).values().cloned().collect();
            verify_capture_names_or_exit(&hfq, &all_names);
            eprintln!("calib prefix '{}' verified: {} capture names all resolve in HFQ (safetensors convention)", calib_prefix, all_names.len());
        }
        drop(hfq);
        // validate weight formats for batched path per forward_batch::batch_weight_formats_supported
        if let Err(e) = hipfire_arch_lfm2moe::forward_batch::batch_weight_formats_supported(&weights) {
        ...
        let mut state = hipfire_arch_lfm2moe::batch::Lfm2DecodeBatchState::new(&mut gpu, &cfg, max_batch, kv_max, 32).unwrap_or_else(|e| panic!("lfm batch state alloc max_batch={max_batch} kv_max={kv_max}: {e:?}"));
        let lpp = lpp.max(1);
        ...
        let summary = if grouped {
            let tc = toks.clone();
            collect_grouped(&mut gpu, arch, cfg.num_hidden_layers, lpp, Vec::new(), Path::new(&output),
                &[("source_model", serde_json::json!(model.clone())), ("corpus", serde_json::json!(corpus.clone())), ("corpus_md5", serde_json::json!(md5.clone())), ("n_calib_tokens", serde_json::json!(n_tok)), ("source_arch_id", serde_json::json!(arch)), ("seq_len", serde_json::json!(seq_len)), ("batch_size", serde_json::json!(batch_actual)), ("syrk_chosen", serde_json::json!(syrk_chosen)), ("syrk_mode", serde_json::json!(syrk_mode.clone())), ("layers_per_pass", serde_json::json!(lpp)), ("batches", serde_json::json!(n_seqs)), ("lane_capacity", serde_json::json!(kv_max)), ("calib_driver", serde_json::json!("calib_sweep lfm forward_decode_batch_lfm:290 → batched_proj:61 tap3"))],
                |s,e| build_capture_lfm(&weights,s,e, &calib_prefix_grouped),
                |gpu,_| { run_lfm_batched(gpu,&weights,&cfg,&mut state,&tc,seq_len)?; Ok(CalibForward::default()) }
            ).unwrap_or_else(|e| { eprintln!("collect_grouped lfm: {e}"); std::process::exit(1); })
        } else {
            let cap = build_capture_lfm(&weights,0,cfg.num_hidden_layers, &calib_prefix_single);
            let tc = toks.clone();
            collect(&mut gpu, arch, cap, Vec::new(), Path::new(&output),
                &[("source_model", serde_json::json!(model.clone())), ("corpus", serde_json::json!(corpus.clone())), ("corpus_md5", serde_json::json!(md5.clone())), ("n_calib_tokens", serde_json::json!(n_tok)), ("source_arch_id", serde_json::json!(arch)), ("seq_len", serde_json::json!(seq_len)), ("batch_size", serde_json::json!(batch_actual)), ("syrk_chosen", serde_json::json!(syrk_chosen)), ("syrk_mode", serde_json::json!(syrk_mode.clone())), ("layers_per_pass", serde_json::json!(cfg.num_hidden_layers)), ("batches", serde_json::json!(n_seqs)), ("lane_capacity", serde_json::json!(kv_max)), ("calib_driver", serde_json::json!("calib_sweep lfm forward_decode_batch_lfm:290 → batched_proj:61 tap3"))],
                |gpu| { run_lfm_batched(gpu,&weights,&cfg,&mut state,&tc,seq_len)?; Ok(CalibForward::default()) }
            ).unwrap_or_else(|e| { eprintln!("collect lfm: {e}"); std::process::exit(1); })
        };
        let _ = &weights;
        let elapsed = t0.elapsed().as_secs_f64();
        let bytes = std::fs::metadata(&output).map(|m| m.len()).unwrap_or(0);
        let expected_names: Vec<String> = build_capture_lfm(&weights, 0, cfg.num_hidden_layers, &calib_prefix).values().cloned().collect();
        match check_coverage(Path::new(&output), &expected_names) {
            Ok(()) => eprintln!("coverage {}/{} [PASS]", expected_names.len(), expected_names.len()),
            Err(m) => { eprintln!("{m}"); print_per_proj_summary(Path::new(&output), &expected_names); std::process::exit(1); }
        }
        ...
        eprintln!("collected {} hessian {} imatrix {:.1}s max_consistency {:.3e} {}", summary.n_hessian, summary.n_imatrix, elapsed, summary.max_consistency, if summary.max_consistency < 1e-4 {"[CONSISTENT]"} else {"[MISMATCH]"});
        if summary.max_consistency >= 1e-4 { eprintln!("FAIL lfm diag(H) vs Σx² >=1e-4"); std::process::exit(1); }
        ...
        eprintln!("wrote {output} {bytes} bytes");
        eprintln!("--- run summary (provenance) ---");
        eprintln!("model: {model} arch={arch} n_layers={} hidden={} kv_max={} lpp={}", cfg.num_hidden_layers, cfg.hidden_size, kv_max, lpp);
