...
fn main() {
    let path = std::env::args()
        .nth(1)
        .expect("usage: check_awq_gptq <path>");
    ...
    let gptq = tensors
        .iter()
        .filter(|t| t.name.contains("gptq") || t.name.contains("g_idx"))
        .count();
    let paro = tensors
        .iter()
        .filter(|t| t.name.contains("paro") || t.name.contains("rotation"))
        .count();
    println!("file: {path}");
    ...
    if awq == 0 && gptq == 0 {
        println!(
            "  PLAIN MQ4 — no AWQ/GPTQ calibration (LOWER quality, not recommended for benches)"
        );
