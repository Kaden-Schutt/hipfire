...
fn main() {
    ...
    eprintln!("\n[Task-5 A/B] running second pass with decomposed pre-down Steps ON...");
    set_moe_step_predown_override(Some(true));
    let mut state_per_rank_on: Vec<DeepseekV4State> = Vec::with_capacity(n);
    let mut partials_on: Vec<GpuTensor> = Vec::with_capacity(n);
    for r in 0..n {
        gpus.devices[r].bind_thread().expect("bind on");
        state_per_rank_on.push(DeepseekV4State::new(&cfg).expect("state on"));
        partials_on.push(
            gpus.devices[r]
                .zeros(&[cfg.hidden_size], DType::F32)
                .expect("partial on"),
