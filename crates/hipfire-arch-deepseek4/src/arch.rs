    ...
    fn load_weights_inner(
        ...
        if let Some(configured) = configured_cache {
            if routed_gpu.is_some() {
                return Err("deepseek4: expert paging and heterogeneous routed execution are \
                            mutually exclusive — paging currently owns slot pools and pointer \
                            publication on the dense model GPU"
                    .into());
            }
            if shard.is_some() {
