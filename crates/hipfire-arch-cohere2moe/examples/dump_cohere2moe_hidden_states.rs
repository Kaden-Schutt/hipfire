...
//! Dump per-layer hidden states from the Cohere2Moe forward for offline
...
//! `scripts/gen_tiny_cohere2moe.py` + `scripts/compare_hidden_states.py`.
//!
//! Usage:
//!   dump_cohere2moe_hidden_states --model <hfq> --ref <kldref> --out <hfhs> [--chunk N]
...
fn main() {
    ...
    use hipfire_arch_cohere2moe::forward::decode_step_capture;
    ...
    eprintln!(
        "arch=cohere2moe hidden={} layers={} heads={}/{} head_dim={} experts={}/{} dense_inter={} moe_inter={} first_k_dense={}",
        cfg.hidden_size,
    ...
    let mut state = CohereState::new_with_max_seq(&mut gpu, &cfg, n_ctx + 16).expect("state");
