    ...
    fn weight_manifest(cfg: &Self::Config) -> Vec<WeightEntry> {
        ...
        for l in 0..cfg.num_hidden_layers {
            ...
            if cfg.attention_bias {
                ...
                m.push(WeightEntry::layer("bq", l, vec![nh * hd], DType::F32, ColumnShard { axis: 0 }));
                m.push(WeightEntry::layer("bk", l, vec![nkv * hd], DType::F32, ColumnShard { axis: 0 }));
                m.push(WeightEntry::layer("bv", l, vec![nkv * hd], DType::F32, ColumnShard { axis: 0 }));
            }
            m.push(WeightEntry::layer("ffn_gate", l, vec![ff, d], DType::F16, ColumnShard { axis: 0 }));
            m.push(WeightEntry::layer("ffn_up", l, vec![ff, d], DType::F16, ColumnShard { axis: 0 }));
            m.push(WeightEntry::layer("ffn_down", l, vec![d, ff], DType::F16, RowShard { axis: 1 }));
        ...
        let lm_policy = if cfg.tie_word_embeddings {
            Tied { source: "token_embd".to_string() }
        } else {
            Pin(PinTarget::Output)
        };
        m.push(WeightEntry::model("lm_head", vec![cfg.vocab_size, d], DType::F16, lm_policy));
        m
    }
