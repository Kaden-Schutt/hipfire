...
struct TokenizerConfig {
    ...
    prompt_heat_json: bool,
    prompt_heat_limit: usize,
}

static TOKENIZER_CONFIG: OnceLock<TokenizerConfig> = OnceLock::new();
...
impl Tokenizer {
    ...
    pub fn from_tokenizer_json(path: &std::path::Path) -> Result<Option<Self>, TokenizerError> {
        let Some(json_str) = crate::read_optional_tokenizer_json(path) else {
            return Ok(None);
        };
    ...
    pub fn from_hfq_metadata(metadata_json: &str) -> Result<Self, TokenizerError> {
        ...
        match crate::hfq_tokenizer_metadata(metadata_json)? {
            Some(crate::HfqTokenizerMetadata::HfJson(tok_str)) => {
            ...
            Some(crate::HfqTokenizerMetadata::GgufMeta(gguf_meta)) => {
...
impl Tokenizer {
    ...
    pub fn dump_prompt_heat(&self, text: &str) {
        ...
        if tokenizer_config().prompt_heat_json {
...
impl hipfire_prompt::PromptTokenizer for Tokenizer {
    fn encode(&self, text: &str) -> Vec<u32> {
        Tokenizer::encode(self, text)
    }

    fn special_token_id(&self, content: &str) -> Option<u32> {
        Tokenizer::special_token_id(self, content)
    }
