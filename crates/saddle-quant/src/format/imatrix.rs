...
mod tests {
    ...
    fn bartowski_qwen3_distribution() {
        ...
        assert_eq!(safetensors_names.len(), 496, "total safetensors count should be 496");

        // map
        let mut counts: std::collections::HashMap<String, usize> = std::collections::HashMap::new();
        for name in &safetensors_names {
            let ggml = safetensors_to_ggml_name(name)
                .unwrap_or_else(|| panic!("should map: {name}"));
    ...
    fn unsupported_version_is_error() {
        ...
        let err = parse_bytes(&bytes).unwrap_err();
        match err {
            QuantError::UnsupportedVersion { artifact, found, supported: _ } => {
                assert_eq!(artifact, "imatrix");
                assert_eq!(found, 99);
            }
