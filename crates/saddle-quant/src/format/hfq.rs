...
impl HfqFile {
    ...
    fn parse(mmap: &[u8]) -> Result<Self> {
        ...
        let metadata_json =
            String::from_utf8_lossy(&meta_bytes[..json_end]).to_string();
        ...
        for _ in 0..n_tensors {
            ...
            for _ in 0..n_dims {
                shape.push(u32::from_le_bytes(
                    mmap[pos..pos + 4].try_into().unwrap(),
                ));
                pos += 4;
            }
...
mod tests {
    ...
    fn round_trip_small_artifact() {
        let tf = build_hfq_file(
            ...
            vec![
                ("model.embed_tokens.weight", 1, vec![5120, 1024], 0, vec![0u8; 1024]),
                ("model.layers.0.attn.q_proj.weight", 13, vec![5120, 5120], 256, vec![1u8; 2048]),
        ...
        let index_len = {
            ...
            for (name, shape, payload) in [
                ...
                ("model.layers.0.attn.q_proj.weight", vec![5120, 5120], 2048usize),
    ...
    fn rejects_bad_magic() {
        ...
        match err {
            QuantError::BadMagic { artifact, expected, found: _ } => {
    ...
    fn rejects_truncated_header() {
        ...
        let err = HfqFile::open(tf.path()).unwrap_err();
        match err {
            QuantError::Truncated { artifact, context: _, need: _, have: _ } => {
                assert_eq!(artifact, "HFQ");
            }
    ...
    fn rejects_truncated_index() {
        ...
        let mut buf = build_raw_buffer(5, 1, "{}", vec![("weight", 1, vec![4, 4], 0, vec![0u8; 32])]);
    ...
    fn dtype_histogram_groups_correctly() {
        ...
        for i in 0..10 {
            tensors.push((format!("layer.{i}.mq4.weight"), 13, vec![1024, 1024], 256, vec![0u8; 100]));
        }
        for i in 0..2 {
            tensors.push((format!("layer.{i}.q8.weight"), 3, vec![512, 512], 0, vec![0u8; 200]));
        }
        for i in 0..6 {
            tensors.push((format!("layer.{i}.f16.weight"), 1, vec![256, 256], 0, vec![0u8; 50]));
        }
        let meta = "{}";
        let n = tensors.len() as u32;
        let metadata_offset = HEADER_SIZE as u64;
        ...
        buf[16..24].copy_from_slice(&metadata_offset.to_le_bytes());
        buf[24..32].copy_from_slice(&data_offset.to_le_bytes());
        buf[metadata_offset as usize..metadata_offset as usize + meta.len()].copy_from_slice(meta.as_bytes());
        let index_start = metadata_offset as usize + meta.len();
        buf[index_start..index_start + index.len()].copy_from_slice(&index);
        let mut off = data_offset as usize;
        for (_, _, _, _, payload) in &tensors {
