...
mod tests {
    ...
    fn nan_implausible_cites_non_finite() {
        ...
        match verdict {
            TeacherVerdict::Implausible { reason, .. } => {
                let lower = reason.to_lowercase();
                assert!(
                    lower.contains("non-finite") || lower.contains("non finite") || lower.contains("finite"),
        ...
        match evaluate(&gate_248320(), stats2) {
            TeacherVerdict::Implausible { reason, .. } => {
                let lower = reason.to_lowercase();
                assert!(lower.contains("finite") || lower.contains("non-finite"), "got {reason}");
            }
    ...
    fn zero_scored_implausible() {
        ...
        match evaluate(&gate_248320(), stats) {
            TeacherVerdict::Implausible { reason, .. } => {
                let lower = reason.to_lowercase();
                assert!(lower.contains("n_scored") || lower.contains("0"), "got {reason}");
            }
    ...
    fn low_ppl_implausible() {
        ...
        match verdict {
            TeacherVerdict::Implausible { reason, .. } => {
                let lower = reason.to_lowercase();
                assert!(lower.contains("perplexity") || lower.contains("ppl") || lower.contains("low"), "got {reason}");
            }
    ...
    fn high_ppl_boundary() {
        ...
        match evaluate(&gate, stats2) {
            TeacherVerdict::Plausible(_) => {}
            TeacherVerdict::Implausible { reason, .. } => panic!("just below max should be plausible, got {reason}"),
        }
    }
    ...
    fn degradation_negative_and_positive() {
        ...
        assert!(report.candidate_better, "candidate 7.4737 < teacher 7.7852 should be better");
        assert!(report.delta_pct < 0.0, "delta_pct negative, got {}", report.delta_pct);
        ...
        // ~+2.73%
        assert!((report2.delta_pct - 2.73).abs() < 0.01, "got {}", report2.delta_pct);
    ...
    fn cross_comparable_all_fields() {
        ...
        e.estimator = Estimator::TopK { k: 256, bias_vs_full: None };
