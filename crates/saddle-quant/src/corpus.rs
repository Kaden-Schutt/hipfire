...
mod tests {
    ...
    fn render_chatml_contains_markers_and_both_contents() {
        ...
        assert!(r.contains("hello assistant content"), "assistant content missing");
        assert!(
    ...
    fn audit_rendered_chat_no_warning() {
        ...
        assert!(a.has_chat_structure);
        assert!(a.warnings.is_empty(), "chat corpus must not warn, got {:?}", a.warnings);
    }
    ...
    fn build_over_tempfile_prose_plus_conversations() {
        ...
        assert_eq!(sep_count, 3, "expected 3 separators, got {sep_count} in {:?}", built.text);
        ...
        assert_ne!(built.id.sha256, built3.id.sha256, "sha256 must change when input changes");
    ...
    fn render_invalid_conversation_structure_malformed() {
        ...
        .to_string();
        let err = render_conversations(&json, raising_tmpl).expect_err("should fail with raise_exception");
        match err {
            QuantError::Malformed(msg) => {
                assert!(msg.contains("conversation 0"), "error must name conversation index, got {msg:?}");
            }
            other => panic!("expected Malformed, got {other:?}"),
        }
    }
