...
impl Gpu {
    ...
    pub fn gemm_qkvza_hfq4g256_route_label(
        &mut self,
        a_qkv: &GpuTensor,
        a_z: &GpuTensor,
        a_beta: &GpuTensor,
        ...
        if self.rocblas_arch_eligible()
            ...
            && !self.capture_mode
        {
            return "gemm_qkvza_hfq4g256_rocblas";
        }
        if batch_size > 1 && !self.flags.fp16_disabled && self.flags.hfq4_qkvza_fast {
            if self.arch_caps.is_gcn5_wave64() {
                if self.arch_caps.is_gfx906() && self.arch_caps.should_use_mmq(batch_size) {
                    ...
                    if qz_safe {
                        return if self.arch_caps.gemv_dp4a_enabled() && !self.capture_mode {
                            "gemm_qkvza_hfq4g256_gfx906_mmq_dp4a_tail"
                        } else {
                            "gemm_qkvza_hfq4g256_gfx906_mmq_fp16_tail"
                        };
                    }
                    return "gemm_qkvza_hfq4g256_fp16_wave64_screen_reject";
                }
                if self.arch_caps.gemv_dp4a_enabled() && !self.capture_mode {
                    return "gemm_qkvza_hfq4g256_wave64_dp4a";
                }
                return "gemm_qkvza_hfq4g256_fp16_wave64";
            }
            if self.arch_caps.should_use_mmq(batch_size) {
                ...
                if use_mmq {
                    return "gemm_qkvza_hfq4g256_mmq_set";
                }
            }
            if self.arch_caps.has_hfq4_mmq() {
                ...
                if all_aligned {
                    return "gemm_qkvza_hfq4g256_mmq";
                }
                if qkv_m % 128 == 0 && z_m % 128 == 0 {
                    return "gemm_qkvza_hfq4g256_mmq_qz_dot2_tail";
                }
            }
            if self.arch_caps.has_wmma_w32_gfx12() {
                return "gemm_qkvza_hfq4g256_wmma_gfx12";
            }
            if self.arch_caps.has_wmma_w32() {
                return "gemm_qkvza_hfq4g256_wmma";
            }
            if self.arch_caps.has_dot2_f32_f16() {
                return "gemm_qkvza_hfq4g256_dot2";
            }
            return "gemm_qkvza_hfq4g256_fp16";
        }
