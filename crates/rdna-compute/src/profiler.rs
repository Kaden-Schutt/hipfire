...
pub fn hip_mp_count_to_cu_count(arch: &str, mp_count: u32) -> u32 {
    let is_rdna_wave32 =
        arch.starts_with("gfx10") || arch.starts_with("gfx11") || arch.starts_with("gfx12");
    if is_rdna_wave32 {
        mp_count.saturating_mul(2)
    } else {
        mp_count
    }
}
...
impl GpuCapability {
    ...
    /// Build a `GpuCapability` purely from arch-keyed static data — NO sysfs
    /// access, no live GPU required. Deterministic regardless of which host
    /// builds it: a live card's sysfs must never leak into a profile keyed
    /// by a DIFFERENT requested arch. Clock/BW/VRAM fields are left at 0
    /// (unknown) — callers that need a `peak_bw_gbs` honesty guard should
    /// source it explicitly rather than trust a sysfs-free zero as measured.
    /// Consumed by `chip_profile::ChipProfile::for_unprofiled` (Phase A T4).
    pub(crate) fn static_capability(arch: &str) -> GpuCapability {
        let spec = arch_spec(arch);
        // Mirrors the arch-keyed CU-count fallback in `detect_with_hint`
        // (kept in lock-step deliberately — this IS that fallback, just
        // reachable without a live sysfs/HIP-runtime hint).
        let cu_count = match arch {
            "gfx906" => 60,
            "gfx1010" => 40,
            "gfx1030" => 60,
            "gfx1100" => 48,
        ...
        GpuCapability {
            ...
            boost_clock_mhz: 0,
            mem_clock_mhz: 0,
            mem_bus_width_bits: spec.default_bus_width,
            vram_mb: 0,
        }
    }
}
