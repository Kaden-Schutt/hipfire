...
impl Gpu {
    ...
    /// Exact-gfx1201 TP3 graph variant that folds the system-scope producer
    /// barrier into the peer HC consumer. It preserves the incumbent fixed-rank
    /// transform sum and is selected only by the DS4 TP3 captured-graph screen.
    #[allow(clippy::too_many_arguments)]
    pub fn hc_mix_4stream_peer3_sync_gfx1201(
        &mut self,
        x_in: &GpuTensor,
        a_matrix: &GpuTensor,
        scale: &GpuTensor,
        ...
        if !self.arch_caps.is_gfx1201() {
            return Err(hip_bridge::HipError::new(
                0,
                &format!(
                    "hc_mix_4stream_peer3_sync_gfx1201 requires gfx1201, got {}",
                    self.arch
                ),
            ));
        }
            ...
            .any(|transform| transform.numel() != hidden_usize)
        {
            return Err(hip_bridge::HipError::new(
                0,
                "hc_mix_4stream_peer3_sync_gfx1201: transform shape mismatch",
            ));
        }
        const KERNEL: &str = "hc_mix_4stream_peer3_sync_gfx1201";
