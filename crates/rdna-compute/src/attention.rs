...
impl Gpu {
    ...
    /// Exact-gfx1201 three-stage compressed-attention occupancy screen.
    /// Product dispatch does not call this method until the channel clears.
    #[allow(clippy::too_many_arguments)]
    pub fn deepseek4_attn_swa_topk_split3_gfx1201(
        &mut self,
        q: &GpuTensor,
        swa_k: &GpuTensor,
        swa_v: &GpuTensor,
        ...
        self.bind_thread()?;
        assert_eq!(self.arch, "gfx1201", "split3 requires exact gfx1201");
        assert_eq!(head_dim, 512, "split3 requires D=512");
        const SCORE: &str = "deepseek4_attn_swa_topk_score_split_gfx1201";
        const SOFTMAX: &str = "deepseek4_attn_swa_topk_softmax_split_gfx1201";
        const VALUE: &str = "deepseek4_attn_swa_topk_value_split_gfx1201";
        for symbol in [SCORE, SOFTMAX, VALUE] {
            self.ensure_kernel(
                symbol,
                kernels::V4F_ATTN_SWA_TOPK_SPLIT3_GFX1201_SRC,
                symbol,
            )?;
        }
        let qp = q.buf.as_ptr();
        let skp = swa_k.buf.as_ptr();
        let svp = swa_v.buf.as_ptr();
        let tkp = topk_k.buf.as_ptr();
        let tvp = topk_v.buf.as_ptr();
        let sp = attn_sink.buf.as_ptr();
        let scratch_ptr = scratch.buf.as_ptr();
        let op = attn_out.buf.as_ptr();
        let nvp = n_valid_swa_buf.buf.as_ptr();
        let nap = n_active_topk_buf.buf.as_ptr();
        let nh = n_heads;
        ...
        let mut score_params: Vec<*mut c_void> = vec![
            &qp as *const _ as *mut c_void,
            &skp as *const _ as *mut c_void,
            &tkp as *const _ as *mut c_void,
            &scratch_ptr as *const _ as *mut c_void,
            &nvp as *const _ as *mut c_void,
            &nap as *const _ as *mut c_void,
            &nh as *const _ as *mut c_void,
            &hd as *const _ as *mut c_void,
            &sw as *const _ as *mut c_void,
            &tw as *const _ as *mut c_void,
        ];
        self.launch_maybe_blob(
            SCORE,
            [
                ((swa_window + topk_window + 127) / 128) as u32,
                n_heads as u32,
                1,
            ],
            [128, 1, 1],
        ...
        let mut softmax_params: Vec<*mut c_void> = vec![
            &scratch_ptr as *const _ as *mut c_void,
            &sp as *const _ as *mut c_void,
            &nvp as *const _ as *mut c_void,
            &nap as *const _ as *mut c_void,
        ...
        let mut value_params: Vec<*mut c_void> = vec![
            &scratch_ptr as *const _ as *mut c_void,
            &svp as *const _ as *mut c_void,
            &tvp as *const _ as *mut c_void,
            &op as *const _ as *mut c_void,
            &nvp as *const _ as *mut c_void,
            &nap as *const _ as *mut c_void,
        ...
        self.launch_maybe_blob(
            VALUE,
            [((head_dim + 127) / 128) as u32, n_heads as u32, 1],
            [128, 1, 1],
            0,
            &mut value_params,
            || {
