...
impl Gpu {
    ...
    /// Exact-gfx1201 micro candidate: four workgroups cooperate per attention
    /// head while retaining a single captured launch. Product dispatch does
    /// not select this method until its occurrence-shaped screen clears.
    pub fn deepseek4_attn_swa_topk_coop4_gfx1201(
        &mut self,
        q: &GpuTensor,
        swa_k: &GpuTensor,
        swa_v: &GpuTensor,
        ...
        assert_eq!(
            self.arch, "gfx1201",
            "coop4 attention requires exact gfx1201"
        );
        assert_eq!(head_dim, 512, "coop4 attention requires head_dim=512");
        const KERNEL: &str = "deepseek4_attn_swa_topk_coop4_gfx1201";
        self.ensure_kernel(KERNEL, kernels::V4F_ATTN_SWA_TOPK_COOP4_GFX1201_SRC, KERNEL)?;
        let qp = q.buf.as_ptr();
        let kp = swa_k.buf.as_ptr();
        let vp = swa_v.buf.as_ptr();
        let tkp = topk_k.buf.as_ptr();
