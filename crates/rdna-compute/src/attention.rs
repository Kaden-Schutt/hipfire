...
impl Gpu {
    ...
    /// Exact-gfx1201 TP3 HC screen which computes all four residual streams in
    /// one thread. Product dispatch continues to use
    /// [`Self::hc_mix_4stream_peer3_gfx1201`] until the 86-boundary graph micro
    /// and model route clear their admission gates.
    pub fn hc_mix_4stream_peer3_vec4_gfx1201(
        &mut self,
        x_in: &GpuTensor,
        a_matrix: &GpuTensor,
        scale: &GpuTensor,
        ...
        if !self.arch_caps.is_gfx1201() {
            return Err(hip_bridge::HipError::new(
                0,
                &format!(
                    "hc_mix_4stream_peer3_vec4_gfx1201 requires gfx1201, got {}",
                    self.arch
                ),
            ));
        }
            ...
            .any(|transform| transform.numel() != hidden_usize)
        {
            return Err(hip_bridge::HipError::new(
                0,
                "hc_mix_4stream_peer3_vec4_gfx1201: transform shape mismatch",
            ));
        }
        const KERNEL: &str = "hc_mix_4stream_peer3_vec4_gfx1201";
