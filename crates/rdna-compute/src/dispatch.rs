...
/// **MUST STAY BIT-IDENTICAL TO `hipfire-quantize::main::GL_CB4S64`.**
///
/// MQ4-G256-SEL (qt=43) codebook family: 64 profiles x 16 levels in MAX-NORMALISED space.
/// See `hipfire_quantize::GL_CB4S64` for provenance. The selector is per-GROUP, so it
/// is wave-uniform -- the 16 live levels can be held in SGPRs via scalar loads.
pub const GL_CB4S64: [[f32; 16]; 64] = [
...
impl Gpu {
    ...
    pub fn convert_f32_to_bf16(
        ...
        let src_need = nelems.checked_mul(4).ok_or_else(|| {
            HipError::new(0, "convert_f32_to_bf16: nelems*4 overflow")
        })?;
        let dst_need = nelems.checked_mul(2).ok_or_else(|| {
            HipError::new(0, "convert_f32_to_bf16: nelems*2 overflow")
        })?;
        ...
        if nelems > i32::MAX as usize {
            return Err(HipError::new(
                0,
                &format!(
                    "convert_f32_to_bf16: nelems {nelems} exceeds i32::MAX"
                ),
            ));
        }
