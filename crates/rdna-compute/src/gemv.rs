...
impl Gpu {
    ...
    pub fn gemv_hfq4g256_moe_gate_up_k8_indexed(
        ...
        let (func_name, block, grid_x) = if cdna_wave64 {
            ...
            static GFX1151_GATE_UP_LATENCY0: OnceLock<bool> = OnceLock::new();
            let gfx1151_latency0 = self.arch_caps.is_gfx1151()
                && *GFX1151_GATE_UP_LATENCY0.get_or_init(|| {
                    std::env::var("HIPFIRE_GFX1151_GATE_UP_LATENCY0").as_deref() == Ok("1")
                });
            ...
            } else if gfx1151_latency0 {
                self.ensure_kernel(
                    "gemv_hfq4g256_moe_gate_up_indexed_latency0_gfx1151",
                    kernels::GEMV_HFQ4G256_MOE_GATE_UP_INDEXED_LATENCY0_GFX1151_SRC,
