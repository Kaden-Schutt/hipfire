...
impl Gpu {
    ...
    /// Exact-gfx1100 raw-buffer addressing screen for the generic E8-SoA
    /// kernel. The kernel preserves the incumbent two-accumulator arithmetic
    /// and wave reduction order; only the weight fetch address space changes.
    pub fn gemv_mfp4g32_e8_soa_buffer_gfx1100(
        &mut self,
        a_raw: &GpuTensor,
        x: &GpuTensor,
        y: &GpuTensor,
        ...
        assert!(
            self.arch_caps.is_gfx1100(),
            "buffer E8 GEMV requires exact gfx1100"
        );
        assert!(k % 256 == 0);
        const KERNEL: &str = "gemv_mfp4g32_e8_soa_buffer_gfx1100";
        self.gemv_mfp4g32_e8_soa_variant(
            KERNEL,
            kernels::GEMV_MFP4G32_E8_SOA_BUFFER_GFX1100_SRC,
            a_raw,
            x,
            y,
            m,
    ...
    /// Exact-gfx1100 shared-input projection screen using the shipping
    /// temporal raw-buffer weight addressing and unchanged E8 arithmetic.
    pub fn gemv_mfp4g32_e8_soa_shared_jobs_buffer_gfx1100(
        &mut self,
        weights: &[&GpuTensor],
        x: &GpuTensor,
        outputs: &[&GpuTensor],
        ...
        assert!(
            self.arch_caps.is_gfx1100(),
            "shared E8 buffer jobs require gfx1100"
        );
        assert!((2..=3).contains(&weights.len()));
        assert_eq!(weights.len(), outputs.len());
        assert!(k % 256 == 0);
        const KERNEL: &str = "gemv_mfp4g32_e8_soa_shared_jobs_buffer_gfx1100";
        self.ensure_kernel(
            KERNEL,
            kernels::GEMV_MFP4G32_E8_SOA_SHARED_JOBS_BUFFER_GFX1100_SRC,
            KERNEL,
        )?;
        let mut a = [weights[0].buf.as_ptr(); 3];
        let mut y = [outputs[0].buf.as_ptr(); 3];
