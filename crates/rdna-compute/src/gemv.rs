...
impl Gpu {
    ...
    /// gfx1201 B4 compact routed gate/up. `sorted_slot_index` and
    /// `sorted_expert_ids` are the stable block_m=1 scatter outputs; workgroup
    /// order is expert-major while writes retain the original slot layout.
    #[allow(clippy::too_many_arguments)]
    pub fn gemv_hfq4g256_moe_gate_up_compact_b4_gfx12(
        &mut self,
        expert_ptrs: &GpuTensor,
        topk_indices: &GpuTensor,
        sorted_slot_index: &GpuTensor,
        sorted_expert_ids: &GpuTensor,
        x: &GpuTensor,
        y_gate: &GpuTensor,
        y_up: &GpuTensor,
        m: usize,
        ...
        self.bind_thread()?;
        const KERNEL: &str = "gemv_hfq4g256_moe_gate_up_k8_compact_b4_gfx12";
        self.ensure_kernel(
            "gemv_hfq4g256_moe_gate_up_compact_b4_gfx12",
            kernels::GEMV_HFQ4G256_MOE_GATE_UP_COMPACT_B4_GFX12_SRC,
            KERNEL,
        )?;
        let pp = expert_ptrs.buf.as_ptr();
        let ip = topk_indices.buf.as_ptr();
        let sp = sorted_slot_index.buf.as_ptr();
        let ep = sorted_expert_ids.buf.as_ptr();
        let xp = x.buf.as_ptr();
        let ygp = y_gate.buf.as_ptr();
        let yup = y_up.buf.as_ptr();
        let m_val = m as i32;
        ...
        let bytes = total_slots * (crate::profile::gemv_hfq4g256_bytes(m, k) + m * 4);
        let timer = crate::profile::begin_timer(&self.hip, "gemv", KERNEL, bytes);
        let result = self.launch_maybe_blob(
            KERNEL,
            [(m / 2) as u32, total_slots as u32, 1],
            [32, 1, 1],
            0,
            &mut params,
            || {
    ...
    /// gfx1201 B4 compact routed down. Sorted workgroups write the same
    /// original `[token, rank, row]` expanded slots consumed by the existing
    /// deterministic combine kernel.
    #[allow(clippy::too_many_arguments)]
    pub fn gemv_hfq4g256_moe_down_compact_b4_gfx12(
        &mut self,
        expert_ptrs: &GpuTensor,
        topk_indices: &GpuTensor,
        sorted_slot_index: &GpuTensor,
        ...
        self.bind_thread()?;
        const KERNEL: &str = "gemv_hfq4g256_moe_down_k8_compact_b4_gfx12";
        self.ensure_kernel(
            "gemv_hfq4g256_moe_down_compact_b4_gfx12",
            kernels::GEMV_HFQ4G256_MOE_DOWN_COMPACT_B4_GFX12_SRC,
            KERNEL,
        )?;
        let pp = expert_ptrs.buf.as_ptr();
        let ip = topk_indices.buf.as_ptr();
        ...
        let result = self.launch_maybe_blob(
            KERNEL,
            [(m as u32).div_ceil(4), total_slots as u32, 1],
            [32, 1, 1],
            0,
            &mut params,
            || {
