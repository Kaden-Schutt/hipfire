...
impl Gpu {
    ...
    pub fn gemv_mq4g256v2_moe_gate_up_k8_indexed(
        ...
        //
        // Opt-in X_BUFFER candidate (NOLDS body + raw_buffer_load_b128 on x):
        // HIPFIRE_GFX1100_MQ4V2_GATE_UP_X_BUFFER=1 until measured.
        let use_nolds = self.arch_caps.is_gfx1100() && m == 1024 && k == 2048;
        let use_x_buffer = use_nolds
            && hipfire_config::developer_var("HIPFIRE_GFX1100_MQ4V2_GATE_UP_X_BUFFER").as_deref()
                == Ok("1");
        if use_x_buffer {
            self.ensure_kernel(
                "gemv_mq4g256v2_moe_gate_up_k8_indexed_k2048_x_buffer_gfx1100",
                kernels::GEMV_MQ4G256V2_MOE_GATE_UP_K8_INDEXED_K2048_X_BUFFER_GFX1100_SRC,
                "gemv_mq4g256v2_moe_gate_up_k8_indexed_k2048_x_buffer_gfx1100",
            )?;
        } else if use_nolds {
            self.ensure_kernel(
    ...
    /// Direct-launch the admitted gfx1100 no-LDS gate_up specialization.
    /// Same frozen 48-byte ABI / `[M/2,8,1]` / block32 / LDS0 as production.
    /// Bypasses arch/env routing so parity oracles can raw-bit compare against
    /// opt-in candidates without depending on the default selector.
    #[doc(hidden)]
    pub fn gemv_mq4g256v2_moe_gate_up_k8_indexed_k2048_nolds_gfx1100_exact(
        &mut self,
        expert_ptrs: &GpuTensor,
        topk_indices: &GpuTensor,
        x: &GpuTensor,
        ...
        self.bind_thread()?;
        const FUNC: &str = "gemv_mq4g256v2_moe_gate_up_k8_indexed_k2048_nolds_gfx1100";
        self.ensure_kernel(
            FUNC,
            kernels::GEMV_MQ4G256V2_MOE_GATE_UP_K8_INDEXED_K2048_NOLDS_GFX1100_SRC,
            FUNC,
    ...
    /// Direct-launch the opt-in gfx1100 no-LDS + X_BUFFER gate_up candidate.
    /// Same ABI/grid/FMA order as the admitted nolds route; activation loads
    /// only. Used by raw-bit parity against the nolds exact wrapper.
    #[doc(hidden)]
    pub fn gemv_mq4g256v2_moe_gate_up_k8_indexed_k2048_x_buffer_gfx1100_exact(
        &mut self,
        expert_ptrs: &GpuTensor,
        topk_indices: &GpuTensor,
        x: &GpuTensor,
        ...
        self.bind_thread()?;
        const FUNC: &str = "gemv_mq4g256v2_moe_gate_up_k8_indexed_k2048_x_buffer_gfx1100";
        self.ensure_kernel(
            FUNC,
            kernels::GEMV_MQ4G256V2_MOE_GATE_UP_K8_INDEXED_K2048_X_BUFFER_GFX1100_SRC,
            FUNC,
