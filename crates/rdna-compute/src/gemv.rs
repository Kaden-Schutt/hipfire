...
impl Gpu {
    ...
    /// Ornith qt44 shared-down R2 A/B: four consecutive rows per wave32 workgroup.
    /// Same ABI/admission as R1; grid is `M.div_ceil(4)`. Default remains R1 —
    /// callers must opt in via `HIPFIRE_MQ4V2_SHARED_DOWN_R2=1`.
    pub fn gemv_mq4g256v2_residual_sigmoid_scaled_k512_r2(
        &mut self,
        a_raw: &GpuTensor,
        x: &GpuTensor,
        y: &GpuTensor,
        ...
        if !(self.arch_caps.is_gfx1100() || self.arch_caps.is_gfx1201()) || m != 2_048 || k != 512 {
            return Err(hip_bridge::HipError::new(
                0,
                "gemv_mq4g256v2_residual_sigmoid_scaled_k512_r2: requires exact gfx1100|gfx1201 M=2048 K=512",
            ));
        }
        const FUNC: &str = "gemv_mq4g256v2_residual_sigmoid_scaled_k512_r2";
        self.ensure_kernel(
            FUNC,
            kernels::GEMV_MQ4G256V2_RESIDUAL_SIGMOID_SCALED_K512_R2_SRC,
            FUNC,
        )?;
        let a_ptr = a_raw.buf.as_ptr();
        let x_ptr = x.buf.as_ptr();
        ...
        let grid_x = (m as u32).div_ceil(4);
        let result =
            self.launch_maybe_blob(FUNC, [grid_x, 1, 1], [32, 1, 1], 0, &mut params, || {
                let mut b = hip_bridge::KernargBlob::new();
                b.push_ptr(a_ptr);
                b.push_ptr(x_ptr);
                b.push_ptr(y_ptr);
