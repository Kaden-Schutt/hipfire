...
impl Gpu {
    ...
    /// Experimental exact-gfx1100 four-wave workgroup packing for the E8-SoA
    /// decode GEMV. This changes only workgroup packing: each width-32 wave
    /// retains the incumbent row mapping and arithmetic order. No product route
    /// selects it until its occurrence-weighted micro screen clears admission.
    pub fn gemv_mfp4g32_e8_soa_pack4_gfx1100(
        &mut self,
        a_raw: &GpuTensor,
        x: &GpuTensor,
        y: &GpuTensor,
        ...
        assert!(
            ...
            "gfx1100 E8 pack4 candidate requires exact gfx1100"
        );
        assert!(k % 256 == 0, "gfx1100 E8 pack4 requires K%256=0");
        const KERNEL: &str = "gemv_mfp4g32_e8_soa_pack4_gfx1100_candidate";
        self.ensure_kernel(
            KERNEL,
            kernels::GEMV_MFP4G32_E8_SOA_PACK4_GFX1100_SRC,
            KERNEL,
        )?;
        let a_ptr = a_raw.buf.as_ptr();
        let x_ptr = x.buf.as_ptr();
