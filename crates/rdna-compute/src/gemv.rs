...
impl Gpu {
    ...
    /// Exact-gfx1201 mixed E8 pack with the model's router
    /// `sqrt(softplus(.))` applied to one selected output at the row store.
    /// The projection arithmetic and every non-selected output are identical
    /// to [`Self::gemv_mfp4g32_e8_soa_mixed_jobs_gfx1201`].
    pub fn gemv_mfp4g32_e8_soa_mixed_jobs_sqrt_softplus_gfx1201(
        &mut self,
        weights: &[&GpuTensor],
        x: &GpuTensor,
        outputs: &[&GpuTensor],
        rows: &[usize],
        k: usize,
        sqrt_softplus_job: usize,
    ) -> HipResult<()> {
        self.bind_thread()?;
        assert_eq!(
            self.arch_caps.arch(),
            "gfx1201",
            "mixed E8 post-op jobs require exact gfx1201"
        );
        assert!((2..=7).contains(&weights.len()));
        assert_eq!(weights.len(), outputs.len());
        assert_eq!(weights.len(), rows.len());
        assert!(sqrt_softplus_job < weights.len());
        assert!(k % 256 == 0);
        assert!(weights
            .iter()
            .all(|weight| weight.dtype == DType::MFP4G32E8SOA));
        const KERNEL: &str = "gemv_mfp4g32_e8_soa_mixed_jobs_sqrt_softplus_gfx1201";
        self.ensure_kernel(
            KERNEL,
            kernels::GEMV_MFP4G32_E8_SOA_MIXED_JOBS_GFX1201_SRC,
            KERNEL,
        ...
        let jobs = weights.len() as i32;
        let k_i32 = k as i32;
        let post_job = sqrt_softplus_job as i32;
        let total_rows = rows.iter().sum::<usize>();
        let mut params: Vec<*mut c_void> = Vec::with_capacity(25);
        for ptr in &a {
            params.push(ptr as *const _ as *mut c_void);
        }
        params.push(&x_ptr as *const _ as *mut c_void);
        ...
        params.push(&jobs as *const _ as *mut c_void);
        params.push(&k_i32 as *const _ as *mut c_void);
        params.push(&post_job as *const _ as *mut c_void);
        self.launch_maybe_blob(
            KERNEL,
            [total_rows as u32, 1, 1],
            [32, 1, 1],
