...
impl Gpu {
    ...
    /// Short causal-batch lift of the certified gfx1201 B256 decode fusion.
    /// The kernel advances the shared convolution ring in row order and emits
    /// already-normalized Q/K, replacing the ordinary batched conv + QK norm
    /// pair with one replay-recordable launch.
    #[cfg(feature = "deltanet")]
    #[allow(clippy::too_many_arguments)]
    pub fn conv1d_silu_split_qknorm_batch4_gfx1201(
        &mut self,
        q_out: &GpuTensor,
        k_out: &GpuTensor,
        v_out: &GpuTensor,
        ...
        self.bind_thread()?;
        const KERNEL: &str = "conv1d_silu_split_qknorm_batch4_gfx1201";
        self.ensure_kernel(
            KERNEL,
            kernels::CONV1D_SILU_SPLIT_QKNORM_BATCH4_GFX1201_SRC,
            KERNEL,
        )?;
        ...
        let qk_blocks = n_heads as u32;
        let v_blocks = (v_dim as u32).div_ceil(256);
        let bytes =
            crate::profile::conv1d_silu_bytes(2 * k_dim + v_dim) * n_tokens;
        let timer = crate::profile::begin_timer(&self.hip, "deltanet", KERNEL, bytes);
        let result = self.launch_maybe_blob(
            KERNEL,
            [qk_blocks + v_blocks, 1, 1],
            [256, 1, 1],
            0,
            &mut params,
            || {
