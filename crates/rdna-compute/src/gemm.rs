...
impl Gpu {
    ...
    /// nibble dequant (K2).
    ///
    /// Sibling of [`Self::gemm_hfq4g256_residual_muse_gfx1100_rm2_pipe`], NOT a
    /// replacement: measurement-only; touches no shared selector. Removes
    /// nibble int→float→half conversions via magic-bit half2 construction.
    /// `folded=false` selects exact-subtract (`…_magic_exact`); `folded=true`
    /// selects folded-bias (`…_magic_fold16`). Preserves exact per-accumulator
    /// K2 g/kt WMMA order, RM2/BV6 B reuse, and `Y +=` semantics. Exact
    /// gate/up only: M=19968, K=6656, batch=192. Grid `[624, 2, 1]`, block
    /// `[32, 1, 1]`, LDS 0.
    ///
    /// Returns `Ok(false)` without bind/compile when arch/shape/batch
    /// preconditions fail. Does not zero Y; performs `Y += W@X`. Exact arm
    /// must oracle bitdiff0; fold16 is allowed nonzero.
    pub fn gemm_hfq4g256_residual_muse_gfx1100_rm2_magic(
        &mut self,
        a: &GpuTensor,
        x: &GpuTensor,
        y: &GpuTensor,
        ...
        let kname = if folded {
            "gemm_hfq4g256_residual_wmma_gfx1100_muse_rm2_bv6_magic_fold16"
        } else {
            "gemm_hfq4g256_residual_wmma_gfx1100_muse_rm2_bv6_magic_exact"
        };
        if !self.arch_caps.is_gfx1100()
            || m != 19_968
            || k != 6_656
        ...
        self.bind_thread()?;
        // Module name is the FILE, not the symbol, so exact and fold16 share
        // one compile.
        self.ensure_kernel(
            "gemm_hfq4g256_residual_wmma_gfx1100_muse_rm_magic",
            kernels::GEMM_HFQ4G256_RESIDUAL_WMMA_GFX1100_MUSE_RM_MAGIC_SRC,
            kname,
        )?;
        let x_f16_ptr = self.ensure_fp16_x(x, batch_size * k)?;
