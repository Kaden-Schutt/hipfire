...
impl Gpu {
    ...
    ///
    /// Sibling of [`Self::gemm_hfq4g256_residual_muse_gfx1100_rm`], NOT a
    /// replacement: measurement-only; touches no shared selector. Same RM2/BV6
    /// tiling, exact per-output F32 accum order, and `Y +=` as the row-major
    /// baseline, but `a` must be the same-size fragment-major layout produced
    /// by [`hfq4g256_row_major_to_fragment_major`]. Exact gate/up only:
    /// M=19968, K=6656, batch=192. Grid `[624, 2, 1]`, block `[32, 1, 1]`, LDS 0.
    ///
    /// Returns `Ok(false)` without bind/compile when arch/shape/batch
    /// preconditions fail. Does not zero Y; performs `Y += W@X`.
    pub fn gemm_hfq4g256_residual_muse_gfx1100_rm2_fg(
        &mut self,
        a: &GpuTensor,
        x: &GpuTensor,
        y: &GpuTensor,
        ...
        let kname = "gemm_hfq4g256_residual_wmma_gfx1100_muse_rm2_bv6_fg";
        if !self.arch_caps.is_gfx1100()
            || m != 19_968
            || k != 6_656
            || batch_size != 192
        ...
        self.bind_thread()?;
        self.ensure_kernel(
            "gemm_hfq4g256_residual_wmma_gfx1100_muse_rm_fg",
            kernels::GEMM_HFQ4G256_RESIDUAL_WMMA_GFX1100_MUSE_RM_FG_SRC,
            kname,
        )?;
        let x_f16_ptr = self.ensure_fp16_x(x, batch_size * k)?;
...
/// Transcode HFQ4-G256 row-major weights into the same-size fragment-major
/// layout consumed by [`Gpu::gemm_hfq4g256_residual_muse_gfx1100_rm2_fg`].
///
/// Source layout (per row, per K-group of 256): 136 bytes =
/// `[f32 scale][f32 zero][128 packed nibbles]`.
///
/// Device layout (per 16-row tile t, per group g): 2176 bytes =
/// - bytes 0..127: 16 row headers, row `ml` at `ml*8` (`scale` then `zero`)
/// - bytes 128..2175: 16 K fragments; fragment `kt` has 16 rows × 8 packed
///   bytes, row `ml` at `128 + kt*128 + ml*8`
///
/// Group base: `(t * groups_per_row + g) * 2176`. Byte length is unchanged
/// (`M * (K/256) * 136`). Requires `M % 16 == 0` and `K % 256 == 0`.
pub fn hfq4g256_row_major_to_fragment_major(src: &[u8], m: usize, k: usize) -> Vec<u8> {
    assert_eq!(m % 16, 0, "fragment-major layout requires M%16==0");
    assert_eq!(k % 256, 0, "fragment-major layout requires K%256==0");
    let gpr = k / 256;
    let expected = m * gpr * 136;
    assert_eq!(
        src.len(),
        expected,
        "row-major HFQ4G256 byte length mismatch"
    );
    let mut out = vec![0u8; expected];
    let n_tiles = m / 16;
    for t in 0..n_tiles {
        for g in 0..gpr {
            let dst_base = (t * gpr + g) * 2176;
            for ml in 0..16 {
                let row = t * 16 + ml;
                let src_grp = row * gpr * 136 + g * 136;
                // Header: scale + zero
                out[dst_base + ml * 8..dst_base + ml * 8 + 8]
                    .copy_from_slice(&src[src_grp..src_grp + 8]);
                // 16 K fragments × 8 packed bytes
                for kt in 0..16 {
                    let s = src_grp + 8 + kt * 8;
                    let d = dst_base + 128 + kt * 128 + ml * 8;
                    out[d..d + 8].copy_from_slice(&src[s..s + 8]);
                }
            }
        }
    }
