...
impl Gpu {
    ...
    /// Batched sigmoid/alpha-gate transform with direct rollback-tape side
    /// outputs. The tape views already include the caller's token offset.
    #[cfg(feature = "deltanet")]
    #[allow(clippy::too_many_arguments)]
    pub fn fused_sigmoid_alpha_gate_f32_batched_tape(
        &mut self,
        beta: &GpuTensor,
        alpha: &GpuTensor,
        dt_bias: &GpuTensor,
        a_log: &GpuTensor,
        alpha_tape: &GpuTensor,
        beta_tape: &GpuTensor,
        n: usize,
        batch_size: usize,
    ) -> HipResult<()> {
        ...
        self.ensure_kernel(
            "fused_sigmoid_alpha_gate",
            kernels::FUSED_SIGMOID_ALPHA_GATE_SRC,
            "fused_sigmoid_alpha_gate_f32_tape",
        )?;
        let bp = beta.buf.as_ptr();
        let ap = alpha.buf.as_ptr();
        let dp = dt_bias.buf.as_ptr();
        let lp = a_log.buf.as_ptr();
        let atp = alpha_tape.buf.as_ptr();
        let btp = beta_tape.buf.as_ptr();
        let nn = n as i32;
        let mut params: Vec<*mut c_void> = vec![
            &bp as *const _ as *mut c_void,
            &ap as *const _ as *mut c_void,
            &dp as *const _ as *mut c_void,
            &lp as *const _ as *mut c_void,
            &atp as *const _ as *mut c_void,
            &btp as *const _ as *mut c_void,
            &nn as *const _ as *mut c_void,
        ];
        let block = 256u32;
        let grid = (n as u32).div_ceil(block);
        let bytes = n * 6 * 4 * batch_size;
        let timer = crate::profile::begin_timer(
            &self.hip,
            "fused",
            "fused_sigmoid_alpha_gate_f32_tape",
