...
impl Gpu {
    ...
    /// Muse Glimmer-owned RM2/BV6 gfx1100 residual GEMM with paired FP16
    /// accumulators (tied WMMA OPSEL probe).
    ///
    /// Sibling of [`Self::gemm_hfq4g256_residual_muse_gfx1100_rm`] (rm=2), NOT a
    /// replacement: measurement-only accuracy probe; touches no shared selector.
    /// Six `half16` paired accumulators hold r0 in low halves and r1 in high
    /// halves; each B fragment updates r0 then r1 via
    /// `__builtin_amdgcn_wmma_f16_16x16x16_f16_tied_w32` with literal false then
    /// true. Preserves exact g/kt/dequant/X-load order and residual `Y +=`
    /// semantics (widened from FP16). Exact gate/up only: M=19968, K=6656,
    /// batch=192. Grid `[624, 2, 1]`, block `[32, 1, 1]`, LDS 0.
    ///
    /// Returns `Ok(false)` without bind/compile when arch/shape/batch
    /// preconditions fail. Does not zero Y; performs `Y += W@X`.
    pub fn gemm_hfq4g256_residual_muse_gfx1100_rm2_f16acc_pair(
        &mut self,
        a: &GpuTensor,
        x: &GpuTensor,
        y: &GpuTensor,
        ...
        const KNAME: &str = "gemm_hfq4g256_residual_wmma_gfx1100_muse_rm2_bv6_f16acc_pair";
        if !self.arch_caps.is_gfx1100()
            || m != 19_968
            || k != 6_656
            || batch_size != 192
        ...
        self.ensure_kernel(
            ...
            kernels::GEMM_HFQ4G256_RESIDUAL_WMMA_GFX1100_MUSE_RM2_BV6_F16ACC_PAIR_SRC,
            KNAME,
        )?;
        let x_f16_ptr = self.ensure_fp16_x(x, batch_size * k)?;
