...
impl Gpu {
    ...
    pub fn fused_qkv_hfq4g256(
        ...
        let gfx1201_27b_r2 = gfx1201_27b_global
            && *GFX1201_27B_QKV_R2.get_or_init(|| {
                // Exact-shape QKV R2 is shadow-verified and spill-free.
                // Retain an opt-out for product bisects.
                !matches!(
                    std::env::var("HIPFIRE_GFX1201_HFQ4_27B_QKV_R2")
                        .ok()
                        .as_deref(),
                    Some("0" | "off" | "false")
                )
    ...
    pub fn fused_qkvza_hfq4g256(
        ...
        let (func_name, block, grid_x) = if gfx1201_27b_r2 {
            ...
            self.ensure_kernel(
                kernel,
                kernels::FUSED_QKVZA_HFQ4G256_R2_K5120_GLOBAL_GFX1201_SRC,
                kernel,
            )?;
            (kernel, [32u32, 1, 1], (total_m as u32).div_ceil(2))
        } else if gfx1201_27b_global {
    ...
    /// Dense Qwen3.5-27B QKV producer partition. This launches the exact
    /// shipping R2 body with the Z/beta/alpha row counts set to zero.
    pub fn fused_qkvza_hfq4g256_r2_k5120_qkv_gfx1201(
        &mut self,
        a_qkv: &GpuTensor,
        x: &GpuTensor,
        y_qkv: &GpuTensor,
    ) -> HipResult<()> {
        self.launch_qkvza_r2_k5120_partition_gfx1201(
            "fused_qkvza_hfq4g256_r2_k5120_qkv_gfx1201",
            kernels::FUSED_QKVZA_HFQ4G256_R2_K5120_QKV_GFX1201_SRC,
            a_qkv, a_qkv, a_qkv, a_qkv, x,
            y_qkv, y_qkv, y_qkv, y_qkv,
            10_240, 0, 0, 0,
        )
    }

    /// Dense Qwen3.5-27B Z/beta/alpha producer partition. It uses the same R2
    /// body with QKV rows set to zero, preserving each output row's reduction.
    #[allow(clippy::too_many_arguments)]
    pub fn fused_qkvza_hfq4g256_r2_k5120_zba_gfx1201(
        &mut self,
        a_z: &GpuTensor,
        a_beta: &GpuTensor,
        a_alpha: &GpuTensor,
        ...
        self.launch_qkvza_r2_k5120_partition_gfx1201(
            ...
            kernels::FUSED_QKVZA_HFQ4G256_R2_K5120_ZBA_GFX1201_SRC,
            a_z, a_z, a_beta, a_alpha, x,
            y_z, y_z, y_beta, y_alpha,
            0, 6_144, 48, 48,
        )
    }

    #[allow(clippy::too_many_arguments)]
    fn launch_qkvza_r2_k5120_partition_gfx1201(
        &mut self,
        kernel: &'static str,
        source: &'static str,
        a_qkv: &GpuTensor,
