...
//!
//!   cargo run --release -p rdna-compute --example parity_two_tier_e2e [n_total] [w_hot]
...
fn main() {
    ...
    let cold = compact_cold_kv(&cold_k, &cold_v, n_cold_tok, NKV, HD, &importance, 0.125, 4, true);
    ...
    let run_tier = |gpu: &mut Gpu, kp: &[f32], vp: &[f32], ns: usize| {
        ...
        gpu.attention_cold_slots(&qd, &kd, &vd, &od, &md, &ld, NH, NKV, ns, scale, 0, 0).unwrap();
        (od, md, ld)
    };
