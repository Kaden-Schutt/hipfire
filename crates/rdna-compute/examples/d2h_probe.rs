fn main() {
    ...
    let n = 6usize;
    let t = gpu.alloc_tensor(&[n], rdna_compute::DType::F32).expect("alloc");
    let big = gpu.alloc_tensor(&[4096 * 4096], rdna_compute::DType::F32).expect("big");
    let mut buf = vec![0u8; n * 4];
    // Cold: D2H with an idle pipeline.
    for _ in 0..20 { let _ = gpu.hip.memcpy_dtoh(&mut buf, &t.buf); }
    let t0 = std::time::Instant::now();
    for _ in 0..200 { gpu.hip.memcpy_dtoh(&mut buf, &t.buf).unwrap(); }
