...
fn diff(label: &str, gpu: &[f32], cpu: &[f32]) -> bool {
    ...
    println!("  {label:32}  max_abs={max_abs:.3e}  max_rel={max_rel:.3e}  {verdict}",);
    pass
}
...
fn test_fused_gate_up(gpu: &mut Gpu) -> HipResult<bool> {
    ...
    gpu.free_tensor(d_ag)?;
    gpu.free_tensor(d_au)?;
    gpu.free_tensor(d_x)?;
    gpu.free_tensor(d_yg)?;
    gpu.free_tensor(d_yu)?;
...
fn test_fused_qkv(gpu: &mut Gpu) -> HipResult<bool> {
    ...
    gpu.fused_qkv_mq4g256_lloyd(
        &d_aq, &d_ak, &d_av, &d_x, &d_yq, &d_yk, &d_yv, q_m, k_m, v_m, k,
    )?;
    ...
    gpu.free_tensor(d_aq)?;
    gpu.free_tensor(d_ak)?;
    gpu.free_tensor(d_av)?;
    gpu.free_tensor(d_x)?;
    gpu.free_tensor(d_yq)?;
    gpu.free_tensor(d_yk)?;
    gpu.free_tensor(d_yv)?;
...
fn test_fused_qkvza(gpu: &mut Gpu) -> HipResult<bool> {
    ...
    gpu.fused_qkvza_mq4g256_lloyd(
        &d_aqkv, &d_az, &d_ab, &d_aa, &d_x, &d_yqkv, &d_yz, &d_yb, &d_ya, qkv_m, z_m, beta_m,
        alpha_m, k,
    )?;
    ...
    gpu.free_tensor(d_aqkv)?;
    gpu.free_tensor(d_az)?;
    gpu.free_tensor(d_ab)?;
    gpu.free_tensor(d_aa)?;
    gpu.free_tensor(d_x)?;
    gpu.free_tensor(d_yqkv)?;
    gpu.free_tensor(d_yz)?;
    gpu.free_tensor(d_yb)?;
    gpu.free_tensor(d_ya)?;
