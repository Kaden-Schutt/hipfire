...
fn test_e4m3_ceil() -> bool {
    ...
    while s < 460.0 {
        ...
        if dec + 1e-6 < s && s < 448.0 {
            eprintln!("[FAIL] e4m3 ceil: s={s} code={code} dec={dec} < s"); ok = false; break;
        }
        // Smaller code must decode < s (otherwise not the ceil) — skip code 0.
        if code > 0 && s < 448.0 {
            let prev = e4m3_scale_decode(code - 1);
            if prev >= s { eprintln!("[FAIL] e4m3 not minimal: s={s} code={code} prev_dec={prev}>=s"); ok=false; break; }
        }
        s *= 1.013;
    }
    println!("[{}] e4m3 ceil round-trip", if ok {"PASS"} else {"FAIL"});
    ok
}
...
fn run(gpu: &mut Gpu, m: usize, k: usize, s1: &[f32], s2: &[f32]) -> bool {
    ...
    let row_bytes = 16 + 17 * (k/32);
    if packed.len() != m*row_bytes { eprintln!("[FAIL] size {} != {}", packed.len(), m*row_bytes); return false; }
    ...
    for i in 0..m*k { let d=(cpu_dq[i]-rot[i]) as f64; se += d*d; sn += (rot[i] as f64)*(rot[i] as f64); }
    let nrmse = (se/sn.max(1e-30)).sqrt();
    let nrmse_ok = nrmse < 0.15;
    println!("[{}] quant K={} NRMSE(vs rotated orig)={:.4}", if nrmse_ok {"PASS"} else {"FAIL"}, k, nrmse);
    if !nrmse_ok { return false; }

    // GPU dequant-to-f16.
    let d_packed = gpu.upload_raw(&packed, &[packed.len()]).unwrap();
    let f16_bytes = m*k*2;
    let d_f16 = gpu.upload_raw(&vec![0u8; f16_bytes], &[f16_bytes]).unwrap();
    gpu.dequantize_mfp4g32_p_to_f16(&d_packed.buf, &d_f16.buf, m, k).unwrap();
