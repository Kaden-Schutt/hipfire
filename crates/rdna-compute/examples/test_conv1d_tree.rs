...
fn main() {
    ...
    fails += run_case(
        &mut gpu, &input, &weight, &state, &parents, k_dim, v_dim, n, "spine", true,
    );
    ...
    fails += run_case(
        &mut gpu, &input_s, &weight, &state, &parents_s, k_dim, v_dim, n_s, "siblings", false,
    );
...
fn run_case(
    ...
    gpu.conv1d_silu_split_tree_f32_n(&q_t, &k_t, &v_t, &x, &w_gpu, &s_tree, &p, k_dim, v_dim, n)
        .unwrap();
    ...
    let mut fails = 0;
    if compare_linear {
        gpu.conv1d_silu_split_f32_n(&q_l, &k_l, &v_l, &x, &w_gpu, &s_lin, k_dim, v_dim, n)
            .unwrap();
...
fn cmp_exact(label: &str, a: &[f32], b: &[f32]) -> usize {
    ...
    for (i, (x, y)) in a.iter().zip(b.iter()).enumerate() {
        if x.to_bits() != y.to_bits() {
            if n < 3 {
                eprintln!("  {label}[{i}]: lin={x} tree={y}");
            }
            n += 1;
        }
    }
    if n > 0 {
        eprintln!("{label}: FAIL {n}/{}", a.len());
        1
    } else {
...
fn cmp_approx(label: &str, a: &[f32], b: &[f32], tol: f32) -> usize {
    ...
    if max_err > tol {
        eprintln!("{label}: FAIL max_err={max_err} tol={tol}");
        1
    } else {
        println!("{label}: approx-ok (max_err={max_err:.2e})");
        0
    }
}
...
fn cpu_ref(
    ...
    for t in 0..n {
        for c in 0..n_ch {
            ...
            let r = y / (1.0 + (-y).exp());
            let (write, idx) = if c < k_dim {
                (target == 0, t * k_dim + c)
            } else if c < 2 * k_dim {
                (target == 1, t * k_dim + (c - k_dim))
            } else {
                (target == 2, t * v_dim + (c - 2 * k_dim))
            };
            if write {
                out[idx] = r;
            }
