...
fn f32_to_f16_bits(v: f32) -> u16 {
    ...
    if exp == 255 { return (sign<<15) | 0x7C00 | if mant!=0 {0x200} else {0}; }
...
fn gemv_hfq4g256_explicit(gpu: &mut Gpu, a: &GpuTensor, x: &GpuTensor, y: &GpuTensor, m: usize, k: usize, rows: usize) -> Result<(), Box<dyn std::error::Error>> {
    gpu.gemv_hfq4g256_with_rows(a, x, y, m, k, rows).map_err(|e| format!("gemv_hfq4g256_with_rows R{}: {:?}", rows, e))?;
    Ok(())
}
const M: usize = 4096;
const K: usize = 5120;
...
fn print_vgpr_report(arch: &str) {
    println!("\n=== VGPR/SGPR/spill report (from .hipfire_kernels/{}/ *.radiowave.json) ===", arch);
    ...
    for (kname, fname) in wanted {
        ...
        while let Some(name_idx) = txt[pos..].find("\"name\"") {
            ...
            let vgpr = snippet.split("\"vgpr_count\"").nth(1).and_then(|s| s.split(|c: char| !c.is_ascii_digit()).find(|t| !t.is_empty())).and_then(|n| n.trim().parse::<u64>().ok()).unwrap_or(0);
            let sgpr = snippet.split("\"sgpr_count\"").nth(1).and_then(|s| s.split(|c: char| !c.is_ascii_digit()).find(|t| !t.is_empty())).and_then(|n| n.trim().parse::<u64>().ok()).unwrap_or(0);
            let vspill = snippet.split("\"vgpr_spill_count\"").nth(1).and_then(|s| s.split(|c: char| !c.is_ascii_digit()).find(|t| !t.is_empty())).and_then(|n| n.trim().parse::<u64>().ok()).unwrap_or(0);
            let sspill = snippet.split("\"sgpr_spill_count\"").nth(1).and_then(|s| s.split(|c: char| !c.is_ascii_digit()).find(|t| !t.is_empty())).and_then(|n| n.trim().parse::<u64>().ok()).unwrap_or(0);
            let privb = snippet.split("\"private_segment_fixed_size\"").nth(1).and_then(|s| s.split(|c: char| !c.is_ascii_digit()).find(|t| !t.is_empty())).and_then(|n| n.trim().parse::<u64>().ok()).unwrap_or(0);
            let spill_ok = if vspill==0 && sspill==0 && privb==0 { "OK zero spill" } else { "!!! SPILL !!!" };
            println!("  {:40} vgpr={:3} sgpr={:3} vspill={} sspill={} priv={:4}  {}", name, vgpr, sgpr, vspill, sspill, privb, spill_ok);
            found = true;
            pos = abs + 10;
            if pos >= txt.len() { break; }
            if txt[pos..].find("\"name\"").is_none() { break; }
        }
        if !found {
            println!("  {:35} — no kernel entries parsed in {}", kname, p.display());
        }
    }
}
...
fn main() {
    ...
    eprintln!("  warmup={} iters={} runs={}  interleave=sample-by-sample  timing=device hip events", WARMUP, ITERS, RUNS);
    eprintln!("  formats:");
    ...
    for &r in &rs {
        ...
        unsafe { std::env::set_var("HIPFIRE_GEMV_ROWS", r.to_string()); }
        eprintln!("[R={}] HIPFIRE_GEMV_ROWS set to {}", r, r);

        let mut gpu = Gpu::init().expect("gpu init");
        let arch = gpu.arch.clone();
        println!("  arch={}  HIPFIRE_GEMV_ROWS={}  gemv_rows_default={}", arch, r, gpu.arch_caps.gemv_rows_default());
        // build weight blobs
        let gpr = K / 256;
        let gpr128 = K / 128;
        ...
        let d_hf = GpuTensor { buf: hb, shape: vec![M, K], dtype: DType::Raw };
        ...
        for run in 0..RUNS {
            ...
            for (name, med) in [("mq4g128", g128_med), ("gl", gl_med), ("sel", sel_med)] {
                ...
                println!("      {:10}  time_ratio={:.4} ({} {}{} )  gbps_ratio={:.4}", name, time_ratio, if time_ratio<1.0 {"faster"} else {"slower"}, format!("{:.1}%", (1.0-time_ratio)*100.0), if time_ratio<1.0 {" faster"} else {" slower"}, gbps_ratio);
            }
        } // runs
    ...
    for &r in &rs {
        ...
        for fmt in ["hfq4g256", "mq4g128", "gl", "sel"] {
            ...
            let (r1, r2, r3) = (meds.get(0).copied().unwrap_or(f64::NAN), meds.get(1).copied().unwrap_or(f64::NAN), meds.get(2).copied().unwrap_or(f64::NAN));
            let time_ratio = if fmt=="hfq4g256" { 1.0 } else { mom / hf_mom };
            let gbps_ratio = if fmt=="hfq4g256" { 1.0 } else { gb / hf_gb_mom };
            let label = match fmt {
    ...
    unsafe { std::env::set_var("HIPFIRE_GEMV_ROWS", "2"); }
    ...
    let hf_r2 = all_results.get(&(2, "hfq4g256".to_string())).map(|v| median(v.clone())).unwrap_or(f64::NAN);
    let g128_r2 = all_results.get(&(2, "mq4g128".to_string())).map(|v| median(v.clone())).unwrap_or(f64::NAN);
    if hf_r2.is_finite() && g128_r2.is_finite() {
        ...
        println!("    hfq4g256 median {:.2} us  {:.1} GB/s  (weight {} B)", hf_r2, hf_gb, weight_bytes("hfq4g256"));
        ...
        println!("      The measured G128 penalty is therefore an UPPER BOUND on the v2 2x128 penalty.");
        if pct_slower < 5.0 {
