//! Tolerance-bounded A/B validation: gemm_hfq4g256_moe_grouped_mmq_k4_gfx1151
//! against gemm_hfq4g256_moe_grouped_wmma_k2 (the FP16 grouped base).
//!
//! The k4 path quantizes X to Q8_1 (7-bit signed int + per-32-element
//! scale), so outputs differ within the Q8_1 quantization noise envelope.
//! Output is numerically equivalent to the k2 i8 path modulo summation
...
//! as the k2 channel test.
//!
//! GFX1151 ONLY. Skips with a clear message on other archs.
//!
//! Run:
//!   cargo run --release -p rdna-compute --example test_moe_grouped_mmq_k4_gfx1151
...
fn run_case(
    ...
    if !arch.starts_with("gfx1151") {
        println!("  SKIP — arch {} is not gfx1151; i8 MMQ MoE grouped k4 kernel only registered for gfx1151", arch);
        return;
    }
    ...
    // Run i8 MMQ k4 path (gated to gfx1151 only — explicit direct call so
