...
//! MQ4V2 residual MW_LDS parity on exact gfx1201 — multi-wave LDS candidate gate.
//!
//! Pack deterministic MQ4V2 residual weights with disjoint halves. M=40 is not
//! 16-aligned so the final row tile is a partial tail. Nonzero deterministic
//! initial Y proves residual `Y += W@X`. On exact gfx1201:
//!   1) run current gfx12 production residual as the arithmetic oracle,
//!   2) run direct `gemm_mq4g256v2_residual_wmma_gfx1201_mw_lds` MW4/8/12
//!      for N∈{191,192,383,384,511,512},
//!  and compare for raw f32 bit equality plus finite/nondegenerate,
//!  relL2<=1e-5, cosine>=1-1e-6.
...
fn main() {
    ...
    eprintln!("arch {arch} confirmed exact gfx1201 — running residual MW_LDS parity (Y+=W@X)");
    ...
    // Boundary-crossing M: 40 is not 16-aligned (partial final row tile of 8).
    let m: usize = 40;
    let k: usize = 256; // not %512 so gfx12 ldsstage cannot steal the oracle
    assert_eq!(k % 256, 0);
    assert_eq!(m % 16, 8, "m must leave a partial final 16-row tile");
    eprintln!("geometry: m={m} K={k} final_row_tile={}", m % 16);

    let w = build_disjoint_halves(m, k);
    let blob = pack_mq4g256v2(&w, m, k);
    eprintln!("packed residual {} B", blob.len());
    let d_a = gpu
    ...
    for &n in &ns_mw {
        ...
        let x_host: Vec<f32> = (0..n * k)
            ...
            .collect();
        // Nonzero deterministic residual seed (offset away from zero).
        ...
        std::env::remove_var("HIPFIRE_MQ4V2_GFX1201_RESID_MW_LDS");
        let t_ref = std::time::Instant::now();
        let ref_res = gpu.gemm_hfq4g256_residual_wmma_gfx12_mq4v2(&d_a, &d_x, &d_y_ref, m, k, n);
        ref_res.expect("gfx12 base residual oracle failed");
        ...
        for &waves in &mws {
            ...
            gpu.hip
                ...
                .expect("htod y_mw");
            gpu.hip.device_synchronize().expect("sync y_mw");

            let res = gpu
                .gemm_mq4g256v2_residual_wmma_gfx1201_mw_lds(&d_a, &d_x, &d_y_mw, m, k, n, waves);
            if let Err(e) = res {
            ...
            if n == 512 {
                ...
                gpu.gemm_mq4g256v2_residual_wmma_gfx1201_mw_lds(
    ...
    if all_ok {
        eprintln!(
            "\nPASS: all N={{191,192,383,384,511,512}} x MW{{4,8,12}} residual raw-bit equal vs gfx12 base with nonzero Y init; finite/nondegenerate; relL2<=1e-5 cosine>=1-1e-6"
        );
    } else {
        eprintln!(
            "\nFAIL: one or more residual parity checks violated raw-bit equality or numeric thresholds"
        );
        std::process::exit(1);
    }
}
