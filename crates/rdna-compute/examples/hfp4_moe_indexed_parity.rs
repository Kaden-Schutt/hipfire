...
fn main() {
    ...
    let d_gate_b = gpu
        ...
        .expect("gate_b");
    let d_up_b = gpu.alloc_tensor(&[N * K_TOP * mi], DType::F32).expect("up_b");
    ...
    for bid in 0..N {
        for kr in 0..K_TOP {
            ...
            for r in 0..M {
                let w = &refs[eid][r];
                let want: f64 = (0..K)
                    .map(|i| w[i] as f64 * xb2[bid * K + i] as f64)
                    .sum();
                let stride = K_TOP * mi;
                let got = if r < mi {
    ...
    let d_resb = gpu.upload_f32(&vec![0.0f32; N * M], &[N * M]).expect("resb");
