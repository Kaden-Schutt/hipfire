...
//! `gemm_hfq6g256_moe_grouped_wmma_k2` (gfx11 _k2 variant).
//!
...
//!
//! GFX11 ONLY (gfx1100/gfx1151). The _k2 kernel uses half16_t / w32 WMMA
//! intrinsics available on RDNA3/3.5. Skips on non-gfx11 archs.
//!
//! Run:
//!   cargo run --release -p rdna-compute --example test_moe_grouped_wmma_hfq6_k2
...
/// to match `gemm_hfq6g256_moe_grouped_wmma_k2`'s inner-loop
/// precision; the WMMA op consumes a_reg as half16_t and accumulates in
...
fn run_case(label: &str, m: usize, k: usize, m_total: usize, num_experts: usize, seed_w: u32, seed_x: u32) {
    ...
    if !arch.starts_with("gfx11") {
        println!("  SKIP — arch {} is not gfx11; HFQ6 k2 kernel only registered for gfx11", arch);
        return;
    }
    ...
    gpu.gemm_hfq6g256_moe_grouped_wmma(
        ...
        m_total,
        m_total, // x_src_rows
    ).expect("hfq6 k2 grouped kernel launch");
    gpu.hip.device_synchronize().expect("sync after hfq6 k2 kernel");
    ...
    let ref_rel = &y_ref[argmax_rel];
    let gpu_rel = &y_gpu_v[argmax_rel];
    println!(
        "  max_rel_diff = {:.6e} (at {}: ref={:.6e}, gpu={:.6e})",
        max_rel, argmax_rel, ref_rel, gpu_rel
    );
    // HFQ6 accumulation tolerance:
    //   abs: 0.05 — HFQ6 dequant range is 4× HFQ4 (q 0..63 vs 0..15, scale
    //        ~0.01), so FP16 rounding per weight element is ~4× larger. For
    //        K=7168 with output magnitudes up to ~100, empirical max_abs stays
    //        below 0.05 (verified on a3b-slice).
    //   rel: 0.05 guarded by ref > 1e-2 — near-zero outputs (|ref| < 1e-2)
    //        have high relative noise from the FP16 floor; those are excluded
    //        from the rel check (they pass the abs check independently).
    //   The 1e-3 abs / 1e-2 rel band from the original comment was too tight
    //   for HFQ6 (written assuming HFQ4 precision; HFQ6 FP16 dequant error is
    //   ~4× larger because q values reach 63, not 15).
    let rel_ok = max_rel < 5e-2 || {
        // If max_rel is large, confirm it is ONLY due to near-zero elements.
        let argmax_ref = y_ref[argmax_rel].abs();
        argmax_ref < 1e-2
    };
    if max_abs > 5e-2 || !rel_ok {
        println!("  FAIL — exceeds tolerance (5e-2 abs; 5e-2 rel for |ref|>1e-2)");
        std::process::exit(1);
    } else {
        println!("  PASS");
    }
