...
fn main() {
    ...
    for &nn in &[5usize, 6, 7, 8, 9] {
        let xv: Vec<f32> = lcgf(100 + nn as u32, nn * k)
            .chunks_exact(4)
        ...
        let dq = gpu
            .upload_raw(&nib(999, 16 * (k / 2)), &[16 * (k / 2)])
            .unwrap();
        ...
        gpu.gemm_oq4_grouped_wmma(&w, &ws, &dq, &ds, &yb, m, k, nn, group)
            .unwrap();
        ...
        for r in 0..nn {
            ...
            gpu.gemm_oq4_grouped_wmma(&w, &ws, &q1, &s1, &yy, m, k, 1, group)
                .unwrap();
            gpu.device_synchronize().unwrap();
        ...
        let mut mxd = 0f32;
        for i in 0..nn * m {
            mxd = mxd.max((fbn[i] - fpn[i]).abs());
        }
        println!(
            "BOUNDARY n={nn}: wmma-vs-gemv max_abs={mxd:.6} -> {}",
            if mxd == 0.0 { "BIT-EXACT" } else { "DIFFERS" }
        );
    }
}
