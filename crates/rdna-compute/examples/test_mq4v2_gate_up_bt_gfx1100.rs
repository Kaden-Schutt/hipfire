...
//! MQ4V2 gate+up BT parity on exact gfx1100 — batch-column reuse gate.
//!
...
fn main() {
    ...
    eprintln!("arch {arch} confirmed exact gfx1100 — running gate+up BT parity");
    ...
    assert_eq!(
        (gate_m + up_m) % 16,
        0,
        "total rows should tile 16 for clean dispatch"
    );

    // Pack gate/up with disjoint halves.
    let w_gate = build_disjoint_halves(gate_m, k);
    let w_up = build_disjoint_halves(up_m, k);
