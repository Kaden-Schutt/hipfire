...
//!   source ./scripts/gpu-lock.sh && gpu_acquire "ssm-dgc"
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let cfg = SsmDrafterConfig {
        h_draft: 8, n_layers: 2, inter: 16, n_kv: 1, head_dim: 4, rope_base: 10000.0, eps: 1e-5,
    };
    let kvd = cfg.kv_dim();
...
fn ssm_param_names(n_layers: usize) -> Vec<String> {
    ...
    for li in 0..n_layers {
        for p in ["w_u", "w_g", "w_o", "wgate", "wup", "wdown", "norm1", "norm2"] {
