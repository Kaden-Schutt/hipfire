...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let path = std::env::args().nth(1).unwrap_or_else(|| {
        "/tmp/hfq-export/supra-50m-qtip3.hfq".to_string()
    });
    ...
    let e = entries.iter().find(|e| e.name == "model.norm.weight").ok_or("no model.norm.weight")?;
    println!("\nmodel.norm.weight: qt={} shape={:?} off={} size={}", e.quant_type, e.shape, e.data_offset, e.data_size);
    ...
    println!("  {} values, mean={mean:.4}, min={mn:.4}, max={mx:.4}, finite={finite}", vals.len());
    ...
    let spread = mx - mn;
    if finite == vals.len() && mean.abs() > 0.05 && mean.abs() < 100.0 && spread > 0.0 && spread < 100.0 {
        println!("\nPASS — norm decodes to real weight data (offset parsing correct).");
        Ok(())
    } else {
        Err(format!("norm values implausible (mean {mean}, spread {spread}) — offset parsing likely wrong").into())
    }
}
