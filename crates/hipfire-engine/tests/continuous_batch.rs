...
//! per-arch routing helpers not part of `hipfire-engine`.
    ...
    use hipfire_engine::scheduler::{
        is_batch_eligible, parse_continuous_batch_size, parse_serve_continuous_batch,
        BatchPendingRequest, BatchSampling, BatchSamplingKey,
        ContinuousBatchScheduler, DaemonInbox, DaemonMsg, BatchLane,
    };
    use hipfire_engine::terminal::{
        ...
        batch_terminal_control, batch_transfer_abort_to_singleton_and_clear, AttemptKey,
        BatchCommitTeardownClass, ClientTerminalDecision, LaneTicket,
    };
    ...
    fn stale_owner_generation_prevents_release_of_reused_slot() {
        ...
        assert!(matches!(sched.lanes[t2.lane], hipfire_engine::scheduler::BatchLane::Running(_)));
    ...
    fn batch_lane_done_metrics_guard_zero_and_one_token() {
        ...
        let zero_gen = hipfire_engine::scheduler::batch_lane_done_metrics(t0, Some(t1), Some(t2), t3, 16, 0);
    ...
    fn batch_lane_done_metrics_multi_token_decode_rate() {
        ...
        let m = hipfire_engine::scheduler::batch_lane_done_metrics(t0, Some(prefill), Some(first), end, 32, 5);
    ...
    fn lfm_capacity_exact_and_plus_one() {
        let _l = begin();
        // exact boundary fits
        assert!(!hipfire_engine::terminal::batch_lfm_exceeds_capacity(100, 10, 110));
        assert!(hipfire_engine::terminal::batch_lfm_admission_ok(100, 10, 110));
        // +1 exceeds
        assert!(hipfire_engine::terminal::batch_lfm_exceeds_capacity(100, 11, 110));
        assert!(!hipfire_engine::terminal::batch_lfm_admission_ok(100, 11, 110));
        // empty prompt invalid
        assert!(!hipfire_engine::terminal::batch_lfm_admission_ok(0, 10, 110));
    ...
    fn lfm_capacity_clamped_lane_vs_max_seq() {
        ...
        assert!(hipfire_engine::terminal::batch_lfm_exceeds_capacity(prompt, 10, lane_cap));
        assert!(!hipfire_engine::terminal::batch_lfm_admission_ok(prompt, 10, lane_cap));
        // Smaller prompt fits lane
        assert!(!hipfire_engine::terminal::batch_lfm_exceeds_capacity(500, 10, lane_cap));
        assert!(hipfire_engine::terminal::batch_lfm_admission_ok(500, 10, lane_cap));
    }
    ...
    fn lfm_capacity_u64_max_and_zero() {
        ...
        let huge = usize::MAX;
        assert!(hipfire_engine::terminal::batch_lfm_exceeds_capacity(10, huge, 4096));
        assert!(!hipfire_engine::terminal::batch_lfm_admission_ok(10, huge, 4096));
        // zero max_tokens: prompt alone must fit
        assert!(!hipfire_engine::terminal::batch_lfm_exceeds_capacity(100, 0, 4096));
        assert!(hipfire_engine::terminal::batch_lfm_admission_ok(100, 0, 4096));
        ...
        assert!(!hipfire_engine::terminal::batch_lfm_exceeds_capacity(4096, 0, 4096));
        assert!(hipfire_engine::terminal::batch_lfm_admission_ok(4096, 0, 4096));
        // zero prompt still invalid even with zero max
        assert!(!hipfire_engine::terminal::batch_lfm_admission_ok(0, 0, 4096));
    }
    ...
    fn lfm_capacity_zero_initial_and_refill_semantics() {
        ...
        let prompt = 100;
        let max = 0;
        assert!(hipfire_engine::terminal::batch_lfm_admission_ok(prompt, max, cap));
        ...
        assert!(!hipfire_engine::terminal::batch_lfm_admission_ok(2040, 10, cap)); // 2050 > 2048
    }
    ...
    fn batch_messages_are_single_user_requires_plain_string_content() {
        ...
        assert!(hipfire_engine::scheduler::batch_messages_are_single_user(&serde_json::json!({
