...
| [`plans/`](plans/) | planned / historical | Plans. |
| [`specs/`](specs/) | planned / historical | Specs. |
...
| [`superpowers/`](superpowers/) | historical | Workflow records. |
"""
...
class DocsReliabilityTests(unittest.TestCase):
    ...
    def test_active_docs_skills_executable_reference_fails(self) -> None:
        ...
        idx = VALID_INDEX.replace(
            "| [`superpowers/`](superpowers/) | historical | Workflow records. |\n",
            "| [`superpowers/`](superpowers/) | historical | Workflow records. |\n"
            "| [`skills/`](skills/) | branch-implemented / transitional | Non-exec only. |\n",
        )
        write(self.fx.root / "docs" / "INDEX.md", idx)
        # Reference from a dynamic active page (GETTING_STARTED) — must be caught
        # by centralized active scan, not only hardcoded ACTIVE_MARKDOWN.
        write(
            self.fx.root / "docs" / "GETTING_STARTED.md",
    ...
    def test_dynamic_active_page_dot_skills_reference_fails(self) -> None:
        # Non-hardcoded file under active classified collection methodology/
        # proves classification-driven active discovery (not ACTIVE_MARKDOWN only).
        write(
            self.fx.root / "docs" / "methodology" / "extra-note.md",
            "# Extra\n\nDo not use `.skills/foo` anymore.\n",
        )
        self.fx.commit_all("dynamic active dot skills")
        proc = self.fx.run_checker("--target-ref", "HEAD", "--base-ref", self.fx.base_ref)
        self.assertNotEqual(proc.returncode, 0)
        self.assertIn("skills-root:", proc.stdout)
